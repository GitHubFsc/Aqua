...
var AssetManagement = (function ($, Table, Dialog, Combo, Switcher, Tree) {
  let Pane = {
    ...
    initWidgets(data) {
      if (this.widgets) {
        ...
        // })
        // //节目类别
        // var genre = API.getPrice()
        // this.widgets.genre = new Combo('#genre_filter', Model.genre(genre), ComboStyle, (value) => {
        //   Pane.setSearch('genre', value)
        //   this.initTable();
        // })
      }
    },
      ...
      $('#setcolumn img[src$="Setcolumn.png"]').unbind().bind('click', ({currentTarget, target}) => {
        if (currentTarget == target) {
          ...
          var tableDefiner = new TableDefiner({titles,widgets}, (columns) => {
            ...
            this.bindEvents(columns.widgets);
          })
        }
      });
      //列表操作
      ...
      for(key in search) {
        ...
        switch (key) {
          ...
          case 'visible':
            break;
          default:
            let arr = [],getAssetDef =  API.getAssetDef((resp) => {
              arr =[...resp] ;
            });
  ...
  let Model = {
    ...
    category(data) {
      ...
      //     })
      //   }
      //   return this.defaultCombo(genre);
      // },
      Type(type,resp){
        var type = [{
          key: type.label,
