...
var node_bundle = (function($, Table){
        ...
        if (currentTarget == target) {
          ...
          function setTop(formattedData, selects) {
            if (selects.length > 0) {
              ...
              formattedData = Model.datasorting(formattedData,selectss);
  ...
  var Model = {
    ...
    datasorting(arr,selects){
      let datasorting=[],directory=[],program=[];
      for (var i = 0; i <Model.data.length; i++) {
        console.log('Model.data[i]',Model.data[i]);
      }
      if (selects.length>0) {
        for (var i = 0; i<arr.length; i++) {
          for (var j = 0; j<Model.data.length; j++) {
            if (Model.data[j].id==arr[i].id) {
              Model.data.splice(j, 1);
              j = j - 1;
            }
          }
        }
        for (var i = 0; i<Model.data.length; i++) {
          if(Model.data[i].index!=undefined){
            program.push({
              id: Model.data[i].id,
              index:Model.data[i].index
            })
          }else{
            directory.push({
              id: Model.data[i].id
        ...
        //   datasorting.push({
        //     id: directory[i].id,
        //     description:directory[i].description
        //   })
        // }
        for (var i = 0; i <arr.length; i++) {
        ...
        for (var i = 0; i <program.length; i++) {
          ...
          datasorting.push({
            id: program[i].id,
            index:program[i].index+arr.length
          })
        }
      }else{
        for (var i = 0; i <Model.data.length; i++) {
          if(Model.data[i].index){
            datasorting.push({
              id: Model.data[i].id,
              index:Model.data[i].index
            })
