...
function makeRequest (obj){
    ...
    addListeners(xhr);  //测试ftp请求
    xhr.open(type, url, async);
    xhr.onreadystatechange = function(){
      log('status='+xhr.status+' &readyState='+xhr.readyState);  //测试ftp请求
        if(xhr.readyState == 4){
    ...
    // 测试ftp请求在XMLHTTPRequest中触发各种事件
    function handleEvent(e) {
      log(e.type +':'+ e.loaded + 'bytes transferred');
    }
