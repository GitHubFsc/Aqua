...
UiCatStar.pageDown = function(){  //演出实况
  var _this = this;
  this.setGridView(function(){
    _this.changeGridFocus();
  }, 'next');
...
UiCatGroup.right = function(){
  ...
  if (this.isNav && (this.grid != null) && (this.grid.num > 0)) {
    _this.grid.focus.row = 0;  //从查询返回到第一行第一个内容
    this.isNav = false;
    this.removeNavFocus();
    this.changeGridFocus();
  } else {
...
UiCatGroup.down = function(){
  ...
  if (this.isNav) {  //搜索区域
    ...
    if(!this.grid.down() && !this.isNav){  //更改切换聚焦逻辑，最后一行内容翻页
      this.setGridView(function(){
        _this.changeGridFocus();
      }, 'next');
    }
...
UiCatGroup.pageDown = function(){  //艺术团
  if (!this.isNav) {
    var _this = this;
    this.setGridView(function(){
      _this.changeGridFocus();
...
UiGroupDetail.pageDown = function(){  //艺术团内容
  var _this = this;
  if (this.focus.row == 1) {
    this.setGridView(function(){
      _this.changeFocus();
