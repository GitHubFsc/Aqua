(function(globle, factory) {
  ...
  var Pane = {
    ...
    loadData: function() {
      ...
      self.api.getList(function(resp) {
        ...
        //console.log("self.model.data()=", self.model.data);
  ...
  var API = {
    ...
    getAssetList: function(data, callback) {
      ...
      var url = paas_host + '/aquapaas/rest/search/contents/asset?visible=all&bundle_id=' + bundle_id + '&bundle_id_op=eq&start=-1&sort_mode=a&sort_by=title&app_key=' + app_key + '&timestamp=' + new Date().toISOString()
      getJson(url, function(resp) {
        var _data = data
        _data.children = resp||[];
  ...
  var Model = {
    ...
    list: function(resp) {
      ...
      try {
        if (resp.ispackage == 'Y') {
          console.log("resp.ispackage="+resp.ispackage);
        ...
        var poster_data = data.metadata.PosterBoard||'',
            poster
        ...
        //console.log("poster_data",poster_data);
       // console.log("poster",poster);
        ...
        ret = {
          ...
          actors: data.metadata.Actors,
          director: data.metadata.Director,
          poster: poster,
          type: data.metadata.Description,
          summary: data.metadata.Summary_Short,
          pid: data.provider_id,
          paid: data.provider_asset_id,
