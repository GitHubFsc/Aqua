(function(global, factory) {
  global.CeLueQiTing = factory(jQuery, StyledList);
  var app = new CeLueQiTing();
})(this, (function($, Table) {
  var Pane = {
    table: null,
    search: {
      type: 'img',   //img：图文；video：视频；subtitle：字幕；
      state: 'enabled',  //enabled：启用；disabled：禁用；
      policyName: ''
    },
    isRefreshTable: { //焦点记忆
      isTrue: false,
    ...
    initPage() {
      if(my.paas.user_type === "0") {
          $("#ce_lue_qi_ting").html("<div style='height:100%;color:#797979;font-size:26px;text-align:center;line-height:600px;background-color:#fcfcf2;'>" + i18n("ADV_SYSUSER_NOTE_NORIGHT") + "</div>");
          return false;
      };
      //type聚焦
      $("#ce_lue_qi_ting .adPolicyAuditMenu .policy_type div[name=type]").removeClass("focus");
      $("#ce_lue_qi_ting .adPolicyAuditMenu .policy_type div[name=type]#ce_lue_qi_ting_" + Pane.search.type).addClass("focus");
      // status聚焦
      $("#ce_lue_qi_ting .adPolicyAuditMenu .policy_status div[name=status]").removeClass("focus");
      $("#ce_lue_qi_ting .adPolicyAuditMenu .policy_status div[name=status]#ce_lue_qi_ting_" + Pane.search.state).addClass("focus");
      return true;
    },
    bindBtnEvents() {
      var _this = this;
      // type 按钮切换
      $("#ce_lue_qi_ting .adPolicyAuditMenu .policy_type div[name=type]").click(function () {
          $("#ce_lue_qi_ting .adPolicyAuditMenu .policy_type div[name=type]").removeClass("focus");
          $(this).addClass("focus")
          switch ($(this).index()) {
              case 0:
      ...
      // status 按钮切换
      $("#ce_lue_qi_ting .adPolicyAuditMenu .policy_status div[name=status]").click(function () {
        $("#ce_lue_qi_ting .adPolicyAuditMenu .policy_status div[name=status]").removeClass("focus");
        $(this).addClass("focus")
        switch ($(this).index()) {
            case 0:
              Pane.search.state = 'enabled';
              break;
            case 1:
              Pane.search.state = 'disabled';
              break;
            default:
              break;
        }
      ...
      // 查询事件
      $("#ce_lue_qi_ting #ce_lue_qi_ting_searchInput").keydown(function (e) {  //回车查询事件
      ...
      $("#ce_lue_qi_ting #ce_lue_qi_ting_searchBtn").click(function () {  //查询按钮查询事件
          Pane.search.policyName = $("#ce_lue_qi_ting #ce_lue_qi_ting_searchInput").val();
          Pane.initTable();
      });
      // 列表操作
      $("#ce_lue_qi_ting #ceLueQiTingTable")
      .on("click", '.view', (e) => {   //查看
      ...
      .on('click', '.enable_state' ,(e) => {   //禁用
          let index = $(e.target).data("index");
          let state = Pane.search.state;
          API.enablePolicy(state, index, () => {
            Pane.isRefreshTable = {
              isTrue: true,
  ...
  var API = {
    ...
    getList(pageNum, callback) {
      ...
      let urlParam = [];
          ...
          urlParam.push("state=" + Pane.search.state);
    ...
    // 启用禁用
    enablePolicy(ret, index, callback) {
      // var confirm_result = confirm(ret == 'enabled' ? i18n("ADPOLICYAUDIT_AUDITDISABLEHINT") : i18n("ADPOLICYAUDIT_AUDITENABLEHINT"));
      // if(!confirm_result) {
      //   return;
      // }
      let url = paasHost + paasAdvDomain + "/ads/adpolicy/state";
      let method = "PUT";
      let urlParam = [];
          urlParam.push("state=" + (ret == 'enabled' ? "disabled" : "enabled")),
          urlParam.push("user_id=" + my.paas.user_id);
          urlParam.push("user_type=" + my.paas.user_type);
          urlParam.push("access_token=" + my.paas.access_token);
          urlParam.push("app_key=" + paasAppKey);
    ...
    table(data) {
      ...
      for(var i = 0; i < data.length; i++) {
          ...
          if(my.paas.user_type == "1"){
              ...
              var op_enable = "<span class='enable_state' data-index=" + i + ">" + (item.enable ? i18n("ADPOLICYAUDIT_TABLE_OPERATION_DISABLE") : i18n("ADPOLICYAUDIT_TABLE_OPERATION_ENABLE")) + "</span>";
          } else {
              var op_audithis = "<span style='cursor:default;color:#797979'>" + i18n("ADPOLICYAUDIT_TABLE_OPERATION_AUDITHIS") + "</span>";
              var op_enable = "<span style='cursor:default;color:#797979'>" + (item.enable ? i18n("ADPOLICYAUDIT_TABLE_OPERATION_DISABLE") : i18n("ADPOLICYAUDIT_TABLE_OPERATION_ENABLE")) + "</span>";
          }
          ...
          switch(item.state.indexOf(":") !== -1 ? item.state.split(":")[0] : item.state) {
            case "enabled":
              //启用
              if(Pane.search.state == 'enabled') {
                listData.push(row);
              }
              break;
            case "disabled":
              //禁用
              if(Pane.search.state == 'disabled') {
                listData.push(row);
              }
              break;
            default:
