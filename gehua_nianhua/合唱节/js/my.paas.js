...
my.paas.directLogin = function(user_name){
	var _this = this;
	this.user_name = user_name;
	this.request({
		url: this.host + '/aquapaas/rest/users/access_token/direct/' + user_name,
		type: 'GET',
		async: false,
		headers: {
...
my.paas.getNavigation = function(path, callback){
	var data = {};
  var url = this.host + path + '?children=1' + (window.AuditTreeVersion != null && window.AuditTreeVersion != '' ? '&tree_version=' + window.AuditTreeVersion : '');
	this.request({
		type: 'GET',
		url: url,
		async: true,
