(function(global, factory) {
  ...
  function exitApp() {
    ...
    if (deviceType.lastIndexOf('iPanel') > -1) { //linux
   ...
   */
  function savePortal() {
    if (deviceType.lastIndexOf('iPanel') > -1) { //iPanel
  ...
  function getHash(key) {
    var ret = ''
    var preHash = location.hash.replace(/^#/, '')
    var hash_array = preHash.split(';')
    for (var i = 0, len = hash_array.length; i < len; i++) {
      var item = hash_array[i].split('=');
      var hash_key =item[0]
      var hash_value = item[1]
      if (hash_key == key) {
        ret = hash_value
      }
    }
  ...
  /**
   * 设置页面的hash值
   * @param {str} str 字符串 key=value or key
   */
  /**
   * 设置页面的hash值
  ...
  function setHash(key, value) {
    ...
    if (hashIndex == -1) {
      hash_array.push([key, value].join('='))
    } else {
      hash_array = hash_array.slice(0, hashIndex).concat([key, value].join('=')).concat(hash_array.slice(hashIndex + 1))
    }
  ...
  function playVideo(opts) {
    ...
    // if (deviceType.lastIndexOf('iPanel') > -1) {
      var playParam = LinuxPlayParam
      playParam.vodObj.videoId = opts.videoId||''
      playParam.vodObj.videoName = encodeURIComponent(opts.name||'')
      playParam.freeVideoFlag = opts.freeVideoFlag||false
      console.log(JSON.stringify(playParam))
      iPanel.eventFrame.playParam = playParam;
      iPanel.eventFrame.gotoPage(LinuxPlayerUrl)
    // } else {
    //   getMovieId(paid, function(vspId, poid) {
    ...
    //       AssetID: paid,
    //       MovieID: vspId,//可选参数
    //       EpisodeID: vspId,//可选参数
    //       ReplayFlag: false,
    //       // Type: 1,
    //       ExtParam: 'app=' + PlayParamExtApp + ';poid=' + poid,
    //       PlayerURL: {
    //         Linux: '',
    //         Android: '',
    //         ThinClient: ''
  ...
  /**
   * 反查vspid以及poid
   * @param  {string}   paid     素材的paid
   * @param  {Function} callback 查询后的回调函数，返回vspid以及poid
   */
  function getMovieId(paid, callback) {
    var vspUrl = VSP_Host + "/vsp_outlet/catalog/reverse/json?id=" + VSP_ProgramId + "&paids=" + paid + "&caid=" + getParam('CardID') + "&key=" + VSP_AppKey;
    getJson(vspUrl, function(resp) {
      var vspId = resp.result.list[0].id;
      var poid = resp.result.list[0].provider.poid;
      callback && callback(vspId, poid);
    })
  }
  /**
   * 获取系统参数
   * @param  {string} key 系统参数名，以GHWEBAPI中定义为准
   * @return {string}     返回内容
   */
  ...
  function keyValue() {
    ...
    keyvalue.BACK = 27; //返回
    keyvalue.MENU = 93; //目录
    keyvalue.menu = 36; //PC端 菜单键
    keyvalue.Menu = 4097; //菜单键
