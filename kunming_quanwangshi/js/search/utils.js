(function(global, factory) {
    ...
    //获得数据
    function postJson(src, data, callback) {
        doRequest({
            type: 'post',
            url: src,
            data: data,
    ...
    function getUrlParam(key) {
        ...
        if (hasUrlParam) {
            ...
            if (key == '?backUrl') {
                ret = search.slice(search.lastIndexOf('?backUrl=') + 9)
            }
        }
        return ret;
    }
    ...
    function getMovieId(paid, callback) {
        var vspUrl = vsphost + "/vsp_outlet/catalog/reverse/json?id=" + nianhuaId + "&paids=" + paid + "&key=" + appKey + "&caid=" + getParam('CardID')
        getJson(vspUrl, function(resp) {
            var result = resp.result.list[0]
            var vspId = result.id;
            var poid = result.provider.poid;
            console.log('success, vspId:' + vspId + ', poid:' + poid);
