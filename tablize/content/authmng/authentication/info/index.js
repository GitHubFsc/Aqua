(function ($, Table) {
  let Pane = {
    ...
    initWidgets() {
      $("#authe_info ._frame_foot ._btn._download").bind("click", () => {
        let el = $("#authe_info #autheInfo_results > div");
        let data = {};
        let srcdata = el.data();
      ...
      $("#authe_info ._btn._query").bind("click", () => {
        autheInfo_dialog.init(undefined, (data) => {
          var dom = $("<div></div>");
          ...
          $("#authe_info #autheInfo_results").empty().append(dom);
          $("#authe_info ._btn._edit").show();
          API.getList(data, (resp) => {
            Pane.table.update(Models.table(resp))
          });
        });
      });
      $("#authe_info ._query_condition ._btn._edit").bind("click", () => {
        let el = $("#authe_info #autheInfo_results > div");
        let data = {
          type: el.data("type"),
        ...
        autheInfo_dialog.init(data, (data) => {
          var dom = $("<div></div>");
          for(var variable in data) {
            if(data.hasOwnProperty(variable)) {
              dom.data(variable, data[variable]);
      ...
      $("#authe_info #autheInfo_list_table").on("click", "a[name=detail]", (e) => {
        let index = $(e.target).data("index");
        let data = Pane._data[index];
        AutheInfoDetailDialog.init(data);
      })
    },
    initTable() {
      let title = [{
      ...
      this.table = new Table({
        containerId: "autheInfo_list_table",
        rows: 13,
        columns: 6,
        titles: title,
        styles: style,
