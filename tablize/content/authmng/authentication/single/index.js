(function ($, Table) {
  let Pane = {
    ...
    initWidgets() {
      $("#authe_single ._frame_foot ._btn._download").bind("click", () => {
        let el = $("#authe_single #autheSingle_results > div");
        let data = {};
        let srcdata = el.data();
      ...
      $("#authe_single ._btn._query").bind("click", () => {
        autheSingle_dialog.init({}, (data) => {
          var dom = $("<div></div>");
          ...
          $("#authe_single #autheSingle_results").empty().append(dom);
      ...
      $("#authe_single ._query_condition ._btn._edit").bind("click", () => {
        let el = $("#authe_single #autheSingle_results > div");
        let data = {
          type: el.data("type"),
        ...
        autheSingle_dialog.init(data, (data) => {
          var dom = $("<div></div>");
          for(var variable in data) {
            if(data.hasOwnProperty(variable)) {
              dom.data(variable, data[variable]);
      ...
      $("#authe_single #autheSingle_list_table").on("click", "a[name=detail]", (e) => {
        let index = $(e.target).data("index");
        let data = Pane._data[index];
        AutheSingleDetailDialog.init(data);
      })
    },
    initTable() {
      let title = [
        ...
        {
          label: i18n("AUTH_MNG_QUERYTABLE_RESULT_AUTHE")
        },
        {
      ...
      this.table = new Table({
        containerId: "autheSingle_list_table",
        rows: 13,
        columns: 10,
        titles: title,
        styles: style,
  ...
  let API = {
    getList(data, callback) {
        let putData = [];
        putData.push("event_time=" + data.startDate + "T00:00:00+0800&event_time_op=gt&event_time=" + data.endDate + "T23:59:59+0800&event_time_op=lt&user_id=" + data.smartcardId + "&user_id_op=eq&asset_type=" + data.type + "&asset_type_op=eq");
        $.ajax({
          type: "POST",
      ...
      download(data) {
        let putData = [];
        putData.push("event_time=" + encodeURIComponent(data.startDate + "T00:00:00+0800") + "&event_time_op=gt&event_time=" + encodeURIComponent(data.endDate + "T23:59:59+0800") + "&event_time_op=lt&user_id=" + data.smartcardId + "&user_id_op=eq&asset_type=" + data.type + "&asset_type_op=eq");
        let url = aquapaas_host + "/aquapaas/rest/search/contents/auditauthentication/report?properties=user_id,ticket_id,asset_id,bundle_id,product_id,create_time,effective_time,expire_time,state";
