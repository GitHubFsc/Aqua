var autheSingle_dialog = (function ($, Dialog, Combo, TimeSelector) {
  let Data = {},
    callback;
  let Pane = {
    dialog: null,
    ...
    initDialog() {
      ...
      this.dialog = new Dialog({
        url: "content/authmng/authentication/single/query.html",
        height: 460,
        width: 453,
        context: this,
        callback: callback
    ...
    initWidgets() {
      ...
      this.widget.date_range = new Combo({
        containerId: "autheSingle_date_range",
        options: [{
      ...
      this.widget.date_start = new TimeSelector({
        containerId: 'autheSingle_date_start',
      ...
      this.widget.date_end = new TimeSelector({
        containerId: 'autheSingle_date_end',
        calendarStyles: calendarStyles,
        dateInputStyles: {
          borderColor: '#d3d3d3'
        },
      ...
      this.widget.type_combo = new Combo({
        containerId: "autheSingle_type_range",
        options: [{
          label: "asset",
          value: "asset"
    			}, {
      ...
      $(".query_dialog_autheSingle ._dialog_body #autheSingle_self_define").bind("click", (e) => {
        ...
        if(value == i18n("AUTH_MNG_DIALOG_SELFDEFINE")) {
          ...
          this.widget.date_range.disable();
          $("#autheSingle_date_start").parent("._dialog_row").show();
          $("#autheSingle_date_end").parent("._dialog_row").show();
          $(".query_dialog_autheSingle").removeClass("_dialog_center")
        } else {
          ...
          $("#autheSingle_date_start").parent("._dialog_row").hide();
          $("#autheSingle_date_end").parent("._dialog_row").hide();
          $(".query_dialog_autheSingle").addClass("_dialog_center")
        }
      });
    },
    bindEvents() {
      $(".query_dialog_autheSingle ._dialog_foot ._query").bind("click", () => {
        Pane.dialog.close();
        callback && callback(Data);
      });
      $(".query_dialog_autheSingle ._dialog_title ._dialog_close").bind("click", () => {
        Pane.dialog.close();
      });
      $("._dialog_foot ._btn._cancel").bind("click", () => {
        Pane.dialog.close();
      });
      $(".query_dialog_autheSingle ._dialog_body #autheSingle_app_key").bind("blur", (e) => {
        Data.appKey = $(e.target).val();
      });
      $(".query_dialog_autheSingle ._dialog_body #autheSingle_smartcard_id").bind("blur", (e) => {
    ...
    loadData(){
      ...
      $(".query_dialog_autheSingle #autheSingle_app_key").val(Data.appKey);
      $(".query_dialog_autheSingle #autheSingle_smartcard_id").val(Data.smartcardId);
    }
  };
  let API = {
    zerolize(num, len) {
