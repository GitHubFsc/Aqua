(function ($, Table) {
  let Pane = {
    ...
    initWidgets() {
      $("#authe_batch ._frame_foot ._btn._download").bind("click", () => {
        let el = $("#authe_batch #autheBatch_results > div");
        let data = {};
        let srcdata = el.data();
      ...
      $("#authe_batch ._btn._query").bind("click", () => {
        autheBatch_dialog.init(undefined, (data) => {
          var dom = $("<div></div>");
          ...
          $("#authe_batch #autheBatch_results").empty().append(dom);
          $("#authe_batch ._btn._edit").show();
          API.getList(data, (resp) => {
            Pane.table.update(Models.table(resp))
          });
        });
      });
      $("#authe_batch ._query_condition ._btn._edit").bind("click", () => {
        let el = $("#authe_batch #autheBatch_results > div");
        let data = {
          type: el.data("type"),
        ...
        autheBatch_dialog.init(data, (data) => {
          var dom = $("<div></div>");
          for(var variable in data) {
            if(data.hasOwnProperty(variable)) {
              dom.data(variable, data[variable]);
      ...
      $("#authe_batch #autheBatch_list_table").on("click", "a[name=detail]", (e) => {
        let index = $(e.target).data("index");
        let data = Pane._data[index];
        AutheBatchDetailDialog.init(data);
      })
    },
    initTable() {
      let title = [
      ...
      this.table = new Table({
        containerId: "autheBatch_list_table",
        rows: 13,
        columns: 10,
        titles: title,
        styles: style,
