(function ($, Table) {
  let Pane = {
    ...
    initWidgets() {
      $("#autho_info ._frame_foot ._btn._download").bind("click", () => {
        let el = $("#autho_info #authoInfo_results > div");
        let data = {};
        let srcdata = el.data();
      ...
      $("#autho_info ._btn._query").bind("click", () => {
        authoInfo_dialog.init(undefined, (data) => {
          var dom = $("<div></div>");
          ...
          $("#autho_info #authoInfo_results").empty().append(dom);
          $("#autho_info ._btn._edit").show();
          API.getList(data, (resp) => {
            Pane.table.update(Models.table(resp))
          });
        });
      });
      $("#autho_info ._query_condition ._btn._edit").bind("click", () => {
        let el = $("#autho_info #authoInfo_results > div");
        let data = {
          type: el.data("type"),
        ...
        authoInfo_dialog.init(data, (data) => {
          var dom = $("<div></div>");
          for(var variable in data) {
            if(data.hasOwnProperty(variable)) {
              dom.data(variable, data[variable]);
      ...
      $("#autho_info #authoInfo_list_table").on("click", "a[name=detail]", (e) => {
        let index = $(e.target).data("index");
        let data = Pane._data[index];
        AuthoInfoDetailDialog.init(data);
      })
    },
    initTable() {
      let title = [{
          ...
          label: i18n("AUTH_MNG_TABLE_AUTHO_TOTAL")
        }, {
          label: i18n("AUTH_MNG_TABLE_AUTHO_SUCCESS")
        }, {
          label: i18n("AUTH_MNG_TABLE_AUTHO_FAIL")
        }, {
          label: i18n("AUTH_MNG_TABLE_AUTHO_RATE")
        }];
      let style = {
        borderColor: "#E2E2E2",
        borderWidth: 1,
      ...
      this.table = new Table({
        containerId: "authoInfo_list_table",
        rows: 13,
        columns: 6,
        titles: title,
        styles: style,
