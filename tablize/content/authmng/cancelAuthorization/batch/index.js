(function ($, Table) {
  let Pane = {
    ...
    initWidgets() {
      $("#cancelAutho_batch ._frame_foot ._btn._download").bind("click", () => {
        let el = $("#cancelAutho_batch #cancelAuthoBatch_results > div");
        let data = {};
        let srcdata = el.data();
      ...
      $("#cancelAutho_batch ._btn._query").bind("click", () => {
        cancelAuthoBatch_dialog.init(undefined, (data) => {
          var dom = $("<div></div>");
          ...
          $("#cancelAutho_batch #cancelAuthoBatch_results").empty().append(dom);
          $("#cancelAutho_batch ._btn._edit").show();
          API.getList(data, (resp) => {
            Pane.table.update(Models.table(resp))
          });
        });
      });
      $("#cancelAutho_batch ._query_condition ._btn._edit").bind("click", () => {
        let el = $("#cancelAutho_batch #cancelAuthoBatch_results > div");
        let data = {
          type: el.data("type"),
        ...
        cancelAuthoBatch_dialog.init(data, (data) => {
          var dom = $("<div></div>");
          for(var variable in data) {
            if(data.hasOwnProperty(variable)) {
              dom.data(variable, data[variable]);
      ...
      $("#cancelAutho_batch #cancelAuthoBatch_list_table").on("click", "a[name=detail]", (e) => {
        let index = $(e.target).data("index");
        let data = Pane._data[index];
        CancelAuthoBatchDetailDialog.init(data);
      })
    },
    initTable() {
      let title = [
        ...
        {
          label: i18n("AUTH_MNG_QUERYTABLE_CANCELAUTHTIME")
        },
        {
          label: i18n("AUTH_MNG_QUERYTABLE_RESULT_CANCELAUTH")
        },
        {
      ...
      let style = {
        ...
        iconColor: "#0099CB",
        columnsWidth: [0.11, 0.09, 0.09, 0.09, 0.10, 0.15, 0.14, 0.15, 0.09]
      }
      this.table = new Table({
        containerId: "cancelAuthoBatch_list_table",
        rows: 13,
        columns: 9,
        titles: title,
        styles: style,
  ...
  let API = {
    getList(data, callback) {
        ...
        for(var i = 0; i < data.smartcardId.length; i++) {
          ...
          putData.push("event_time=" + data.startDate + "T00:00:00+0800&event_time_op=gt&event_time=" + data.endDate + "T23:59:59+0800&event_time_op=lt&user_id=" + card_id + "&user_id_op=eq&asset_type=" + data.type + "&asset_type_op=eq&is_delete=1&is_delete_op=eq");
        }
        $.ajax({
          type: "POST",
      ...
      download(data) {
        ...
        for(var i = 0; i < data.smartcardId.length; i++) {
          ...
          putData.push("event_time=" + encodeURIComponent(data.startDate + "T00:00:00+0800") + "&event_time_op=gt&event_time=" + encodeURIComponent(data.endDate + "T23:59:59+0800") + "&event_time_op=lt&user_id=" + item + "&user_id_op=eq&asset_type=" + data.type + "&asset_type_op=eq&is_delete=1&is_delete_op=eq");
        }
        let url = aquapaas_host + "/aquapaas/rest/search/contents/auditauthorization/report?properties=user_id,ticket_id,asset_id,bundle_id,product_id,create_time,effective_time,expire_time,state&is_delete=1&is_delete_op=eq";
  ...
  let Models = {
    table(data) {
      ...
      for(var i = 0; i < data.length; i++) {
        ...
        row.push({
          label: API.convertTimeString(item.event_time || "")
        });
        row.push({
