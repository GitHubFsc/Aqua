  ...
  let Pane = {
    ...
    initTable() {
      ...
      let style = {
        ...
        iconColor: "#0099CB",
        columnsWidth: [0.03, 0.12, 0.12, 0.09, 0.14, 0.19, 0.15, 0.16]
      }
      Pane.table = new Table({
        containerId: "assetShelf_list_table",
        rows: 15,
        columns: 8,
        titles: title,
        styles: style,
    ...
    bindEvents() {
      ...
      .on('click', '.header_item.btn[name=on_off]', ({currentTarget, target}) => {
        if (currentTarget == target) {
          var els = $('.asset_shelf .i-check input:checked')
          var finish = [];
      ...
      .on('click', '.i-check input', ({currentTarget, target}) => {
        if (currentTarget == target) {
          $(currentTarget).prop('checked', 'checked');
        }
      })
      // 类型过滤
      var combo = new Combo("#assetShelf_type_filter", [
