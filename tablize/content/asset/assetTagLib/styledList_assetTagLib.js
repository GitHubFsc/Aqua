...
function AssetTagLibStyledList(obj){
  this.everyPageDatas={};
	var conf = obj;
	if(conf){
		...
		this.returnToObj=conf.returnToObj|| null;
		this.totalCount = conf.totalCount || 0;
		this.updateTitle = conf.updateTitle || false;
		this.async = conf.async;
		this.itemsCountOneRow= conf.itemsCountOneRow//一行显示几条数据,
		this.specialStyle=conf.specialStyle;
	}
	if(!this.containerId)
		return null;
	else
...
AssetTagLibStyledList.prototype.init = function(){
	this.footLabels = {
		navPre: '转到第',
		navSuf: '页，',
		curPre: '当前第',
...
AssetTagLibStyledList.prototype.create = function(){
	this.createList();
	var self = this;
...
AssetTagLibStyledList.prototype.getPageData = function(pageNumber, callback){

};

AssetTagLibStyledList.prototype.onTotalCount = function(count){
    this.totalCount = Number(count);
    this.pagesLmt = Math.ceil((this.totalCount)/this.itemsCountOneRow/this.rowsLmt);
};

AssetTagLibStyledList.prototype.update = function(param){
	if(this.listType === 0){
		this.data = param;
		this.totalCount = this.data && this.data.length  || 0;
		this.pagesLmt = Math.ceil(this.totalCount/this.rowsLmt);
...
AssetTagLibStyledList.prototype.fillContent = function(callback){
...
AssetTagLibStyledList.prototype.createList = function(){
...
AssetTagLibStyledList.prototype.fillList = function(){
	this.fillListHead();
	this.fillListBody();
	this.fillListFoot();
};

AssetTagLibStyledList.prototype.addListHead = function(jqTable){
...
AssetTagLibStyledList.prototype.fillListHead = function(){
	var self = this;
...
AssetTagLibStyledList.prototype.addListBody = function(jqTable){
...
AssetTagLibStyledList.prototype.fillListBody = function(){
	var jq=jQuery;
	var self = this;
	...
	var tranfer_text="page_"+this.currPage;
	var tranfer_text_datas="pageDatas_"+this.currPage;
	var resultArray=[];
	if(typeof this.everyPageDatas[tranfer_text]!=="undefined"){
		var curpageLastCheckArray=this.everyPageDatas[tranfer_text];
		jq("#"+this.containerId+"").find(".addTagsForMulti_checkBox").each(function(index_){
			if(jq.inArray(index_,curpageLastCheckArray)!=-1){
				jq(this).children().css("display","block");
			}
		})	
	};
	jq("#"+this.containerId+"").find(".addTagsForMulti_checkBox").each(function(index_){
	   	jq(this).unbind().bind("click", () => {
				var prevPage_text_datas="pageDatas_"+self.currPage+"index_"+index_+"";
				if(jq(this).children().css("display")=="none"){
					jq(this).children().css("display","block");
					self.everyPageDatas[prevPage_text_datas]=jq(this).closest("td").find(".tag_name_stored").text();
				}
				else{
					jq(this).children().css("display","none");
					delete self.everyPageDatas[prevPage_text_datas];
				}
        var tagsNameArray=[];
				for(var key in self.everyPageDatas){
					if(key.indexOf("pageDatas_")!=-1){
						var temp=self.everyPageDatas[key];
						tagsNameArray.push(temp)
					}
				}
				self.returnToObj.tagsNameArray=tagsNameArray;
			})
		})
  };

AssetTagLibStyledList.prototype.addListFoot = function(jqTable){
...
AssetTagLibStyledList.prototype.fillListFoot = function(){
	var self = this;
...
AssetTagLibStyledList.prototype.getPrevIcon = function(){
...
AssetTagLibStyledList.prototype.getNextIcon = function(){
	var iconColor = this.styles && this.styles.iconColor || 'black';
...
AssetTagLibStyledList.prototype.setStyles = function(jqTable){
	...
	if(!cellHeight){
		...
		cellHeight = Math.floor((jqTable.height() - titleHeight - footHeight - (this.rowsLmt + this.itemsCountOneRow) * cellspacing)/this.rowsLmt);
	}
	...
	tmpJQMB.find('td').each(function(){
		...
		var num_=self.columnsLmt/self.itemsCountOneRow;
		var specialStyle=self.specialStyle;
		var tdTextAlignLeftIndex=specialStyle.tdTextAlignLeftIndex;
		var tdColorBlueCursorPointerIndex=specialStyle.tdColorBlueCursorPointerIndex;
		if((jQuery(this).index()+tdTextAlignLeftIndex)%num_==0){
			jQuery(this).css({"text-align":"left","padding-left":"20px"})
		}
		else{
	     if(tdColorBlueCursorPointerIndex!="none"){
				if((jQuery(this).index()+tdColorBlueCursorPointerIndex)%num_==0)
				{jQuery(this).css({"cursor":"pointer","color":"#37a2ce"})}
			}
		}
	})
...
AssetTagLibStyledList.prototype.changePage = function(changeType){
  var that=this;
	var checkedArray=[];
	var checkedArrayValue=[];
	var prevPage = this.currPage;
	var prevPage_text="page_"+prevPage;
	var prevPage_text_datas="pageDatas_"+prevPage;
	var jq=jQuery;
  jq("#"+this.containerId+"").find(".addTagsForMulti_checkBox").each(function(index_){
		if(jq(this).children().css("display")=="block"){
			checkedArray.push(index_);
			//checkedArrayValue.push(that.content[index_])
		}
	});
  this.everyPageDatas[prevPage_text]=checkedArray;
	//this.everyPageDatas[prevPage_text_datas]=checkedArrayValue;
	switch(changeType){
		case -1:
...
AssetTagLibStyledList.prototype.refreshList = function(){
	var self = this;
	this.fillContent(function(){
		if(self.updateTitle){
			self.fillListHead();
...
AssetTagLibStyledList.prototype.updateInPage = function(data){
	//Same length of Data
	if(this.listType === 0){
		this.data = data;
		this.fillContent();
...
AssetTagLibStyledList.prototype.resize = function(){
    this.createList();
    this.fillList();
};
