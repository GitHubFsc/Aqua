var UserAdmin = (function($){
  ...
  var userList = {
    init: function(){
      ...
      self.listCnt = $('#sysUser_user-admin-index')[0];
    },
		...
		goToAuthPage:function(){
			...
			$("#sysUser_sys-user-admin-page2-bottom").show();
      ...
      var self=this;
		  ...
		  $(".tab_switch_unit").each(function(index,element){
				$(this).unbind().bind('click', function () {
				  ...
				  $("#sysUser_sys-user-admin-page2-part2").children().each(function(){
						$(this).hide()
					})
					$("#sysUser_sys-user-admin-page2-part2").children().eq(index).show();
					switch(index)
					{
					case 0:
						$("#sysUser_sys-user-admin-page2-part2-hiddenWords").hide();
						...
						break;
					case 1:
						$("#sysUser_sys-user-admin-page2-part2-hiddenWords").show();
		...
		bindBottomButtons:function(){
			var self=this;
			var authType=this.authType;
			$("#sysUser_sys-user-admin-page2-btn-cancel").unbind().bind('click', function () {
        self.returnFirstPage()
      });
			$("#sysUser_sys-user-admin-page2-btn-confirm").unbind().bind('click', function () {
        self.returnFirstPage();
		...
		initSecondPage:function(){
         ...
         this.dataStored="";
			...
			this.leftContainerId[this.authType]="sysUser_sys-user-admin-page2-part2-menu2-left-table";
			this.rightContainerId[this.authType]="sysUser_sys-user-admin-page2-part2-menu2-right";
		...
		initThirdPage:function(){
		  ...
		  this.getThirdPageData(function(resp){
			...
			this.leftContainerId[this.authType]="sysUser_sys-user-admin-page2-part2-menu3-left-table";
			this.rightContainerId[this.authType]="sysUser_sys-user-admin-page2-part2-menu3-right";
		  this.displayRightBodyLeft();
			this.displayRightBodyRight();
			this.bindRightPageLeftContainer();
			this.bindRightPageRightContainer();
      ...
      this.bindRightPageRightContainer();
		...
		initFirstPage:function(){//第一个权限页面 
		  ...
		  $(".tab_switch_unit").eq(0).addClass("tab_switch_unit_focus");
			$("#sysUser_sys-user-admin-page2-part2").children().eq(0).show();
			$("#sysUser_sys-user-admin-page2-part2").children().eq(1).hide();
			$("#sysUser_sys-user-admin-page2-part2").children().eq(2).hide();
			this.authType="menuAuth";
			this.bindBottomButtons();
			document.getElementById("sysUser_sys-user-admin-page2-part2-menu-tree-table").innerHTML="";
			document.getElementById("sysUser_sys-user-admin-page2-part2-menu-tree").innerHTML="";
			document.getElementById("sysUser_sys-user-admin-page2-part2-menu-tree-right").innerHTML="";
			var zTreeObj;
			var self=this;
			...
			*/			
			  callback: {
						...
						if(self.treeInitFinish){//初始化tree完成
							 if(treeNode.menuTreeId!="all"){
							    if(self.allChooseClickBySelf=="true"){//点击全选，跳过一个个刷新
										...
										var content="n";
										var zTree = $.fn.zTree.getZTreeObj("sysUser_sys-user-admin-page2-part2-menu-tree");
					...
					onCollapse:function(event, treeId, treeNode){
						var menuTreeId=treeNode.menuTreeId;
						if(menuTreeId=="all"){
							$("#sysUser_sys-user-admin-page2-part2-menu-tree-table").children().each(function(index){
								if(index!=0){
									$(this).hide()
								}
							})
			...
			zTreeObj = $.fn.zTree.init($("#sysUser_sys-user-admin-page2-part2-menu-tree"), setting, zNodes); //初始化zTree，三个参数一次分别是容器(zTree 的容器 className 别忘了设置为 "ztree")、参数配置、数据源
			...
			*/
			...
			for(var i= 0,len = self.currentUserMenuRightAllNotChoose.length;i<len;i++){
				$("#sysUser_sys-user-admin-page2-part2-menu-tree-table").append("<div style=\"height:"+treeRowDistanceRemoveBottomLine+"px;width:100%;border-bottom:1px #e2e2e2 solid\"></div>")
			};
		...
		drawMenuAuthRightPart:function(zNodesChildren){
			var currentUserMenuRight=this.currentUserMenuRight;
			var container=document.getElementById("sysUser_sys-user-admin-page2-part2-menu-tree-right");
			container.innerHTML="";
    ...
    handleEvents: function(){
      var self = this;
      $('#sysUser_sys-user-admin-user-list').on('click', '.opr-default', function(){
        var opr = $(this);
        ...
        case 'delete':
          ...
          break;
			  case 'audit':
					$("#sysUser_sys-user-admin-page1-part1,#sysUser_sys-user-admin-page1-part2").hide();
					$("#sysUser_sys-user-admin-page2-part1,#sysUser_sys-user-admin-page2-part2").show();
					self.goToAuthPage();
					//user
          break;
				 case 'resetPassword':
      ...
      $('#sysUser_user-admin-list-search').on('keydown', function(e){
        if(e.keyCode == 13){
          self.listSearch();
        }
      ...
      $('#sysUser_user-admin-user-add').on('click', function(){
		...
		returnFirstPage:function(){
		  this.hasFirstAuthPagedEntered=false;
			this.hasSecondAuthPagedEntered=false;
			this.hasThirdAuthPagedEntered=false;
			$("#sysUser_sys-user-admin-page2-bottom").hide();
			$("#sysUser_sys-user-admin-page1-part1,#sysUser_sys-user-admin-page1-part2").show();
			$("#sysUser_sys-user-admin-page2-part1,#sysUser_sys-user-admin-page2-part2").hide();
		},
		
    handleDropdowns: function(target, delegate){
      var self = this;
      if(self.listCnt != $('#sysUser_user-admin-index')[0]){
    ...
    setList: function(){
      ...
      self.list = new StyledList({
        ...
        rows: 15,
        columns: 9,
        containerId: 'sysUser_sys-user-admin-user-list',
        listType: 1,
        titles: [
    ...
    listSearch: function(){
      this.searchText = $('#sysUser_user-admin-list-search').val();
      this.list.update();
    },
      ...
      var dialog = new PopupDialog({
        ...
        context: {},
        callback: function(){
            $('#sysUser_user-admin-user-dialog-title').text(i18n("SYS_USER_RESET_PASSWORD"));
            $('#sysUser_user-admin-dialog-cancel').show();
            $('#sysUser_user-admin-dialog-submit').html('确&nbsp;&nbsp;定')
              .on('click', function(){
                if($('#sysUser_user-admin-user-pwd2').val()==""){
									showMsgDialogOneButton(i18n("SYS_USER_CONFIRM_INPUT_PASSWORD"));
								}
								else if ($('#sysUser_user-admin-user-pwd-check-new').val()==""){
									showMsgDialogOneButton(i18n("SYS_USER_CONFIRM_INPUT_CONFIRM_PASSWORD"));
								}
								else if($('#sysUser_user-admin-user-pwd2').val()!=$('#sysUser_user-admin-user-pwd-check-new').val()){
									showMsgDialogOneButton(i18n("SYS_USER_CONFIRM_INPUT_PASSWORD_DIFF2"))
								}
								else{
									self.resetUserPassword(user.user_name,$('#sysUser_user-admin-user-pwd2').val());
									dialog.close();
								}
              });
          }
    ...
    openUserDialog: function(uiType, user){
      ...
      var dialog = new PopupDialog({
        ...
        callback: function(){
          switch(uiType){
          case 'add':
            $('#sysUser_user-admin-user-dialog-title').text(i18n("SYS_USER_ADDUSER"));
            $('#sysUser_user-admin-dialog-cancel').show();
            $('#sysUser_user-admin-dialog-submit').html(i18n("SYS_USER_ADD"))
              .on('click', function(){
          ...
          case 'edit':
            ...
            $('#sysUser_user-admin-user-dialog-title').text(i18n("SYS_USER_EDIT_USER"));
            $('#sysUser_user-admin-user-name').val(user.user_name).attr("readonly", "readonly").css("background-color","#e0e0e0");
						...
						*/
            $('#sysUser_user-admin-user-nickname').val(user.nick_name);
            $('#sysUser_user-admin-user-mobile').val(user.mobile);
						/*
						$("#user-admin-user-pwd").focus(function(){
							$(this).val(""); 
						});
						*/
						$("#sysUser_user-admin-user-pwd-check").focus(function(){
							$(this).val(""); 
						});
						
            $('#sysUser_user-admin-dialog-cancel').show();
            $('#sysUser_user-admin-dialog-submit').html(i18n("SYS_USER_UPDATE"))
              .on('click', function(){
    ...
    addUser: function(user, dialog){
		  var self=this;
      var ret = false;
      var user_name = $('#sysUser_user-admin-user-name').val();
      if(user_name==""){
        showMsgDialogOneButton(i18n("SYS_USER_CONFIRM_INPUT_USERNAME"));
        return "confirm";
      }
			else if($('#sysUser_user-admin-user-pwd').val()==""){
        showMsgDialogOneButton(i18n("SYS_USER_CONFIRM_INPUT_PASSWORD"));
        return "confirm";
      }
			else if($('#sysUser_user-admin-user-pwd-check').val()==""){
        showMsgDialogOneButton(i18n("SYS_USER_CONFIRM_INPUT_CONFIRM_PASSWORD"));
        return "confirm";
      }
			else if($('#sysUser_user-admin-user-pwd').val()!=$('#sysUser_user-admin-user-pwd-check').val()){
      ...
      var pwd = $('#sysUser_user-admin-user-pwd').val();
      var pwdCheck = $('#sysUser_user-admin-user-pwd-check').val();
      if(pwd != pwdCheck){
        showMsgDialogOneButton(i18n("SYS_USER_PASSWORD_CONFIRM"));
        return false;
      }

      var nickname = $('#sysUser_user-admin-user-nickname').val();
      var mobile = $('#sysUser_user-admin-user-mobile').val();
