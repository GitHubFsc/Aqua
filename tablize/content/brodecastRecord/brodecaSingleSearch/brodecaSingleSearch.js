...
brodecaSingleSearch.checkSizeStyle=function(){//伸缩弹出窗体
  if(document.getElementById("singleQuery_broSingle_selfDefine").innerHTML=="自定义")//日期部分为非自定义
  {
    if(jQuery("#singleQuery_preSetContainer").css("display")!="none")//组合类型为预设组合
    {
      return "date_notSelfDefined_combine_preset"
    }
    else{
      if(jQuery("#singleQuery_broSingle_broType").find("#singleQuery_downMenuText").text()=="时移回看")
      {
...
brodecaSingleSearch.initDefaultTale=function(){
	...
	jQuery("#singleQuery_tableContent tbody").append("<tr></tr>");
	for(var a=0;a<tdLength;a++)
	{
		jQuery("#singleQuery_tableContent tr:eq(0)").append("<td style=\"width:"+td_[a]+"px\"></td>")
	}
	for(var b=0;b<trLength;b++)
	{
		var html_="";
		for(var a=0;a<tdLength;a++){html_=html_+"<td></td>"}
		jQuery("#singleQuery_tableContent").append("<tr>"+html_+"</tr>")
	}
	document.getElementById("singleQuery_tableContent").style.visibility="visible"
}
...
brodecaSingleSearch.switchRedio=function(obj){
  ...
  if(text_=="singleQuery_radioB1"){
    document.getElementById("singleQuery_combineType_title").innerHTML="预设组合";
    document.getElementById("singleQuery_preSetContainer").style.display="block";
    document.getElementById("singleQuery_broSingle_broType").style.display="none";
    document.getElementById("singleQuery_singleSearch_displayIfTypeSelfDefined").style.display="none";
    document.getElementById("singleQuery_singleSearch_displayIfTypeSelfDefinedB").style.display="none";
  }
  else{
    document.getElementById("singleQuery_combineType_title").innerHTML="点播类型";
    jQuery("#singleQuery_broSingle_broType").find("#singleQuery_downMenuText").text("服务包(SVOD)");
    document.getElementById("singleQuery_preSetContainer").style.display="none";
    document.getElementById("singleQuery_broSingle_broType").style.display="block";
    document.getElementById("singleQuery_singleSearch_displayIfTypeSelfDefined").style.display="block";
    document.getElementById("singleQuery_singleSearch_displayIfTypeSelfDefinedB").style.display="block";
  }
...
brodecaSingleSearch.resetPopupSize=function(value_){
  ...
  jQuery("#singleQuery_singleSearchPopBody").height(temp_[0]);
  jQuery("#singleQuery_centerPopupStyle").height(temp_[1]);
...
brodecaSingleSearch.userDefined=function(obj){
  ...
  switch(test_)
  {
    case "自定义":
      ...
      document.getElementById("singleQuery_downMenuText").innerHTML="自定义";
      jQuery("#singleQuery_broSingle_dateRegion").css("background-color","#ebebeb");
      jQuery("#singleQuery_broSingle_dateRegion").find(".smallArrowInDiv").attr("src","images/downArrow-dark.png");
      document.getElementById("singleQuery_broSingle_startEnd").style.display="block";
      jQuery("#singleQuery_date-input-from").empty();
      jQuery("#singleQuery_date-input-to").empty();
      this.dataPickerIssue();
      jQuery("#singleQuery_downMenu").css("display","none");
      break;
    case "取消自定义":
      jQuery(obj).text("自定义");
      document.getElementById("singleQuery_downMenuText").innerHTML="一个月";
      jQuery("#singleQuery_broSingle_dateRegion").css("background-color","#ffffff");
      jQuery("#singleQuery_broSingle_dateRegion").find(".smallArrowInDiv").attr("src","images/downArrow-SkyBlue.png");
      document.getElementById("singleQuery_broSingle_startEnd").style.display="none";
      break;
    default:
      break;
  }
...
brodecaSingleSearch.switchPreset= function(obj){
  ...
  {
    var id__=ele_[a].id;
    if(id__&&id__=="singleQuery_selectedPreset")
    {
      jQuery("#singleQuery_selectedPreset").find("div[name='radio1']").attr('class','radioB_noSelected');
      ele_[a].style.backgroundColor="#ffffff";
      ele_[a].setAttribute("id","");
      break
    }
  }
  jQuery(obj).attr('id','singleQuery_selectedPreset');
  jQuery(obj).css("background-color","#f5fdff");
  jQuery("#singleQuery_selectedPreset").find("div[name='radio1']").attr('class','radioB_selected');
};

brodecaSingleSearch.writePopContent= function(indexNum){
  document.getElementById("singleQuery_broad_singleSearch_combineType").style.display="none";
  var indexNum=indexNum;
  var temp=BROADCAST_PO_DEFAULT;
  var content=temp[indexNum];
  document.getElementById("singleQuery_broad_singleSearch_broadType").innerHTML=content.POTYPE?content.POTYPE:"";
  document.getElementById("singleQuery_broad_singleSearch_servicePackage").innerHTML=content.POIDS?"PO ID":"";
  jQuery("#singleQuery_inputText_id_field").val(content.POIDS?content.POIDS:"");
}

brodecaSingleSearch.enddateAappear= function(){
    var current_height=jQuery("#singleQuery_centerPopupStyle").height();
	if(jQuery("#singleQuery_downMenuText").text().indexOf("自定义")!=-1)
	{
		...
		jQuery("#singleQuery_centerPopupStyle").height(tempHeight);
...
brodecaSingleSearch.clearAllTableData=function(){
	...
	for(i=0;i<jQuery("#singleQuery_tableContent tr").length;i++)
		{
		for(j=0;j<jQuery("#singleQuery_tableContent tr:eq("+i+") td").length;j++)
			{jQuery("#singleQuery_tableContent tr:eq("+i+") td:eq("+j+")").text("")}
		};
		jQuery(".spanDiv .inputShift").text("")
}

brodecaSingleSearch.exportResult= function(){
	if((jQuery("#singleQuery_tableContent tr:eq(0)").find("td:eq(0)").text()!="")&&(jQuery("#singleQuery_tableContent tr:eq(0)").find("td:eq(0)").text()!=undefined))
		{
...
brodecaSingleSearch.popup_newSearch= function(){
  ...
  this.dialog.open();
  jQuery("#singleQuery_cardIDinput").val("");
  jQuery("#singleQuery_date-input-from #singleQuery_date-input-from-datepicker-input").val("");
  jQuery("#singleQuery_date-input-to #singleQuery_date-input-to-datepicker-input").val("");
  this.dateInputChosed_appear1="true";
  this.dateInputChosed_disappear1="false";
  this.current_popup_height=jQuery("#singleQuery_centerPopupStyle").height();
}
...
brodecaSingleSearch.searchOriginVal= function(){
	jQuery("#singleQuery_cardIDinput").val(brodecaSingleSearch.cardIDInput);
	jQuery("#singleQuery_date-input-from").val(brodecaSingleSearch.begindateInput);
	jQuery("#singleQuery_date-input-to").val(brodecaSingleSearch.enddateInput);
...
brodecaSingleSearch.callScroll=function(){
            jQuery("#singleQuery_tableContentOuterr").mCustomScrollbar(
				{
					//setHeight:280,
					theme:"purple"
				});
...
{	
	jQuery(window).unbind("resize");
		var bodyWidth=jQuery("#tabs_content").innerWidth();
		var BodyContainerHeight=jQuery("#tabs_content").height()-jQuery("#singleQuery_boRecord_pageSingleSearch").outerHeight()-jQuery("#singleQuery_pageAllBodyContainer").css("marginTop").replace(/px/g,"");
		...
		var tableRowHeight=(BodyContainerHeight-jQuery(".pageAllBodyContainerTitle").outerHeight()-jQuery(".spanDivContainer").outerHeight()-parseInt(jQuery("#singleQuery_resultTable").css("top").replace(/px/g,""))-jQuery("#singleQuery_buttonDown").outerHeight()-parseInt(jQuery("#singleQuery_buttonDown").css("marginTop").replace(/px/g,""))-document.getElementById("singleQuery_brodecastRecordTableTread").offsetHeight)/11;
        jQuery("#singleQuery_tableContent tr td").css("height",tableRowHeight);
		var tableContentNoTitle= tableRowHeight*11+14;
		
		brodecaSingleSearch.tableRowHeighT=tableRowHeight;
		jQuery("#singleQuery_tableContentOuter").css("height",tableContentNoTitle);
        var  height_=document.getElementById("singleQuery_tableContentOuter").offsetHeight+"px";
        jQuery("#singleQuery_tableContentOuterr").height(height_);
        ...
        document.getElementById("singleQuery_pageAllBodyContainer").style.visibility="visible"
	}
...
brodecaSingleSearch.ifPreSet=function(str){
  ...
  {
    case "预设组合":
      document.getElementById("singleQuery_broad_singleSearch_combineType").style.display="block"
      break;
    default:
      break;
  }
...
if(data.details){
	...
	jQuery("#singleQuery_tableContent").empty();
  ...
  if (data.statistics) {
	...
	jQuery("#singleQuery_spanAccID").text(jQuery("#singleQuery_tableContent tr:eq(0)").find("td:eq(0)").text());
	jQuery("#singleQuery_spanAccName").text(jQuery("#singleQuery_tableContent tr:eq(0)").find("td:eq(1)").text());
	jQuery("#singleQuery_spanCardNo").text(jQuery("#singleQuery_tableContent tr:eq(0)").find("td:eq(2)").text());
	jQuery("#singleQuery_spanBuyCount").text("");
	jQuery("#singleQuery_spanBuyCount").text(spanBuyCount);
	jQuery("#singleQuery_spanTurnover").text("");
	jQuery("#singleQuery_spanTurnover").text(turnovercount);
	jQuery("#singleQuery_spanBroCount").text("");
	jQuery("#singleQuery_spanBroCount").text(spanBroCount);
	jQuery("#singleQuery_spanBroTime").text(broTime);
	spanBuyCount=0;
	spanBroCount=0;
	broTime=0;
	turnovercount=0;

	jQuery("#singleQuery_buttonDown").css("background-color","#2ea2d7");
	}
	else{
...
brodecaSingleSearch.zerolize = function(num, len) {
  var _returnValue = "";
...
brodecaSingleSearch.singleSearch= function()
{
  ...
  if(jQuery("#singleQuery_cardIDinput").val()==""){alert("请填写卡号，谢谢。");return}
  url_=url_+jQuery("#singleQuery_cardIDinput").val();
  //时间项
  var start_date="";var end_date="";
  if(jQuery("#singleQuery_broSingle_selfDefine").text()=="自定义")//非自定义时间
  {
    var time_=jQuery("#singleQuery_broSingle_dateRegion").find("#singleQuery_downMenuText").text();
    var num__=1;
    ...
    end_date=_cur_date.getFullYear() + "-" + brodecaSingleSearch.zerolize(_cur_date.getMonth() + 1, 2) + "-" + brodecaSingleSearch.zerolize(_cur_date.getDate(), 2);
    start_date= _begin_d.getFullYear() + "-" + brodecaSingleSearch.zerolize(_begin_d.getMonth() + 1, 2) + "-" + brodecaSingleSearch.zerolize(_begin_d.getDate(), 2);
  }
  else{  //自定义选择日历时间
    start_date=jQuery("#singleQuery_date-input-from #singleQuery_date-input-from-datepicker-input").val();
    end_date=jQuery("#singleQuery_date-input-to #singleQuery_date-input-to-datepicker-input").val();
    if(jQuery("#singleQuery_date-input-from #singleQuery_date-input-from-datepicker-input").val()=="")
    {alert("开始日期不能为空！谢谢。");}
    else if(jQuery("#singleQuery_date-input-to #singleQuery_date-input-to-datepicker-input").val()=="")
    {alert("结束日期不能为空！谢谢。");}
  }
  url_=url_+"?start_date=" +start_date+"&end_date=" + end_date;

  //如果预设组合，增加预设值
  if(jQuery("#singleQuery_preSetContainer").css("display")!="none")
  {
    if(document.getElementById("singleQuery_selectedPreset")==null){}
    else{
      var value_=jQuery("#singleQuery_selectedPreset").find(".divContainerSon").text();
    ...
    if(jQuery("#singleQuery_broSingle_broType").find("#singleQuery_downMenuText").text()=="时移回看")
    {
      ...
      var text_=jQuery("#singleQuery_broSingle_broType").find("#singleQuery_downMenuText").text();
      switch(text_)
      {
        case "按次点播(RVOD)":
          var POTYPE_="1,3";var poids=jQuery("#singleQuery_singleSearch_inputIDregion").val();
        ...
        case "服务包(SVOD)":
          ...
          var inputText_package_id=jQuery("#singleQuery_singleSearch_inputIDregion").val();
  ...
  var cardIDInput=jQuery("#singleQuery_cardIDinput").val()+",";
  brodecaSingleSearch.cardIDInput=jQuery("#singleQuery_cardIDinput").val();
  var begindateInput="("+start_date+"--";
  brodecaSingleSearch.begindateInput=jQuery("#singleQuery_date-input-from #singleQuery_date-input-from-datepicker-input").val();
  var enddateInput= end_date+")";
  brodecaSingleSearch.enddateInput=jQuery("#singleQuery_date-input-to #singleQuery_date-input-to-datepicker-input").val();
  brodecaSingleSearch.date_field= jQuery("#singleQuery_downMenuText").text();
  if((jQuery("#singleQuery_cardIDinput").val()!="")&&(jQuery("#singleQuery_date-input-from #singleQuery_date-input-from-datepicker-input").val()!="")&&(jQuery("#singleQuery_date-input-to #singleQuery_date-input-to-datepicker-input").val()!=""))
  {
    jQuery("#singleQuery_cardID-result").text(cardIDInput);
    jQuery("#singleQuery_begindate-result").text(begindateInput);
    jQuery("#singleQuery_enddate-result").text(enddateInput);
    if(jQuery("#singleQuery_newCreateButton").length==0){
      jQuery("#singleQuery_pageAllBodyContainerTitle").append("<div alt=\"edit\" id= \"singleQuery_newCreateButton\" class=\"pageTitleContainerRightButt\" onclick=\"brodecaSingleSearch.popup_editSearch();\" style=\"background-color:#4eb7f0;border-color:#0b88ca;float:left;width:70px;height:28px;line-height:28px;margin-left:3px;margin-top:4px;font-size:14px;font-family: 'Hiragino';\">编&nbsp;辑</div>")};
    brodecaSingleSearch.current_popup_height=jQuery("#singleQuery_centerPopupStyle").height();
...
brodecaSingleSearch.mopen = function(object,id,isFather){
  if( (id=="singleQuery_downMenu")&&( (document.getElementById("singleQuery_downMenuText")!=null)&&(document.getElementById("singleQuery_downMenuText").innerHTML.indexOf("自定义")!=-1 )) )
  {
    return
  }
  jQuery(".downMenu").each(function(){
    ...
    if(id=="singleQuery_downMenu_bro_typeCombine")
    {
      var id_=jQuery(object).attr("id");
      var text_="";
      switch(id_)
      {
        case "singleQuery_typeCombin1":
          document.getElementById("singleQuery_singleSearch_displayIfTypeSelfDefined").style.display="block";
          document.getElementById("singleQuery_singleSearch_displayIfTypeSelfDefinedB").style.display="block";
          document.getElementById("singleQuery_singleSearch_displayIfTypeSelfDefinedB").innerHTML="请输入服务包代号(Service Package),用“,”号分隔";
          break;
        case "singleQuery_typeCombin2":
          document.getElementById("singleQuery_singleSearch_displayIfTypeSelfDefined").style.display="block";
          document.getElementById("singleQuery_singleSearch_displayIfTypeSelfDefinedB").style.display="block";
          document.getElementById("singleQuery_singleSearch_displayIfTypeSelfDefinedB").innerHTML="请输入PO ID , 用“,”号分隔";
          break;
        case "singleQuery_typeCombin3":
          document.getElementById("singleQuery_singleSearch_displayIfTypeSelfDefined").style.display="none";
          document.getElementById("singleQuery_singleSearch_displayIfTypeSelfDefinedB").style.display="none";
          break;
        default:
...
brodecaSingleSearch.change_link_broType = function(obj,dropMenu_title_ID) {
  var current_height=jQuery("#singleQuery_centerPopupStyle").height();
  var diverse_height=jQuery("#singleQuery_broad_singleSearch_servicePackage_downMenu_container").outerHeight(true)+jQuery("#singleQuery_inputText_id_field").outerHeight(true)+jQuery("#singleQuery_broad_singleSearch_id_input_noteWords").outerHeight(true)//消失或出现的这些元素的高度。
  if(jQuery(obj).text()=="RVOD")
  {
    //var new_height=(current_height-diverse_height)+"px";
    var new_height=current_height;
    if(jQuery("#singleQuery_broad_singleSearch_servicePackage_downMenu_container").css("display")=="block")
      {jQuery("#singleQuery_centerPopupStyle").height(new_height)}
  ...
  else
  {
    //var new_height=(current_height+diverse_height)+"px";
    var new_height=current_height;
    if(jQuery("#singleQuery_broad_singleSearch_servicePackage_downMenu_container").css("display")=="none")
    {jQuery("#singleQuery_centerPopupStyle").height(new_height)}
    jQuery("#singleQuery_broad_singleSearch_servicePackage_downMenu_container").css("display","block");
    jQuery("#singleQuery_inputText_id_field").css("display","block");
    jQuery("#singleQuery_broad_singleSearch_id_input_noteWords").css("display","block");
    jQuery("#singleQuery_broad_singleSearch_servicePackage").text("Service Package");
  }
}
...
brodecaSingleSearch.dataPickerIssue = function() {
	...
	this.datePicker_startDate = new DatePicker({
		containerId: 'singleQuery_date-input-from',
	...
	this.datePicker_endDate = new DatePicker({
		containerId: 'singleQuery_date-input-to',
		calendarStyles: calendarStyles,
		dateInputStyles: {
			borderColor: '#d3d3d3'
		},
...
brodecaSingleSearch.setDateRange = function(day, month, year, calId){
	switch(calId){
		case "from-date-selector":
			var from_input =jQuery("#singleQuery_date-input-from");
		...
		case "to-date-selector":
			var to_input = jQuery("#singleQuery_date-input-to");
