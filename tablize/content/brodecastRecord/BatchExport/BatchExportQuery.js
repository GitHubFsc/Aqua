var BatchExportQuery = function (option) {
    var _this = this;
    _this._isEdit = option.isEdit;
    _this.isPreset = true;
    if(_this._isEdit) {
      _this._data = option.data;
    } else {
      _this._data = {
        smartCardIds: []
      };
    ...
    _this._win = new PopupDialog({
      url: 'content/brodecastRecord/BatchExport/BatchExportQuery.html',
      width: 452,
      height: 400,
      callback: BatchExportQueryBind,
      context: this,
      ajaxOnEachCreate: false
    });
    var _data = {};
    ...
    function BatchExportQueryBind() {
      ...
      _this.datePicker_startDate = new DatePicker({
        containerId: 'exportTask_start_date',
      ...
      _this.datePicker_endDate = new DatePicker({
        containerId: 'exportTask_end_date',
        calendarStyles: calendarStyles,
        dateInputStyles: {
          borderColor: '#d3d3d3'
        },
      ...
      jQuery("[name=addSmartCard]").bind("click", function () {
        ...
        if(!_exist) {
          ...
          _this._data.smartCardIds = arrayIDs;
        }
        reload_card_table();
      ...
      _this.poTypeComb.onChange = function (selector) {
          ...
          _this._data.poids = jQuery("#po_ids input").val();
        }
        //选择组合类型
      ...
      if(document.getElementById("exportTask_pageTitle_export_") != null) //在导入页面里。
      {
        //绑定导入文件的radio button
        var jQ_ = jQuery;
    ...
    function reload_card_table() {
      ...
      for(var i = 0; i < Math.max(_this._data.smartCardIds.length, 6); i++) {
        var item = _this._data.smartCardIds[i];
        ...
        } else if(i == Math.max(_this._data.smartCardIds.length, 6) - 1) {
          row.appendChild(_label);
          row.appendChild(_op);
  ...
  BatchExportQuery.prototype.create = function () {
    this._win.create()
  };
  
  BatchExportQuery.prototype.show = function () {
    this._win.show()
  }
