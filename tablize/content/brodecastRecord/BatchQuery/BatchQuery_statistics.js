...
batchQueryStatistics.init = function () {
	this.rowNum = 14;
	//绑定滚动条
	jQuery("#multiStatistics_resultTable").mCustomScrollbar({
		theme: "skyblue"
	});
...
batchQueryStatistics.createTable = function () {
	var container = jQuery("#multiStatistics_resultTable .mCSB_container");
...
batchQueryStatistics.resizePage = function () {
	...
	var topHeight = jQuery(".pageTitleContainer").outerHeight() + parseInt(jQuery("#multiStatistics_pageAllBody").css("margin-top").replace("px", "")) + jQuery(".pageAllBodyContainerTitle").outerHeight() + (jQuery(".spanDivContainer").outerHeight() === null ? 0 : jQuery(".spanDivContainer").outerHeight());
	var bottomHeight = 20;
	...
	jQuery(".multiStatistics_pageAllContainer").outerHeight(clientHeight);
	jQuery("#multiStatistics_resultTable").outerHeight(clientHeight - topHeight - bottomHeight - tableHead);
};
/*
 * 打开查询窗口
 */
batchQueryStatistics.openBatchQueryWin = function (obj) {
	// if(!this.queryWin) {
	...
	if(jQuery(obj).attr("alt") !== 'edit') {
		this.BatchQueryWin = new BatchQueryWin({
			isEdit: false,
			// data: {},
			callback: batchQueryStatistics.seriesData
		});
		this.BatchQueryWin.create();
		jQuery(".popupCenterDivTop .popupCenterDivTopLeft").html("查询条件");
	} else {
		var data = jQuery('#multiStatistics_requestPanel').data('value')
		this.BatchQueryWin = new BatchQueryWin({
			isEdit: true,
			data: JSON.parse(data),
			callback: batchQueryStatistics.seriesData
		});
		this.BatchQueryWin.show();
...
batchQueryStatistics.refreshRequestOnMainPanel = function () {
	jQuery("#multiStatistics_requestPanel").empty();
	jQuery("#multiStatistics_requestPanel").append(this.result.smart_card_ids.join("、") + "，"); //smartcard
	jQuery("#multiStatistics_requestPanel").append("(" + this.result.start_date + "--" + this.result.end_date + ")"); //daterange
	jQuery("#multiStatistics_requestPanel").append("<div alt=\"edit\" class=\"pageTitleContainerRightButt\" style=\"width:70px;height:30px;line-height:30px;margin:3px;font-size:14px;\" onclick=\"batchQueryStatistics.openBatchQueryWin(this)\">编&nbsp;辑</div>"); //editBtn
};
batchQueryStatistics.seriesData = function (putData) {
	...
	url += "&potypes=" + po_type;
	jQuery('#multiStatistics_requestPanel').data('value', JSON.stringify(putData))
...
batchQueryStatistics.loadData = function () {
	...
	var dataCount = 0;
	//清空
	jQuery("#multiStatistics_resultTable .mCSB_container .brodecastRecordTable tbody").empty();
	//加载
	for(var i = 0; i < dataSet.length; i++) {
		...
		html += "</tr>";
		dataCount++;
		jQuery("#multiStatistics_resultTable .mCSB_container .brodecastRecordTable tbody").append(html);
		// }
	};
	//不撑满则补充满行数
