...
PopupDialog_filterAndGroup.prototype.bindPopupEvent = function() {
    ...
    jQuery("#distributionAnalysis_Popup_filterAndGroup_close").bind('click', function () {
        that.close()
    });
    jQuery("#distributionAnalysis_manage_dialog_warn_foot_cancel_btn").bind('click', function () {
        that.close()
    });
    jQuery("#distributionAnalysis_manage_dialog_warn_foot_confirm_btn").bind('click', function () {
        var filterArray=[];
        var zhibiaoArray=[];
        var jq=jQuery;
        jq("#distributionAnalysis_filter_filtergroupContainer").find(".delete_div").each(function(){
        ...
        jq("#distributionAnalysis_zhibiao_filtergroupContainer").find(".delete_div").each(function(){
...
PopupDialog_filterAndGroup.prototype.setDownMenuWidth = function(array_,para) {//设置弹出窗体中最小宽度：最小108像素，最多324像素。
    ...
    array_.sort(compare);  
		...
		jq("#distributionAnalysis_filterAndGroup_menuBody_"+para+"_son").children().each(function(){
        jq(this).width(maxWidth);
    })
		jq("#distributionAnalysis_filterAndGroup_menuBody_"+para+",#distributionAnalysis_filterAndGroup_menuBody_"+para+"_son,#distributionAnalysis_filterAndGroup_menuBody_"+para+",#distributionAnalysis_filterAndGroup_menuTemp_"+para+",#distributionAnalysis_"+para+"_filtergroupContainer__son_menuContainer").width(maxWidth);
		if(para=="filter"){//指标没有一级分类的下拉菜单。
			jq("#distributionAnalysis_menuB_filterAndGroup_menuBody_" + para + "_sonMenu").width(maxWidth_menuB);
			jq("#distributionAnalysis_menuB_filterAndGroup_menuBody_" + para + "_sonMenu").children().each(function(){
...
PopupDialog_filterAndGroup.prototype.showDownMenu = function(para) {
    var that=this;
    if(document.getElementById("distributionAnalysis_filterAndGroup_menuTemp_"+para+"")!=null){
        return;
    }
    var contentArray=[];
    var gradeObjNameArray=[];
	...
	var gradeLength_=0;
    switch(para){
        case "distributionAnalysis_filter":
            ...
            menuTitle="年级";
            break;
        case "distributionAnalysis_zhibiao":
    ...
    var menuTopStyle="width:108px;border-top-left-radius:4px;border-top-right-radius:4px;display:inline-block;margin-top:10px;margin-left:14px;height:24px;border:1px #dcdcdc solid;font-size:12px;color:#848484;background-color:#ffffff;cursor:pointer";
		var diverse=3;
		var topDistance=parseInt(jQuery("#distributionAnalysis_PopupDialog_filterAndGroup_addButton_"+para+"").position().top)+parseInt(jQuery("#distributionAnalysis_PopupDialog_filterAndGroup_addButton_filter").css("marginTop"));
		var leftDistance=parseInt(jQuery("#PopupDialog_filterAndGroup_addButton_"+para+"").position().left)+parseInt(jQuery("#distributionAnalysis_PopupDialog_filterAndGroup_addButton_filter").css("marginLeft"))+108;
		var firstRowMarginLeft=14;
		if(leftDistance+132>(jq("#distributionAnalysis_"+para+"_filtergroupContainer_parent").innerWidth()+diverse)){//换行
			topDistance=topDistance+parseInt(jQuery("#distributionAnalysis_PopupDialog_filterAndGroup_addButton_"+para+"").css("marginTop"))+parseInt(jQuery("#distributionAnalysis_PopupDialog_filterAndGroup_addButton_filter").innerHeight());
			leftDistance=firstRowMarginLeft;
		}
		if(topDistance>jq("#distributionAnalysis_"+para+"_filtergroupContainer_parent").innerHeight()+3){
			topDistance=jq("#distributionAnalysis_"+para+"_filtergroupContainer_parent").innerHeight()
		}
		jq("#distributionAnalysis_"+para+"_filtergroupContainer_parent").append("<div id=distributionAnalysis_\""+para+"_filtergroupContainer__son_menuContainer\" style=\""+menuTopStyle+";position;absolute;left:"+leftDistance+"px;top:"+topDistance+"px;position:absolute;margin-left:0;margin-top:0;z-index:2\"><div id=\"distributionAnalysis_filterAndGroup_menuTemp_"+para+"\" style=\"position:relative;width:100%;height:100%\"><div id=\"distributionAnalysis_filterAndGroup_menuTemp_"+para+"_menuTitle\"style=\"width:auto;position:absolute;left:2px;right:2px;top:2px;border:1px #dcdcdc solid;height:18px;overflow:hidden\"><div style=\"float:left;width:auto;height:18px;line-height:18px;padding-left:10px;font-weight:bold\" class=\"ifLongSuplusStyle\">"+menuTitle+"</div><img src=\"images/downMenu.png\" style=\"float:right;margin-right:9px;margin-top:4px\"></div></div>");
    ...
    jq("#distributionAnalysis_filterAndGroup_menuTemp_"+para+"").append("<div id=\"distributionAnalysis_filterAndGroup_menuBody_"+para+"\" style=\""+menuBodyContainerStyleb+";max-height:300px\"></div>");
    var class_="";
    var array__=[];
    jq("#distributionAnalysis_filterAndGroup_menuBody_"+para+"").append("<div id=\"distributionAnalysis_filterAndGroup_menuBody_"+para+"_son\" style=\"height:auto;width:100%;\"></div>");
		jq("#distributionAnalysis_filterAndGroup_menuBody_"+para+"").each(function(){
    ...
    {
        if(para=="zhibiao")
        {jq("#distributionAnalysis_filterAndGroup_menuBody_"+para+"_son").append("<div id=\"distributionAnalysis_menuBodySon_"+para+"_"+a+"\" title=\""+contentArray[a]+"\" style=\"height:30px;width:200px;border-bottom:1px #dcdcdc solid;position:relative;overflow-x:hidden;background-color:#ffffff;z-index:10;\"><div class=\"each_checkbox_"+para+"\" style=\""+td_leftCheckBoxStyle+"\"><div class=\""+class_+"\" style=\""+td_leftCheckBox_sonStyle+"\"></div></div><div class=\"ifLongSuplusStyle\" style=\"text-align:left;position:absolute;left:47px;height:28px;line-height:28px;width:auto;\">"+contentArray[a]+"</div></div>")}
        else{
        ...
        var width__=47+ jq("#distributionAnalysis_menuBodySon_"+para+"_"+a+"").find(".ifLongSuplusStyle").outerWidth()+2+23+"px";
        array__.push(parseInt(width__))
    }
		this.setDownMenuWidth(array__,para);
    var h=document.getElementById("distributionAnalysis_filterAndGroup_menuBody_"+para+"_son").offsetHeight+"px";
    document.getElementById("distributionAnalysis_filterAndGroup_menuBody_"+para+"").style.height=h;
    jq("#distributionAnalysis_filterAndGroup_menuBody_"+para+"").children().each(function(){
    });
    var buttonTop=document.getElementById("distributionAnalysis_filterAndGroup_menuBody_"+para+"").offsetHeight+25+"px";
    jq("#distributionAnalysis_filterAndGroup_menuTemp_"+para+"").append("<div id=\"distributionAnalysis_filterAndGroup_button_"+para+"_container\" style=\""+menuBodyContainerStyle+"top:"+buttonTop+";height:32px;border:0;background-color:#ffffff;z-index:10;width:100%;display:none\"><div style=\"height:30px;border:0;position:relative;overflow-x:hidden;\"><div id=\"distributionAnalysis_filterAndGroup_button_"+para+"\" style=\"border-radius:3px;color:#ffffff;font-size:12px;text-align:center;position:absolute;right:0;height:20px;line-height:20px;background-color:#2cbdea;border:0;bottom:0px;width:50px;\">确&nbsp&nbsp定</div></div></div>");
    var bodyTop=parseInt(jq("#distributionAnalysis_filterAndGroup_menuBody_"+para+"").css("top"));
		var bodyH=document.getElementById("distributionAnalysis_filterAndGroup_menuBody_"+para+"").offsetHeight;
		var top_=bodyTop+bodyH+"px";
		jq("#distributionAnalysis_filterAndGroup_button_"+para+"_container").css({"top":top_,"display":"block"});
		var h__=document.getElementById("distributionAnalysis_filterAndGroup_menuTemp_"+para+"").offsetHeight+document.getElementById("distributionAnalysis_filterAndGroup_menuBody_"+para+"").offsetHeight+document.getElementById("distributionAnalysis_filterAndGroup_button_"+para+"").parentNode.offsetHeight+"px";
    jq("#distributionAnalysis_filterAndGroup_menuTemp_"+para+"").parent().height(h__);
    ...
    jQuery("#distributionAnalysis_filterAndGroup_button_"+para+"").bind('click',{para: para}, function () {
    ...
    */
    ...
    if(para=="distributionAnalysis_zhibiao"){
        jq("#distributionAnalysis_filterAndGroup_menuTemp_" + para + "").append("<div id=\"distributionAnalysis_menuB_filterAndGroup_menuBody_" + para + "_sonMenu\" style=\"" + menuBodyContainerStyleb + ";z-index:100;display:none;border-bottom:1px solid #dcdcdc;height:30px;left:2px;top:22px;width:105px\"></div>");
        jq("#distributionAnalysis_menuB_filterAndGroup_menuBody_" + para + "_sonMenu").append("<div class=\"menuB_menuBodySon_" + para + "_son\" id=\"distributionAnalysis_menuB_menuBodySon_" + para + "_0\" style=\"height:30px;line-height:30px;text-align:center;width:auto;border-bottom:1px #dcdcdc solid;position:relative;overflow-x:hidden;background-color:#ffffff;z-index:10;\">"+i18n('FENBUFENXI_ZHIBIAOC')+"</div>")
    }

    if(para=="distributionAnalysis_filter")
    {
        var filterArray_=this.filterArray;
        var width__=document.getElementById("distributionAnalysis_filterAndGroup_menuTemp_"+para+"_menuTitle").offsetWidth-2+"px";
        jq("#distributionAnalysis_filterAndGroup_menuTemp_" + para + "").append("<div id=\"distributionAnalysis_menuB_filterAndGroup_menuBody_" + para + "_sonMenu\" style=\"" + menuBodyContainerStyleb + ";z-index:100;display:none;border-bottom:1px solid #dcdcdc;height:93px;left:2px;top:22px;width:" + width__ + "\"></div>");
        for (var a = 0; a < filterArray_.length; a++) {
           jq("#distributionAnalysis_menuB_filterAndGroup_menuBody_" + para + "_sonMenu").append("<div class=\"menuB_menuBodySon_" + para + "_son\" id=\"distributionAnalysis_menuB_menuBodySon_" + para + "_" + a + "\" style=\"height:30px;line-height:30px;text-align:center;width:auto;border-bottom:1px #dcdcdc solid;position:relative;overflow-x:hidden;background-color:#ffffff;z-index:10;\">"+filterArray_[a] +"</div>")
        }
        document.getElementById("distributionAnalysis_filterAndGroup_menuTemp_"+para+"_menuTitle").onclick=function(){
            if(jq("#distributionAnalysis_menuB_filterAndGroup_menuBody_" + para + "_sonMenu").css("display")=="none"){
                jq("#distributionAnalysis_menuB_filterAndGroup_menuBody_" + para + "_sonMenu").css("display","block");
            }
            else{
                jq("#distributionAnalysis_menuB_filterAndGroup_menuBody_" + para + "_sonMenu").css("display","none");
            }
            var w=document.getElementById("distributionAnalysis_menuB_filterAndGroup_menuBody_" + para + "_sonMenu").offsetWidth+"px";
        }
        jq(".menuB_menuBodySon_" + para + "_son").each(function(){

            jq(this).bind('click', function () {
                jq("#distributionAnalysis_menuB_filterAndGroup_menuBody_" + para + "_sonMenu").css("display","none");
                jq("#distributionAnalysis_filterAndGroup_menuBody_"+para+"_son").empty();
                jq("#distributionAnalysis_filterAndGroup_button_"+para+"_container").remove();
                var contentArray=[];
                var gradeObjNameArray=[];
                var text__=jq(this).text();
                jq("#distributionAnalysis_filterAndGroup_menuTemp_"+para+"_menuTitle").find(".ifLongSuplusStyle").text(text__);
                switch(jq(this).text())
                {
                    case filterArray_[0]:
                ...
                for(var a=0;a<contentArray.length;a++)
                {
                    jq("#distributionAnalysis_filterAndGroup_menuBody_"+para+"_son").append("<div title=\""+contentArray[a]+"\" id=\"distributionAnalysis_menuBodySon_"+para+"_"+a+"\" style=\"height:30px;width:auto;border-bottom:1px #dcdcdc solid;position:relative;overflow-x:hidden;background-color:#ffffff;z-index:10;width:200px;\"><div class=\"each_checkbox_"+para+"\" style=\"border-radius:2px;position:absolute;height:16px;width:16px;border:1px #d6d6d6 solid;left:16px;top:6px\"><div class=\""+class_+"\" style=\"border-radius:1px;margin-left:3px;margin-top:3px;height:10px;width:10px\"></div></div><div name=\""+gradeObjNameArray[a]+"\"  class=\"ifLongSuplusStyle\" style=\"text-align:left;position:absolute;left:47px;height:28px;line-height:28px;width:auto;\">"+contentArray[a]+"</div></div>")
                    var width__=47+ jq("#distributionAnalysis_menuBodySon_"+para+"_"+a+"").find(".ifLongSuplusStyle").outerWidth()+23+"px";
                    array__.push(parseInt(width__))
                }
                ...
                jq("#distributionAnalysis_filterAndGroup_menuTemp_"+para+"").append("<div id=\"distributionAnalysis_filterAndGroup_button_"+para+"_container\" style=\""+menuBodyContainerStyle+"top:"+buttonTop+";height:32px;border:0;background-color:#ffffff;z-index:10;width:100%;display:none\"><div style=\"height:30px;border:0;position:relative;overflow-x:hidden;\"><div id=\"distributionAnalysis_filterAndGroup_button_"+para+"\" style=\"border-radius:3px;color:#ffffff;font-size:12px;text-align:center;position:absolute;right:0;height:20px;line-height:20px;background-color:#2cbdea;border:0;bottom:0px;width:50px;\">确&nbsp&nbsp定</div></div></div>");
								...
								var top_=bodyTop+bodyH+"px";
								jq("#distributionAnalysis_filterAndGroup_button_"+para+"_container").css({"top":top_,"display":"block"});
								var h__=document.getElementById("distributionAnalysis_filterAndGroup_menuTemp_"+para+"_menuTitle").offsetHeight+document.getElementById("distributionAnalysis_filterAndGroup_menuBody_"+para+"").offsetHeight+document.getElementById("distributionAnalysis_filterAndGroup_button_"+para+"").parentNode.offsetHeight+"px";
                jQuery("#distributionAnalysis_filterAndGroup_button_"+para+"").bind('click',{para: para}, function () {
                    that.addConditions(para)
                })
                jq("#distributionAnalysis_filterAndGroup_menuTemp_"+para+"").parent().height(h__);
...
PopupDialog_filterAndGroup.prototype.drawContainer = function(para) {
    ...
    jq("#distributionAnalysis_"+para+"_filtergroupContainer").empty();

    for(var a=0;a<data_.length;a++){}
    jq("#distributionAnalysis_"+para+"_filtergroupContainer").append("<div style=\"height:auto;min-height:"+minH+";width:418px\" id=distributionAnalysis_\""+para+"_filtergroupContainer__son\"><div id=\"distributionAnalysis_PopupDialog_filterAndGroup_addButton_"+para+"\" style=\"vertical-align:top;display:inline-block;;margin-left:14px;margin-top:10px;width:94px;height:26px;position:relative;cursor:pointer\"><img src=\"images/addfilter.png\"  style=\"position:absolute;top:0;left:0;width:94px;height:26px;\"/><div style=\"color:#82daf6;height:26px;line-height:26px;position:absolute;left:31px;top:0px;font-weight:bold;font-size:12px\">"+addButtonText+"</div></div>")
    jq("#distributionAnalysis_PopupDialog_filterAndGroup_addButton_"+para+"").bind('click', {para: para},function() {
        if(para=="distributionAnalysis_filter"){  that.filterType="grade";}
        that.showDownMenu(para);
        });

    jQuery("#distributionAnalysis_"+para+"_filtergroupContainer").mCustomScrollbar({
        axis: 'yx'
    });
}
...
PopupDialog_filterAndGroup.prototype.addConditions = function(para) {
    ...
    jq("#distributionAnalysis_"+para+"_filtergroupContainer__son").find(".delete_words").each(function(){
        ...
        jq(this).bind('click',function() {
              ...
              var leftDistance=parseInt(tempElement.style.left);
							...
							if(leftDistance-firstRowMarginLeft-delete_div_width-diverse<0){//返回上一行
								var beforeButton=jQuery("#distributionAnalysis_PopupDialog_filterAndGroup_addButton_"+para+"");
