var addAdmin={
    ...
    jQuery.ajax({
      ...
      headers: {
        ...
        "x-aqua-sign":getPaaS_x_aqua_sign(type_,url_)
          ...
          document.getElementById("appAdmin_addAdmin_appSecret_son").value=newKey;
        }).fail(function () {
          alert("重置失败")
        })
  },
  initPopupCommon:function(){
    ...
    if(this.enterAppMethod=="notLogin")
    {
      ...
      {
        jQuery("#appAdmin_popup_app_tableRegion").append("<tr><td></td><td></td><td></td><td></td></tr>")
      }
      ...
      headers: {
        ...
        "x-aqua-sign":getPaaS_x_aqua_sign(type_,url_)
          ...
          for(var a=0;a<data.length;a++)
          {
            ...
            jQuery("#appAdmin_popup_app_tableRegion").append("<tr><td><div class=\"squareChoosed_outer\" onclick=\"addAdmin.switch_(this)\"><div style=\"display:none\" class=\"squareChoosed_inner\"></div></div></td><td>"+id_+"</td><td>"+name_+"</td><td>"+description_+"</td></tr>")
          }
  ...
  initPopup:function(){
    ...
    {
      case "create":
					this.__validComb = new newSelect("#appAdmin_app_admin_token_check_container", [{
        ...
        break;
      case "edit":
				jq("#appAdmin_addAdmin_appSecret").css("width","calc(100% - 48px)");
        this.initPopupCommon();
        this.initPopupEdit();
  ...
  initPopupOnlyView:function(){
	  var jq=jQuery;
		jq("#appAdmin_addAdmin_appSecret").parent().css("display","block");
		//jq("#app_admin_token_check_container").append("<div style=\"width:100%;height:100%;position:relative;\"></div>");
		jq("#appAdmin_app_admin_token_check_container").css("background-color","#ebebeb");
		jq("#appAdmin_app_admin_token_check_container").find(".select-status").css("background-color","#ebebeb");
		jq("#appAdmin_app_admin_token_check_container").find(".select-header").css("background-color","#ebebeb");
    ...
    jQuery("#appAdmin_addAdmin_resetSecret").css("display","none");
		document.getElementById("appAdmin_lectureCreatePage_info_course_description_value").readOnly=true;
		jq("#appAdmin_app_admin_token_check_container").find(".select-header").css("cursor","default")   
  },
  initPopupEdit:function(){
    ...
    document.getElementById("appAdmin_lectureCreatePage_info_course_description_value").value=des_;
	...
	if( ( typeof(hasToken)=="undefined")||(hasToken=="false") ){
	  ...
	  jq("#appAdmin_app_admin_token_check_container").find(".select-header").text(hasToken_);
    document.getElementById("appAdmin_addAdmin_appSecret").style.display="inline-block";
    document.getElementById("appAdmin_addAdmin_resetSecret").style.display="inline-block";
		var currentRawDate=this.rawData[this.rowIndex];//here
		var secret_=currentRawDate.app_secret||"";
    document.getElementById("appAdmin_addAdmin_appSecret_son").value=secret_;//here
    if(this.action_=="edit")
    {document.getElementById("appAdmin_popupCenterDivTopLeft").innerHTML="编辑应用";}
    else{document.getElementById("appAdmin_popupCenterDivTopLeft").innerHTML="查看应用";};
    document.getElementById("appAdmin_appKey").readOnly=true;
    document.getElementById("appAdmin_addAdmin_appSecret_son").readOnly=true;
		jq("#appAdmin_appKey").removeClass("app_admin_input_white").addClass("app_admin_input_grey");
		jq("#appAdmin_addAdmin_appSecret_son").removeClass("app_admin_input_white").addClass("app_admin_input_grey");
    jQuery("#appAdmin_popup_app_tableRegion").find("tr").each(function(){
  ...
  commitApp:function(key_){
    ...
    var appName=document.getElementById("appAdmin_appName").value;
		var tmp_=jq("#appAdmin_app_admin_token_check_container").find(".select-header").text();
    var tokenCheck=( tmp_==i18n("APP_ADMIN_NEED_TOKEN_FALSE")?true:false);
    var desc_=document.getElementById("appAdmin_lectureCreatePage_info_course_description_value").value;
    var scopeIdList=[];
    jQuery(".squareChoosed_inner").each(function(){
      ...
      data: objStr
          ...
          if(key_=="edit")
          {
            ...
            jQuery(obj_).closest("tr").children("td").eq(3).find(".desc").text(desc_);
						var secret=document.getElementById("appAdmin_addAdmin_appSecret_son").value;
  ...
  queryDataSearch:function(){
    ...
    var keyWord_=document.getElementById("appAdmin_search_input_").value;
    var type_="GET";
  ...
  switch_:function(obj){
    ...
    if(ele_=="none")
    {
      ...
      jQuery(obj).find(".squareChoosed_inner").attr("id","appAdmin_chosedIcon_scope")
    }
    else{
  ...
  updateListTable: function (restorePage) {
    var that=addAdmin;
    this._listTable = new AppAdmin_StyledList({
      containerId: "appAdmin_mainTableContainer",
      rows: this._listRows,
  ...
  bindEvent:function(){
    var that=addAdmin;
    document.getElementById('appAdmin_ErrorMSG_close_cancelOrder').onclick = function () {
      that.opendialog('appAdmin_ErrorMSG_cancelOrder', false);
    };
    document.getElementById('appAdmin_ErrorMSG_cancel_cancelOrder').onclick = function () {
      that.opendialog('appAdmin_ErrorMSG_cancelOrder', false);
    }
  },
  ...
  popupDelete:function(obj){
    ...
    var obj = {
      ...
      callback: function () {
        ...
        document.getElementById('appAdmin_ErrorMes_cancelOrder').innerHTML = text_;
      }
    }
  ...
  openErrorMsgB: function (obj, callback) {
    ...
    if (obj.event) {
      document.getElementById('appAdmin_ErrorMSG_ok_cancelOrder').onclick = function () {
        obj.event();
        self.opendialog('appAdmin_ErrorMSG_cancelOrder', false);
      }
    }
    if (typeof obj.callback == 'function') {
      obj.callback();
    }
    this.opendialog('appAdmin_ErrorMSG_cancelOrder', true, callback);
  },
  ...
  mopen: function (object, id, isFather) {
    if( (id=="appAdmin_downMenu")&&(this.action_=="onlyView") ){return}
