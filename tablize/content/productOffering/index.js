var productOffering = {
  ...
  initWidgets: function () {
    ...
    _this.__listTable = new StyledList({
      containerId: "productOffering_list_table",
      rows: 15,
      columns: 8,
      titles: _list_titles,
      listType: 0,
  ...
  alertDialog: function (context, callback) {
    var dialog = new PopupDialog({
      ...
      callback: function () {
        //初始化文字
        jQuery(".productOffering_dialog .body").html(context);
        //确定按钮事件绑定
        jQuery(".productOffering_dialog .foot .btn.confirm").bind('click', function () {
            callback(function () {
              dialog.close();
            })
          })
          //取消按钮事件绑定
        jQuery(".productOffering_dialog .foot .btn.cancel").bind('click', function () {
          dialog.close();
        })
      }
    })
