var UserTag = (function($){
  ...
  var UserTagList = {
    init: function(){
      ...
      self.listCnt = $('#userTag-admin-index')[0];
    },
    handleEvents: function(){
      var self = this;
      $('#userTag-admin-list').on('click', '.opr-default', function(){
        var opr = $(this);
      ...
      $('#userTag-admin-list-search').on('keydown', function(e){
        if(e.keyCode == 13){
          //self.listSearch();
        }
      ...
      $('#userTag-admin-add').on('click', function(){
        self.openUserGroupDialog('add', {});
      });
    },
    handleDropdowns: function(target, delegate){
      var self = this;
      if(self.listCnt != $('#userTag-admin-index')[0]){
    ...
    setList: function(){
      ...
      self.list = new StyledList({
        ...
        rows: 15,
        columns: 6,
        containerId: 'userTag-admin-list',
        listType: 1,
        titles: [
    ...
    openUserGroupDialog: function(uiType, user){
      ...
      var dialog = new PopupDialog({
        ...
        context: {},
        callback: function(){
          $("#userTag-admin-user-dialog-title").text(i18n("USERTAG_ADDUSERTAG"));
          $("#userTag-admin-dialog-submit").html(i18n("USERGROUP_QUEDING"));
          $("#userTag-admin-user-name").focus();
          $("#userTag-admin-dialog-submit").click(function(){
            var name = $("#userTag-admin-user-name").val();
