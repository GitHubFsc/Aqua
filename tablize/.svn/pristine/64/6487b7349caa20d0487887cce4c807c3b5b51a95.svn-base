...
fenbu_fenxi.initPara = function(){
  ...
  this.measure_="";
	this.checkBoxOrRadio={"distributionAnalysis_display_container_fenzu":"radioButton"};//分组默认单选
...
fenbu_fenxi.refreshGroupDisplay_noGroup=function(){
	var jq=jQuery;
	var self=this;
	self.choosedQuotasInQuotasPopup["distributionAnalysis_display_container_fenzu"]=[];//指标联动的默认分组数据为空。
	self.choosedQuotasInPage["distributionAnalysis_display_container_fenzu"]=[];//过去选择好提交的分组清空。
...
fenbu_fenxi.refreshGroupDisplay=function(index){
	...
	self.choosedQuotasInQuotasPopup["distributionAnalysis_display_container_fenzu"]=fenzuArray;
	self.refreshfenzuCommon();
...
fenbu_fenxi.refreshfenzuCommon=function(){
  var self=this;
	var obj={
	"clickType":self.checkBoxOrRadio["distributionAnalysis_display_container_fenzu"],
	"containerId":"distributionAnalysis_display_container_fenzu",
	...
	var height=document.getElementById("distributionAnalysis_display_container_fenzu").offsetHeight+"px";
	document.getElementById("distributionAnalysis_distribution_fenzu_container").style.height=height
}
...
fenbu_fenxi.showFenzuDownload=function(formatFenzuData){
  ...
  var jq=jQuery;
	...
	jq("#distributionAnalysis_distribution_fenzuMenu_body").empty();
	document.getElementById("distributionAnalysis_distribution_fenzuMenu_title").innerHTML=i18n("FENBUFENXI_BUFENZU");//默认不分组
	jq("#distributionAnalysis_distribution_fenzuMenu_body").append("<div class=\"distribution_downMenu_row\" title=\""+i18n("FENBUFENXI_BUFENZU")+"\" onmouseover=\"javascript:this.style.color='#29bff4'\" onmouseout=\"javascript:this.style.color='#888888'\">"+i18n("FENBUFENXI_BUFENZU")+"</div>");
	document.getElementById("distributionAnalysis_distribution_fenzuMenu").style.display="block";
	if(formatFenzuData.length==0){
		self.no_group=true;
		document.getElementById("distributionAnalysis_display_container_fenzu").style.display="none";
	}
	else{
		self.no_group=false;
		...
		for(var i=0;i<groupCount;i++){
		  var eachFormatFenzuData=formatFenzuData[i];
			...
			jq("#distributionAnalysis_distribution_fenzuMenu_body").append("<div style=\"display:"+menuBodyDisplay+";max-width:calc(100% - 12px);overflow:hidden;white-space:nowrap;text-overflow:ellipsis\" class=\"distribution_downMenu_row\" title=\""+menuRowDisplayName+"\" onmouseover=\"javascript:this.style.color='#29bff4'\" onmouseout=\"javascript:this.style.color='#888888'\">"+menuRowDisplayName+"</div>");
		};
		document.getElementById("distributionAnalysis_display_container_fenzu").style.display="block";
	}
	...
	jq(".distribution_downMenu_row").each(function(){
		jq(this).unbind().bind('click', function () {
				...
				self.hide('distributionAnalysis_distribution_fenzuMenu_body',this);
				jq("#distributionAnalysis_distribution_fenzuMenu_top").find("img").css("transform","rotate(0deg)");
    });
  });
};
...
fenbu_fenxi.hide=function(id,obj){
	...
	document.getElementById("distributionAnalysis_distribution_fenzuMenu_title").innerHTML=jQuery(obj).text()
};
...
fenbu_fenxi.expandSonContainer=function(){
  var jq=jQuery;
	//   根据tabs_header计算select的top高度
	var containerTop=this.offSet(document.getElementById("tabs_header")).top+148+"px";
	jq("#distributionAnalysis_distribution_fenzuMenu_body").css({"top":containerTop,"left":"79px"})
	var border=2;
	var height=document.getElementById("distributionAnalysis_distribution_fenzuMenu_top").offsetHeight;
	var top=document.getElementById("distributionAnalysis_distribution_fenzuMenu").offsetTop;
	var bodyHeight=document.getElementById("distributionAnalysis_distribution_fenzuMenu_body").offsetHeight;
  ...
  var self=this;
	self.getAllQuotas(function(data){
		...
		self._actionChecker.onSelect = function() {
			...
			self.choosedQuotasInQuotasPopup["distributionAnalysis_display_container_zhibiao"]=array_;
			self.choosedQuotasInPage["distributionAnalysis_display_container_zhibiao"]=array_;//指标弹框选择后，在外部界面默认全选。
			var obj={
				"clickType":"checkBox",
				"containerId":"distributionAnalysis_display_container_zhibiao",
				"data":self.choosedQuotasInQuotasPopup,
				"isChoosed":"choosed",
...
fenbu_fenxi.initGroupByChoosedQuotas=function(selected){//指标更新时，分组更新
  var jq=jQuery;
	...
	if(selected.length==0){//没选任何指标时，下面下拉菜单不显示。
		jq("#distributionAnalysis_display_container_fenzu").empty().hide();
		self.groupStatus="notChooseQuotas_noGroup";
	  //不管有没有分组，下拉菜单头都要显示，保留箭头，默认不分组。
	  document.getElementById("distributionAnalysis_distribution_fenzuMenu_title").innerHTML=i18n("FENBUFENXI_BUFENZU");//默认不分组
	  jq("#distributionAnalysis_distribution_fenzuMenu_body").empty().append("<div class=\"distribution_downMenu_row\" onmouseover=\"javascript:this.style.color='#29bff4'\" onmouseout=\"javascript:this.style.color='#888888'\">"+i18n("FENBUFENXI_BUFENZU")+"</div>");
	  document.getElementById("distributionAnalysis_distribution_fenzuMenu").style.display="block";
		self.refreshGroupDisplay_noGroup();
		this.bindEvents_fenzu();//暂且定下不管分不分组都能点菜单展开
	  jq(".distribution_downMenu_row").each(function(){
...
fenbu_fenxi.clearFilter=function(){
	var jq=jQuery;
	var self=this;
	jq("#distributionAnalysis_filter_display_container").empty();
	for(var a in self.choosedQuotasInQuotasPopup){
	 if( (a.indexOf("distributionAnalysis_filter_display_container")!=-1)&&(a.indexOf("_son")!=-1) ){
		 delete(self.choosedQuotasInQuotasPopup[a]);
	 }
	}
	for(var b in self.choosedQuotasInPage){
	 if( (b.indexOf("distributionAnalysis_filter_display_container")!=-1)&&(b.indexOf("_son")!=-1) ){
  ...
  var jq=jQuery;
	...
	for(var i=0;i<data_len;i++){
	  ...
	  var title=data[i]["categary"].name+i18n("FENBUFENXI_GUOLVB");
		jq("#distributionAnalysis_filter_display_container").append("<div id=\"distributionAnalysis_filter_display_container"+i+"\" style=\"width:819px;display:"+containerDisplay+"\"><div class=\"hiddenCategary\" style=\"display:none\">"+data[i]["categary"].id+"</div><div title=\""+title+"\" style=\"font-size:14px;width:auto;width:auto;display:inline-block;margin-top:2px;white-space:nowrap;text-overflow:ellipsis;overflow:hidden;vertical-align:top;\">"+title+"</div><div id=\"distributionAnalysis_filter_display_container"+i+"_son\" style=\"display:inline-block;height:auto;width:697px;overflow:hidden;\"></div>");
		...
		this.choosedQuotasInQuotasPopup["distributionAnalysis_filter_display_container"+i+"_son"]=array_;
	  this.choosedQuotasInPage["distributionAnalysis_filter_display_container"+i+"_son"]=array_;//指标弹框选择后，在外部界面默认全选。
		var obj={
			"clickType":"checkBox",
			"containerId":"distributionAnalysis_filter_display_container"+i+"_son",
...
fenbu_fenxi.bindEvents=function(){
  var jq=jQuery;
	...
	document.getElementById("distributionAnalysis_fenzuSwitchButton").onclick=function(){
	  var checkBoxOrRadio=(self.checkBoxOrRadio.distributionAnalysis_display_container_fenzu=="radioButton")?"checkBox":"radioButton";
		this.checkBoxOrRadio={"distributionAnalysis_display_container_fenzu":checkBoxOrRadio}
		self.checkBoxOrRadio.distributionAnalysis_display_container_fenzu=checkBoxOrRadio;
		...
		self.choosedQuotasInPage["distributionAnalysis_display_container_fenzu"]=[];//分组切换多选单选后，默认一个都不选择。
		var obj={
			"clickType":self.checkBoxOrRadio["distributionAnalysis_display_container_fenzu"],
			"containerId":"distributionAnalysis_display_container_fenzu",
			"data":self.choosedQuotasInQuotasPopup,
...
fenbu_fenxi.bindEvents_fenzu=function(){
  var self=this;
  var jq=jQuery;
	jQuery("#distributionAnalysis_distribution_fenzuMenu_top").unbind().bind('click',function(){
		if(jq("#distributionAnalysis_distribution_fenzuMenu_body").css("display")=="none"){
		  self.expandSonContainer();
			jq("#distributionAnalysis_distribution_fenzuMenu_body").css("display","block");
			jq(this).find("img").css("transform","rotate(180deg)");
		}
	  else{
			jq("#distributionAnalysis_distribution_fenzuMenu_body").css("display","none");
			jq(this).find("img").css("transform","rotate(0deg)");
		}
	})
	jq("#distributionAnalysis_distribution_fenzuMenu_top").mouseout(function(){
	  
		//self.fenzuMenuTimeout=setTimeout(function(){jq("#distribution_fenzuMenu_body").css("display","none")},2000)
	})
	jq("#distributionAnalysis_distribution_fenzuMenu_body").mouseover(function(){
		clearTimeout(self.fenzuMenuTimeout);
		self.fenzuMenuTimeout=null;
	})
	jq("#distributionAnalysis_distribution_fenzuMenu_body").mouseout(function(){
...
fenbu_fenxi.drawFilterDisplayRegion=function(obj){
  ...
  var self=this;
	...
	for(var i=0;i<len;i++){
		...
		if(obj.containerId=="distributionAnalysis_display_container_fenzu"){
				...
				jq(this).bind('click', function () {
					...
					if(clickType_=="radioButton"){
						...
						if(jq(this).children().attr("class")=="d4d4d4"){
							...
							if(container!="distributionAnalysis_display_container_fenzu"){  //here
						...
						if(container=="distributionAnalysis_display_container_zhibiao")
							{
							 let tempArr=[];
					...
					else if(obj.containerId=="distributionAnalysis_display_container_zhibiao"){
						let array_=[];
...
fenbu_fenxi.drawDefaultTable=function(){
	...
	var zhibiao_length=jQ("#distributionAnalysis_display_container_zhibiao").find(".ifLongSuplusStyle").length;
	for(var a=0;a<zhibiao_length;a++){
			var lr="";
			var name_=jQ("#distributionAnalysis_display_container_zhibiao").find(".ifLongSuplusStyle").eq(a).text();
			if(a%2==0){lr="left"}
			else{lr="right"}
			jQ("#distributionAnalysis_searchResultContainer").append("<div style=\"width:calc(50% - 24px);height:271px;float:"+lr+";border:1px #eaeaea solid;overflow:hidden;margin-top:39px;\"><div style=\"float:left;height:100%;width:208px;\"><div style=\"height:23px;width:100%;overflow:hidden;margin-left:15px;margin-top:13px\"><div style=\"font-size:12px;float:left;width:auto;height:24px;line-height:24px\">"+name_+"</div><div style=\"font-size:6px;float:left;width:26px;;height:11px;text-align:center;line-height:11px;border-radius:2px;background-color:#fd807f;color:#fff9f9;font-family:Arial;margin-left:6px;font-weight:bold\">index</div></div><div style=\"height:calc(100% - 39px);width:100%;position:relative;\"><div id=\"distributionAnalysis_echartt_"+a+"\" style=\"position:absolute;height:220px;width:100%;top:0px\"></div></div></div><div style=\"float:right;height:100%;width:calc(100% - 220px);margin-right:12px;color:#848484;\"><div id=\"distributionAnalysis_analyze_chartContainer_"+a+"\" style=\"margin-top:27px;height:232px;\" class=\"chartEachTableContainer\"></div></div>");
			var thA=i18n("FENBUFENXI_TULI");
			var thB=i18n("FENBUFENXI_FENZU");
			var thC=i18n("FENBUFENXI_ZHANBI");
			var tableWidth = (window.innerWidth / 2) -341 +'px';
			jQ("#distributionAnalysis_analyze_chartContainer_"+a+"").append("<table id=\"distributionAnalysis_analyze_chartContainer_"+a+"_table\" border=\"1\" cellpadding=\"0\" cellspacing=\"0\" style=\"table-layout:fixed;min-width:"+tableWidth+";border:1px solid #eaeaea;border-right:0;border-bottom:0;border-radius:4px\"><tr style=\"font-size:14px\"><th class=\"fenbufenxi_td\"  style=\"height:38px\"  width=\"19%\">"+thA+"</th><th class=\"fenbufenxi_td\"  style=\"height:38px\"  width=\"54%\">"+thB+"</th><th class=\"fenbufenxi_td\"  style=\"height:38px\"  width=\"26%\">"+thC+"</th></tr></table>")
			for(var b=0;b<defaultTableLength;b++){
					jQ("#distributionAnalysis_analyze_chartContainer_"+a+"_table").append("<tr><td class=\"fenbufenxi_td\"></td><td class=\"fenbufenxi_td\"></td><td class=\"fenbufenxi_td\"></td></tr>")
			}
	}
}
...
fenbu_fenxi.searchData=function(){
	var jq=jQuery;
	var that=this;
	jq("#distributionAnalysis_display_container_fenzu").find(".bg32beea").each(function(){
  ...
  jq("#distributionAnalysis_searchButton").unbind();
	...
	if(typeof choosedQuotasInPage["distributionAnalysis_display_container_zhibiao"]!="undefined"){
		measure_para = (choosedQuotasInPage["distributionAnalysis_display_container_zhibiao"].map(function (item) {
			return item.id;
		})).join(",");
	};
	if(typeof choosedQuotasInPage["distributionAnalysis_display_container_fenzu"]!="undefined"){
		fenzuPara_original = (choosedQuotasInPage["distributionAnalysis_display_container_fenzu"].map(function (item) {
			return item.id;
		})).join(",");
	};
	var fenzuPara_array=fenzuPara_original.split(":");
  var fenzuPara=fenzuPara_array[0]
	var l1_len=jq("#distributionAnalysis_filter_display_container").children().length;
	var array_condition=[];
	for(var i=0;i<l1_len;i++){
	  var array_condition_l2=[];
		var title=jq("#distributionAnalysis_filter_display_container").find(".hiddenCategary").eq(i).text();
		var l2_len=jq("#distributionAnalysis_filter_display_container"+i+"_son").children().length; 
		for(var j=0;j<l2_len;j++){
		  var l2_name=jq("#distributionAnalysis_filter_display_container"+i+"_son").find(".ifLongSuplusStyle").eq(j).text();
			if(jq("#distributionAnalysis_filter_display_container"+i+"_son").find(".each_checkbox_filter_display").eq(j).children().attr("class")=="bg32beea"){
	...
	var beginStr="";
	var endStr="";
	if(document.getElementById("distributionAnalysis_date_picker_begin-datepicker-input").value!="")
	{
			beginStr=document.getElementById("distributionAnalysis_date_picker_begin-datepicker-input").value+"T00:00:00"+encodeURIComponent("+")+"0800";
	}
	if(document.getElementById("distributionAnalysis_date_picker_end-datepicker-input").value!="")
	{
			endStr=document.getElementById("distributionAnalysis_date_picker_end-datepicker-input").value+"T00:00:00"+encodeURIComponent("+")+"0800";
	}
	var time_unit="day";
	...
	jQuery.ajax({
			...
			jQuery("#distributionAnalysis_searchResultContainer").empty();
...
fenbu_fenxi.bubbleSort=function(arr,containerid){
  var self=this;
	var jq=jQuery;
	var len = arr.length;
	var display_container_zhibiao=self.choosedQuotasInPage["distributionAnalysis_display_container_zhibiao"];
　for (var i = 0; i < len; i++){
...
fenbu_fenxi.drawSeachrResults=function(data_old,measure,fenzu){
  ...
  var jq=jQuery;
	var self=this;
	if(typeof this.choosedQuotasInPage["distributionAnalysis_display_container_zhibiao"]!="undefined"){//选择了指标
		for(var i= 0,len_quotaNumber = self.choosedQuotasInPage["distributionAnalysis_display_container_zhibiao"].length;i<len_quotaNumber;i++){
			var display_container_zhibiao=self.choosedQuotasInPage["distributionAnalysis_display_container_zhibiao"];
			...
			jq("#distributionAnalysis_searchResultContainer").append("<div style=\"float:"+lr+";"+l1_style+"\"><div style=\""+l2_a_style+"\"><div style=\""+l2_a_l3_style+"\"><div style=\""+l2_a_l3_l4_a_style+"\">"+display_container_zhibiao[i].name+"</div>"+l2_a_l3_l4_b_ele+"<div style=\"height:calc(100% - 39px);width:100%;position:relative;\"><div id=\"distributionAnalysis_echartt_"+i+"\" style=\"position:absolute;height:220px;width:calc(100% + 220px);top:0px;z-index:5\"></div></div></div><div style=\""+l2_b_style+"\"><div id=\"distributionAnalysis_analyze_chartContainer_"+i+"\" style=\"margin-top:27px;height:232px;\" class=\"chartEachTableContainer\"></div></div></div>");
			...
			var tableWidth = (window.innerWidth / 2) -341 +'px';
			jq("#distributionAnalysis_analyze_chartContainer_"+i+"").append("<table id=\"distributionAnalysis_analyze_chartContainer_"+i+"_table\" border=\"1\" cellpadding=\"0\" cellspacing=\"0\" style=\"table-layout:fixed;min-width:"+tableWidth+";border:1px solid #eaeaea;border-right:0;border-bottom:0;border-radius:4px\"><tr id=\"distributionAnalysis_analyze_chartContainer_"+i+"_table_th\" style=\"font-size:14px\"></tr></table>");
			for(var j= 0,th_length = thArray.length;j<th_length;j++){
				jq("#distributionAnalysis_analyze_chartContainer_"+i+"_table_th").append("<th class=\"fenbufenxi_td\" style=\"height:38px\"  width=\""+thWidthArray[j]+"\">"+thArray[j]+"</th>")
			} 
			...
			while (colorArray.length<groupCount)
      {
       colorArray=colorArray.concat(colorArray_concat)
			...
			for(var j= beginColorIndex;j<groupCount;j++){
				...
				jq("#distributionAnalysis_analyze_chartContainer_"+i+"_table").append("<tr>"+td1_html+""+td2_html+""+td3_html+"</tr>");
			...
			if(surplusCount>0){
			  for(var a= 0;a<surplusCount;a++){
					jq("#distributionAnalysis_analyze_chartContainer_"+i+"_table").append("<tr><td class=\"fenbufenxi_td\"></td><td class=\"fenbufenxi_td\"></td><td class=\"fenbufenxi_td\"></td></tr>");
				}
			}
			jq("#distributionAnalysis_analyze_chartContainer_"+i+"").mCustomScrollbar({
        axis: 'y'
			});
		}
	}
...
fenbu_fenxi.drawPie=function(index_,pieData,pieColorArray,title){
	...
	//初始化echarts实例
	var myChart = echarts.init(document.getElementById('distributionAnalysis_echartt_'+index_+''));
...
fenbu_fenxi.fillFilterDisplay=function(){
    ...
    jQuery("#distributionAnalysis_filter_display_container").empty();
    jQuery("#distributionAnalysis_display_container_zhibiao").empty();
}
...
fenbu_fenxi.initCalendar=function(){
    var datePicker_start = new DatePicker({
        containerId: "distributionAnalysis_date_picker_begin",
        editable: true,
        dateInputStyles: {
            borderColor: "#eaeef0",
            width: "142px"
     ...
     var datePicker_end = new DatePicker({
        containerId: "distributionAnalysis_date_picker_end",
        editable: true,
        dateInputStyles: {
            borderColor: "#eaeef0",
            width: "142px"
  ...
  var jq=jQuery;
	...
	if(this.groupStatus=="chooseQuotas_hasGroup"){
		...
		jq("#distributionAnalysis_filterButton").unbind().bind('click',function(){
		 if(typeof self.choosedQuotasInPage["distributionAnalysis_display_container_zhibiao"]=="undefined"){
			trendAnalysis_Lib.openMessageDialog(i18n("TRENDANALYSIS_DIALOG_CONTENT2"));
      return;
		 }
