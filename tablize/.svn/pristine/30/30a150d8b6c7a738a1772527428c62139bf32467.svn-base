var setTagByTag = (($, Table, Dialog) => {
	...
	var	returnToObj={tagsNameArray:[]};
	var	everyPageDatas={};
	var	productsByTagSearch=[];
		...
		function bindEvents(){
		  var that=this;
			 ...
			 $("#setAssetTagByTag_batchSetTag").bind("click", (e) => {
				...
				if(returnToObj.tagsNameArray.length>0){
				  var originalTagName=returnToObj.tagsNameArray.join(",")
					var objType_addTagToAsset="asset";
					var url = aquapaas_host +"/aquapaas/rest"+ "/tag/application/"+appKey_onlyUsedForSetTagForProduct+"/" + objType_addTagToAsset+ "?tags="+originalTagName+"&" + token + new Date().toISOString();
					jQuery.ajax({
						type: "GET",
						async: false,
						url: url,
				...
				$.ajax({
						...
						if(data){
						  var dataLen=data.length;
							...
							var itemsCountOneRow=itemsCountOneRow;//一行显示几条数据
	...
	initTable("all");
	bindEvents();
})(jQuery, SetTagByTagStyledList, PopupDialog)
