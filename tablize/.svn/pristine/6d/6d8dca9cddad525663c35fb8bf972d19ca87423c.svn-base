(function ($, Table) {
  let Pane = {
    ...
    initWidgets() {
      $("#autho_single ._frame_foot ._btn._download").bind("click", () => {
        let el = $("#autho_single #authoSingle_results > div");
        let data = {};
        let srcdata = el.data();
      ...
      $("#autho_single ._btn._query").bind("click", () => {
        authoSingle_dialog.init({}, (data) => {
          var dom = $("<div></div>");
          ...
          $("#autho_single #authoSingle_results").empty().append(dom);
          $("#autho_single ._btn._edit").show();
          API.getList(data, (resp) => {
            Pane.table.update(Models.table(resp))
          });
        });
      });
      $("#autho_single ._query_condition ._btn._edit").bind("click", () => {
        let el = $("#autho_single #authoSingle_results > div");
        let data = {
          type: el.data("type"),
        ...
        authoSingle_dialog.init(data, (data) => {
          var dom = $("<div></div>");
          for(var variable in data) {
            if(data.hasOwnProperty(variable)) {
              dom.data(variable, data[variable]);
      ...
      $("#autho_single #authoSingle_list_table").on("click", "a[name=detail]", (e) => {
        let index = $(e.target).data("index");
        let data = Pane._data[index];
        AuthoSingleDetailDialog.init(data);
      })
    },
    initTable() {
      let title = [
      ...
      this.table = new Table({
        containerId: "authoSingle_list_table",
        rows: 13,
        columns: 10,
        titles: title,
        styles: style,
