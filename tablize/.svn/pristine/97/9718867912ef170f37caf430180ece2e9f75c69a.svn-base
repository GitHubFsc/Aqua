(function($){
	...
	var heatStats = {
		init: function(){
			...
			$('#heatAnalysis_htsDownBtn').on('click', function(){
				self.downloadStats();
			});
		},
		setTypeSelector: function(){
			var sel = new StyledSelector({
				containerId: 'heatAnalysis_heatStatsType',
				styles: {
					optionHeight: 30,
					iconSize: 18,
					iconColor: '#2ea2d7',
		...
		changeCheckPanel: function(val){
			...
			$('#heatAnalysis_htStartDate').val('');
			$('#heatAnalysis_htEndDate').val('');
			switch(val){
			case 'day':
			case 'week':
			case 'month':
				$('#heatAnalysis_htsCheckCalPanel').show();
				self.buildCalendarPanel(val);
				break;
			case 'custom':
				$('#heatAnalysis_htsCheckCustom').show();
		...
		buildCalendarPanel: function(val){
			...
			$('#heatAnalysis_htsCheckCalendar').empty().append(frag);
			switch(val){
			case 'day':
				var today = new Date();
				...
				$('#heatAnalysis_htStartDate').val(tstr);
				$('#heatAnalysis_htEndDate').val(tstr);
				break;
			case 'week':
				var today = new Date();
				...
				$('#heatAnalysis_htStartDate').val(wfstr);
				$('#heatAnalysis_htEndDate').val(wlstr);
				break;
			case 'month':
				var $curs = $('.hts-cal-cell-cur');
				...
				$('#heatAnalysis_htStartDate').val(mfstr);
				$('#heatAnalysis_htEndDate').val(mlstr);
				break;
			}
		},
		...
		buildCustomPanel: function(){
			...
			var interSel = new StyledSelector({
				containerId: 'heatAnalysis_htsCustomSel',
				styles: {
					optionHeight: 30,
					iconSize: 18,
					iconColor: '#2ea2d7',
			...
			interSel.onChange = function(){
				...
				switch(val){
				case 'day':
					$('#heatAnalysis_htsCustomDayShortcut').show();
					$('#heatAnalysis_htsCustomDay').show();
					var $check = $('#heatAnalysis_htsCustomDayDefault');
					$check.trigger('change');
					if(!$check.prop('checked')){
						$('#heatAnalysis_htStartDate').val(self.startDatePicker.getDateStr());
						$('#heatAnalysis_htEndDate').val(self.endDatePicker.getDateStr());
					}
					break;
				case 'week':
					$('#heatAnalysis_htsCustomDayShortcut').hide();
					$('#heatAnalysis_htsCustomWeek').show();
					var today = new Date();
					var wldate = new Date(today.setDate(today.getDate() + 0 - today.getDay() - 1));
					var wftmp = new Date(wldate);
					var weeknum = Number($('#heatAnalysis_htsCustomWeekNum').attr('data-num'));
				...
				case 'month':
					$('#heatAnalysis_htsCustomDayShortcut').hide();
					$('#heatAnalysis_htsCustomMonth').show();
					var monthNum = Number($('#heatAnalysis_htsCustomMonthNum').attr('data-num'));
					var today = new Date();
			...
			var startPicker = new DatePicker($.extend({
				containerId: 'heatAnalysis_htsCustomDayFrom'
			}, datePickerStyle));
			var endPicker =new DatePicker($.extend({
				containerId: 'heatAnalysis_htsCustomDayTo'
			}, datePickerStyle));
			self.startDatePicker = startPicker;
			self.endDatePicker = endPicker;
			startPicker.onChange = function(){
				var dtstr = this.getDateStr();
				$('#heatAnalysis_htStartDate').val(dtstr);
			};
			endPicker.onChange = function(){
				var dtstr = this.getDateStr();
				$('#heatAnalysis_htEndDate').val(dtstr);
			};

			$('#heatAnalysis_htsCustomDayDefault').off().on('change', function(){
				var $check = $(this);
				if($check.prop('checked')){
					var today = new Date();
		...
		downloadStats: function(){
			var self = this;
			var startDate = $('#heatAnalysis_htStartDate').val();
			var endDate = $('#heatAnalysis_htEndDate').val();
