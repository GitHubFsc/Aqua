var autheBatch_dialog = (function ($, Dialog, Combo, TimeSelector) {
  let Data = {
      smartcardId: []
    },
    callback;
  let Pane = {
    ...
    initWidgets() {
      ...
      this.widget.date_range = new Combo({
        containerId: "autheBatch_date_range",
        options: [{
      ...
      this.widget.date_start = new TimeSelector({
        containerId: 'autheBatch_date_start',
      ...
      this.widget.date_end = new TimeSelector({
        containerId: 'autheBatch_date_end',
        calendarStyles: calendarStyles,
        dateInputStyles: {
          borderColor: '#d3d3d3'
        },
      ...
      this.widget.type_combo = new Combo({
        containerId: "autheBatch_type_range",
        options: [{
          label: "asset",
          value: "asset"
    			}, {
      ...
      $(".query_dialog_autheBatch ._dialog_body #autheBatch_self_define").bind("click", (e) => {
        ...
        if(value == i18n("AUTH_MNG_DIALOG_SELFDEFINE")) {
          ...
          this.widget.date_range.disable();
          $("#autheBatch_date_start").parent("._dialog_row").show();
          $("#autheBatch_date_end").parent("._dialog_row").show();
          $(".query_dialog_autheBatch").removeClass("_dialog_center")
        } else {
          ...
          $("#autheBatch_date_start").parent("._dialog_row").hide();
          $("#autheBatch_date_end").parent("._dialog_row").hide();
          $(".query_dialog_autheBatch").addClass("_dialog_center")
        }
      });
    },
    bindEvents() {
      $(".query_dialog_autheBatch ._dialog_foot ._query").bind("click", () => {
        Pane.dialog.close();
        callback && callback(Data);
      });
      $(".query_dialog_autheBatch ._dialog_title ._dialog_close").bind("click", () => {
        Pane.dialog.close();
      });
      $(".query_dialog_autheBatch ._dialog_foot ._btn._cancel").bind("click", () => {
        Pane.dialog.close();
      });
      $(".query_dialog_autheBatch ._dialog_body #autheBatch_app_key").bind("blur", (e) => {
        Data.appKey = $(e.target).val();
      });
      // $(".query_dialog_autheBatch ._dialog_body #smartcard_id").bind("blur", (e) => {
      //   Data.smartcardId = $(e.target).val();
      // });
      $(".query_dialog_autheBatch ._dialog_body ._btn._add").bind("click", (e) => {
        let _exist = false;
        let _val = $(".query_dialog_autheBatch ._dialog_body #autheBatch_smartcard_id").val();
        ...
        this.loadSmartCard(Data.smartcardId);
         $(".query_dialog_autheBatch ._dialog_body #autheBatch_smartcard_id").val("");
      });
      $(".query_dialog_autheBatch ._dialog_body ._table").on("click", "._table_cell[name=del]", (e) => {
        let _exist = false;
    ...
    loadData() {
      ...
      $(".query_dialog_autheBatch #autheBatch_app_key").val(Data.appKey);
      this.loadSmartCard(Data.smartcardId);
    },
    loadSmartCard(data) {
      let ctn = $(".query_dialog_autheBatch ._dialog_body ._table ._table_body").empty();
