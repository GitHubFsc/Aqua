var supervisor = {
  ...
  initData() {
  let ctn = jQuery("#querySupervisor_query_supervisor_ctn"),
  group = [{
    name: "缓存层",
  ...
  data = this.data,
    ...
    if (data[0].length > 0) {
      ...
      let pane = new querySupervisor_Pane(name, englishname);
      pane.appendTo(ctn)
      this.pane.push(pane);
      for(var i = 0; i < 4; i++) {
        var item = data[0][i]
        if(item) {
          let block = new querySupervisor_Block({
            name: item.name,
            data: item
          });
    ...
    if(data[2][0].length > 0) {
      let asset = new querySupervisor_Set("Asset");
    ...
    if (data[2][1].length>0) {
      let user = new querySupervisor_Set("User");
    ...
    if (data[2][2].length>0) {
      let trans = new querySupervisor_Set("Transaction");
    ...
    if (data[2][3].length>0) {
      let adapter = new querySupervisor_Set("TVBSAdapter");
    ...
    if (data[2][4].length > 0) {
      let common = new querySupervisor_Set("Common");
