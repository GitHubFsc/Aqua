...
var node_bundle = (function($, Table){
  var Pane = {
    ...
    init({type = Pane.type, el}) {
      var dom = `<div id='navigation_bundle-pane'>${
            type == 'manul'  ?
            `
            <div class='btn-bar'>
              ...
              <div class='btn-body'>
                ...
                <div class='btn' name='add_bundle'>${i18n('NAVIGATE_EDITOR_ADDBUNDLE')}</div>
                <div class="i-check"><input name="check" id="navigation_checkBox" type="checkbox"><label></label></div>
          ...
          <div id='nav_tree_asset_list' class='list_table' style='margin: 0;'></div>
          </div>`
      $(el).empty().append($(dom))
      this.initTable();
      this.bindEvents();
    ...
    initTable() {
      ...
      Pane.table = new Table({
        containerId: "nav_tree_asset_list",
    ...
    bindEvents(){
      $('#navigation_bundle-pane #nav_tree_asset_list.list_table ')
      ...
      $('#navigation_bundle-pane .btn-bar')
      ...
      .on('click', '.btn[name=add_bundle]', ({target, currentTarget}) => {
        if (currentTarget == target) {
          var dialog = new navigation_add_asset({
            type: 'bundle',
            node: Model.parentNode,
      ...
      .on('click', '.btn[name=settop]', ({target, currentTarget}) => {
        if (currentTarget == target) {
          var selectedEl = $('#navigation_bundle-pane #nav_tree_asset_list.list_table .i-check input:checked')
          var selects = []
      ...
      .on('click', '#navigation_checkBox', ({target, currentTarget}) => {
        if (currentTarget == target) {
