(function ($, Table) {
  let Pane = {
    ...
    initWidgets() {
      $("#cancelAutho_single ._frame_foot ._btn._download").bind("click", () => {
        let el = $("#cancelAutho_single #cancelAuthoSingle_results > div");
        let data = {};
        let srcdata = el.data();
      ...
      $("#cancelAutho_single ._btn._query").bind("click", () => {
        cancelAutho_dialog.init({}, (data) => {
          var dom = $("<div></div>");
          ...
          $("#cancelAutho_single #cancelAuthoSingle_results").empty().append(dom);
          $("#cancelAutho_single ._btn._edit").show();
          API.getList(data, (resp) => {
            Pane.table.update(Models.table(resp))
          });
        });
      });
      $("#cancelAutho_single ._query_condition ._btn._edit").bind("click", () => {
        let el = $("#cancelAutho_single #cancelAuthoSingle_results > div");
        let data = {
          type: el.data("type"),
        ...
        cancelAutho_dialog.init(data, (data) => {
          var dom = $("<div></div>");
          for(var variable in data) {
            if(data.hasOwnProperty(variable)) {
              dom.data(variable, data[variable]);
      ...
      $("#cancelAutho_single #cancelAuthoSingle_list_table").on("click", "a[name=detail]", (e) => {
        let index = $(e.target).data("index");
        let data = Pane._data[index];
        CancelAuthoDetailDialog.init(data);
      })
    },
    initTable() {
      let title = [
      ...
      this.table = new Table({
        containerId: "cancelAuthoSingle_list_table",
        rows: 13,
        columns: 9,
        titles: title,
        styles: style,
  ...
  let API = {
    getList(data, callback) {
        let putData = [];
        putData.push("event_time=" + data.startDate + "T00:00:00+0800&event_time_op=gt&event_time=" + data.endDate + "T23:59:59+0800&event_time_op=lt&user_id=" + data.smartcardId + "&user_id_op=eq&asset_type=" + data.type + "&asset_type_op=eq&is_delete=1&is_delete_op=eq");
        $.ajax({
          type: "POST",
      ...
      download(data) {
        let putData = [];
        putData.push("event_time=" + encodeURIComponent(data.startDate + "T00:00:00+0800") + "&event_time_op=gt&event_time=" + encodeURIComponent(data.endDate + "T23:59:59+0800") + "&event_time_op=lt&user_id=" + data.smartcardId + "&user_id_op=eq&asset_type=" + data.type + "&asset_type_op=eq&is_delete=1&is_delete_op=eq");
