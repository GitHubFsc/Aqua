	...
	var TabId = 0;
                ...
                if(menu.isGroup){
                    ...
                    .attr('attr-meta', menu.meta || '').on('click', function(){
                        ...
                        if(menu.contents){
                        	...
                        	$(menuView).trigger('viewresize').trigger('entermenu', [menu.id, menu.name, menu.meta]);
                        }
                    }).hover(function(){
                        addMenuTooltip(this);
                    }, function(){
            ...
            function onServantClick(){
            	$('.servant_menu_focus').each(function(){
             	...
             	var id = $servant.attr('id');
							var name = $servant.html();
             	var meta = $servant.attr('attr-meta');
             	$(menuView).trigger('entermenu', [id, name, meta]);
            }
      ...
      navigationManagementSlider: {name: '轮播编排', id: 'nav_manage_slider'},
		...
		var adminMenuData = [{
			...
			children: [{
				name: "节目", id: "asset",
				contents: [menuEntries.asset, menuEntries.assetShelf, menuEntries.setTagByTag, menuEntries.assetMeta, menuEntries.assetTagLib,menuEntries.assetImport, menuEntries.assetDel]
		...
		var menuView = {
			...
			listenEvents: function(){
				...
				$(menuView).on('entermenu', function(ev, id, name, meta){
					...
					menuView.activeTab({id, name, meta})
			...
			activeTab: function(menu) {
				var $ = jQuery, exist = $('#tabs_header .' + menu.id + '_tab').length > 0 ? true: false;
				if (exist) {
					var tabId = $('#tabs_header .' + menu.id + '_tab').attr('tabid');
					menuView.showTab(tabId)
				} else {
					menuView.addTab(menu)
				}
			},
			addTab: function(menu) {
				...
				var id = menu.id
				TabId += 1;
				var currentTabId = [id, TabId].join('_')
				$('#tabs_header').append(
					$('<div>').addClass('tab_header').addClass(id + '_tab').attr('tabid', currentTabId)
					.append(
						$('<div>').addClass('tab_header_name').html(menu.name).attr('title', menu.name)
					)
					.append(
						$('<div>').addClass('tab_close_btn').one('click', () => {
							this.destroyTab(currentTabId);
						})
					)
					.on('click', ':not(.tab_close_btn)', () => {
						this.showTab(currentTabId);
					})
				)
				$('#tabs_content').append(
					$('<div>').attr('id', currentTabId).addClass('tab_content')
				)
				this.showTab(currentTabId);
					switch(id){
					case 'single_query':
					navigate.toDemandRecord(currentTabId, $('#m_20')[0], function(){
						demandRrecord.toSingleQuery(currentTabId, $('#demand_record_single_query_container')[0]);
						});
						break;
					case 'multi_detail_list':
					navigate.toDemandRecord(currentTabId, $('#m_20')[0], function(){
						demandRrecord.toBatchQuery(currentTabId, $('#demand_record_batch_query_container')[0]);
						});
						break;
					case 'multi_statistics':
					navigate.toDemandRecord(currentTabId, $('#m_20')[0], function(){
						demandRrecord.toBatchQueryStatistics(currentTabId, $('#demand_record_batch_query_container')[0]);
						});
						break;
					case 'export_task':
					navigate.toDemandRecord(currentTabId, $('#m_20')[0], function(){
						demandRrecord.toBatchExport(currentTabId, $('#demandRrecord_invit_list_container')[0]);
						});
						break;
					case 'stat_setting':
					navigate.toDemandRecord(currentTabId, $('#m_20')[0], function(){
						demandRrecord.toBasicSettings(currentTabId, $('#demand_record_basicsetting')[0]);
						});
						break;
					case 'channel_realtime':
					navigate.toRealtimeAnalysis(currentTabId, $('#realtime_analysis')[0], function(){
						window.JS.auAnalysis.realtimeAnalysis.analyzeChannelRealtime(currentTabId, $('#channel_realtime_container')[0]);
						});
						break;
					case 'channel_history':
					navigate.toRealtimeAnalysis(currentTabId, $('#realtime_analysis')[0], function(){
						window.JS.auAnalysis.realtimeAnalysis.analyzeChannelHistory(currentTabId, $('#channel_history_container')[0]);
						});
						break;
					case 'program_history':
					navigate.toRealtimeAnalysis(currentTabId, $('#realtime_analysis')[0], function(){
						window.JS.auAnalysis.realtimeAnalysis.analyzeProgramHistory(currentTabId, $('#program_history_container')[0]);
						});
						break;
					case 'user_admin':
					navigate.toUserAdmin(currentTabId);
						break;
					case 'usergroup_admin':
					navigate.toUserGroupAdmin(currentTabId);
						break;
          case 'user_tag':
					navigate.toUserTag(currentTabId);
            break;
            case 'app_admin':
					navigate.toAppAdmin(currentTabId);
						break;
					case 'heat_analysis':
					navigate.toHeatStats(currentTabId);
						break;
					case 'product_package':
					navigate.toProductPkg(currentTabId, function(){
							productPackage.init();
						});
						break;
					case "query_supervisor":
				navigate.toSupervisor(currentTabId, function(){
						supervisor.init()
					});
					break;
					case "authe_sing":
					navigate.toAutheSingle(currentTabId);
					break;
					case "authe_batch":
					navigate.toAutheBatch(currentTabId);
					break;
					case "authe_info":
					navigate.toAutheInfo(currentTabId);
					break;
					case "autho_sing":
					navigate.toAuthoSingle(currentTabId);
					break;
					case "autho_batch":
					navigate.toAuthoBatch(currentTabId);
					break;
					case "autho_info":
					navigate.toAuthoInfo(currentTabId);
					break;
          case "cancel_autho_sing":
					navigate.toCancelAuthoSingle(currentTabId);
            break;
          case "cancel_autho_batch":
					navigate.toCancelAuthoBatch(currentTabId);
            break;
					case "product_offering":
					navigate.toProductOffering(currentTabId, function(){
							productOffering.init();
						});
						break;
					case 'authstats':
					navigate.toAuthStats(currentTabId);
					break;
					case 'asset_list':
					navigate.toAsset(currentTabId, function(){
							AssetManagement.init();
						});
					break;
					case 'asset_del':
					navigate.toAssetDel(currentTabId, function(){
							var dialog = new AssetDel();
						});
					break;
					case 'asset_shelf':
					navigate.toAssetShelf(currentTabId, function(){
							AssetShelf.init();
						});
					break;
				case 'setTagByTag':
					navigate.setTagByTag(currentTabId, function(){
						});
					break;
					case 'asset_meta':
					navigate.toAssetMeta(currentTabId, function(){
							AssetMeta.init();
						});
					break;
					case 'ticket':
					navigate.toLectureTicket(currentTabId, function(){
							LectureTicket.init();
						});
					break;
					case 'asset_tag_lib':
					navigate.toAssetTagLib(currentTabId, function(){
						});
					break;
					case 'nav_manage':
					navigate.toNavMng(currentTabId, function(){
							navMng.init();
						})
						break;
					case 'nav_audit':
					navigate.toNavigationAudit(currentTabId, function(){
							var dialog = new NavAudit();
						});
						break;
					case 'nav_publish':
					navigate.toNavigationPublish(currentTabId, function(){
							var dialog = new NavPublish();
						});
						break;
					case 'nav_version':
					navigate.toNavigationPublishVersion(currentTabId, function(){
							var dialog = new NavVersion();
						});
						break;
					case 'entry_CMS_nav_audit':
					navigate.toCMSNavigationAudit(currentTabId, function(){
							var dialog = new NavAudit('cms_tree');
						});
						break;
					case 'entry_CMS_nav_publish':
					navigate.toCMSNavigationPublish(currentTabId, function(){
							var dialog = new NavPublish('cms_tree');
						});
						break;
					case 'entry_CMS_nav_version':
					navigate.toCMSNavigationPublishVersion(currentTabId, function(){
							var dialog = new NavVersion('cms_tree');
						});
						break;
          case 'nav_manage_slider':
					navigate.toNavMngSlider(currentTabId)
            break;
					case 'entry_day_sum':
					navigate.toOperateDaySum(currentTabId);
						break;
					case 'entry_distributionAnalysis':
					navigate.toDistributionAnalysis(currentTabId);
						break;
					case 'entry_trendAnalysis':
					navigate.toTrendAnalysis(currentTabId);
						break;
					case 'entry_userAction':
					navigate.toUserAction(currentTabId);
						break;
					case 'entry_oa_sumday':
					navigate.toOASumDay(currentTabId);
						break;
					case 'entry_oa_trendhour':
					navigate.toOATrendHour(currentTabId);
						break;
					case 'entry_oa_trendday':
					navigate.toOATrendDay(currentTabId);
						break;
					case 'entry_oa_trendweek':
					navigate.toOATrendWeek(currentTabId);
						break;
					case 'entry_oa_trendmonth':
					navigate.toOATrendMonth(currentTabId);
						break;
					case 'entry_job_manage':
					navigate.toJobManage(currentTabId);
            break;
					case 'entry_sys_user':
					navigate.toSysUser(currentTabId);
						break;
					case 'entry_asset_import':
					navigate.toAssetImport(currentTabId, function(){
							broadcastAssetImport.init();
						});
						break;
					case 'entry_CMS_nav_manage':
					navigate.toCMSAppAdmin(currentTabId, function(){
							CMSAppAdmin.init();
						});
					break;
					case 'entry_post_management':
						navigate.toPostManagement(currentTabId, function(){
			...
			showTab: function(tabId) {
				var $ = jQuery;
				if ($('#tabs_header .tab_header_active').attr('tabid') == tabId) {
					return;
				}
				$('#tabs_header .tab_header_active').removeClass('tab_header_active')
				$('#tabs_content .tab_content_active').removeClass('tab_content_active')
				$('#tabs_header').find('.tab_header[tabid=' + tabId + ']').addClass('tab_header_active');
				$('#tabs_content').find('.tab_content[id=' + tabId + ']').addClass('tab_content_active');
			},
			destroyTab: function(tabId) {
				var currentTabHeader = $('#tabs_header').find('.tab_header[tabid=' + tabId + ']'),
				next = currentTabHeader.next('.tab_header'),
				prev = currentTabHeader.prev('.tab_header'),
				showId = null;
				if (next.length > 0) {
					showId = next.attr('tabid')
				} else if (prev.length > 0) {
					showId = prev.attr('tabid')
					}
				if (showId !== null) {
					this.showTab(showId)
				}
				currentTabHeader.remove();
				$('#tabs_content').find('.tab_content[id=' + tabId + ']').remove();
