var UserGroupAdmin = (function($){
  ...
  function showMsgDialog(msg, callback){
    var dialog = new OverlayDialog({
      url: 'content/userGroupAdmin/msg-dialog.html',
      width: 470,
      height: 266,
      context: {},
      zIndex: 3000,
      id: 'user-group-admin-msg',
      callback: function(){
        $('#user-group-admin-msg-content').text(msg);
        $('#user-group-admin-msg-close').on('click', function(){
          dialog.close();
        });
        $('#user-group-admin-msg-cancel').on('click', function(){
          dialog.close();
        });
        $('#user-group-admin-msg-submit').on('click', function(){
          dialog.close();
  ...
  var userGroupList = {
    ...
    handleDropdowns: function(target, delegate){
      var self = this;
      if(self.listCnt != $('#user-group-user-admin-index')[0]){
    ...
    setList: function(){
      ...
      self.list = new StyledList({
        ...
        listType: 1,
        titles: [
          {label: i18n('USERGROUP_USERGROUPNAME')},
    ...
    openUserGroupDialog: function(uiType, user){
      ...
      var dialog = new PopupDialog({
        width: 390,
        height: 200,
        url: 'content/userGroupAdmin/user-group-dialog.html',
        context: {},
        callback: function(){
          $("#user-group-admin-user-dialog-title").text(i18n("USERGROUP_ADDUSERGROUP"));
