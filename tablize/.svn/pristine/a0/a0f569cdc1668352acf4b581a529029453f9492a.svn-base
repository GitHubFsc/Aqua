var navigationSlider = (function ($) {
  ...
  function showMsgDialog(option, callback) {
    ...
    var dialog = new OverlayDialog({
      ...
      context: {},
      zIndex: 3000,
      id: 'navigationSlider_user-admin-msg',
      callback: function () {
        if (option.buttonMsg) {
          $('#navigationSlider_user-admin-msg-submit').html(option.buttonMsg)
        }
        $('#navigationSlider_user-admin-msg-content').text(msg);
        $('#navigationSlider_user-admin-msg-close').on('click', function () {
          dialog.close();
        });
        $('#navigationSlider_user-admin-msg-cancel').on('click', function () {
          dialog.close();
        });
        $('#navigationSlider_user-admin-msg-submit').on('click', function () {
          dialog.close();
  ...
  var Main = {
    ...
    recover: function () {
      ...
      $("#navigationSlider_panel_page_head_joblist").show();
      $("#navigationSlider_panel_page_head_jobHistory .panel_page_head_title").text("");
    ...
    importDialog: {
        ...
        var taskDialog = new PopupDialog({
          ...
          callback: function () {
            ...
            var list = new scrollBarList({
              ...
              containerId: 'navigationSlider_importDialog_list',
              titles: titles,
              listType: 0,
              data: [],
              styles: {
            ...
            $("#navigationSlider_dialog_task_confirm").click(function () {
              if ($("#navigationSlider_dialog_task_confirm").hasClass('disable')) {
              } else {
                taskDialog.close();
                ViewImportLogListPage.init();
              }
      ...
      readyJumpToImportLog: function () {
        var that = this;
        $("#navigationSlider_dialog_task_confirm").removeClass('disable');
  ...
  var ViewImportLogListPage = {
    init: function () {
      ...
      $("#navigationSlider_panel_page_head_jobHistory").show();
      $("#navigationSlider_panel_page_head_jobHistory .panel_page_head_title").text(i18n('NAVMNGSLIDER_PINDAODAORURIZHI'));
    ...
    bindEvent: function () {
      var that = this;
      $('#navigationSlider_panel_page_head_jobHistory .panel_page_head_button.back').unbind("click").click(function () {
        Main.recover();
      });
      ...
      open: function (item, callback) {
        ...
        var taskDialog = new PopupDialog({
          ...
          callback: function () {
            ...
            $('#navigationSlider_importLogDetail_id').val(arr2);
            $('#navigationSlider_importLogDetail_channel_name').val(arr3);
            $('#navigationSlider_importLogDetail_name').val(arr4);
            $('#navigationSlider_importLogDetail_source').val(arr5);
            $('#navigationSlider_importLogDetail_importtime').val(arr1);
            $('#navigationSlider_importLogDetail_starttime').val(arr6);
            $('#navigationSlider_importLogDetail_endtime').val(arr7);
            $('#navigationSlider_importLogDetail_state').val(arr8);
            $('#navigationSlider_importLogDetail_appid').val(arr9);
            $('#navigationSlider_importLogDetail_message').val(arr10);
          }
        });
        taskDialog.open();
        that.taskDialog = taskDialog;
  ...
  var ViewProgramListPage = {
    init: function (option) {
      ...
      $("#navigationSlider_panel_page_head_jobHistory").show();
      $("#navigationSlider_panel_page_head_jobHistory .panel_page_head_title").text(option.name + i18n("NAVMNGSLIDER_JIEMULIEBIAO"));
    ...
    initParm: function (option) {
      ...
      $("#navigationSlider_viewProgramList_calendar_start").empty();
      $("#navigationSlider_viewProgramList_calendar_end").empty();
      var calendarStyles = {
        width: 282,
        navTitleHeight: 25,
      ...
      that.datePicker_start = new navigationSlider_DatePicker({
        containerId: 'navigationSlider_viewProgramList_calendar_start',
      ...
      that.datePicker_end = new navigationSlider_DatePicker({
        containerId: 'navigationSlider_viewProgramList_calendar_end',
        calendarStyles: calendarStyles,
        dateInputStyles: dateInputStyles,
        hmsInputStyles: hmsInputStyles,
        iconImage: 'images/operateanalyze/oa-dp-cal-icon.png',
    ...
    initTable: function () {
      ...
      list_table.getPageData = function (page, callback) {
        ...
        var calendar_start = $("#navigationSlider_viewProgramList_calendar_start-datepicker-input").val();
        var calendar_end = $("#navigationSlider_viewProgramList_calendar_end-datepicker-input").val();
        var start_hourminute = $("#navigationSlider_viewProgramList_calendar_start-hms-input").val() || "00:00";
        var end_hourminute = $("#navigationSlider_viewProgramList_calendar_end-hms-input").val() || "23:59";
    ...
    emitCalendarsButtonEvent: function(type){
      ...
      switch (type) {
        case 'playing':
          ...
          $("#navigationSlider_viewProgramList_calendar_start-datepicker-input").val(date_start);
          $("#navigationSlider_viewProgramList_calendar_end-datepicker-input").val(date_end);
          $("#navigationSlider_viewProgramList_calendar_start-hms-input").val(hms_start);
          $("#navigationSlider_viewProgramList_calendar_end-hms-input").val(hms_end);
          break;
        case 'today':
          var now = new Date();
          var year = now.getFullYear();
    ...
    detailDialog: {
      open: function (item, callback) {
        ...
        var taskDialog = new PopupDialog({
          ...
          context: that,
          callback: function () {
            $("#navigationSlider_viewProgramDetailContent .content").mCustomScrollbar({
              theme: "light"
            });

            $('#navigationSlider_taskDialog_title').text((item.title || "") + i18n('NAVMNGSLIDER_DETAIL'));
            ...
            $('#navigationSlider_viewProgram_channel_id').val(channel_id);
            $('#navigationSlider_viewProgram_channel_name').val(channel_name);
            $('#navigationSlider_viewProgram_program_name').val(program_name);
            $('#navigationSlider_viewProgram_title').val(title);
            $('#navigationSlider_viewProgram_blank').val(blank);
            $('#navigationSlider_viewProgram_start').val(start);
            $('#navigationSlider_viewProgram_end').val(end);
            $('#navigationSlider_viewProgram_run_time').val(run_time);
            $('#navigationSlider_viewProgram_confilct').val(confilct);
            $('#navigationSlider_viewProgram_assetrecommendclass').val(assetrecommendclass);
            $('#navigationSlider_viewProgram_posterboard').val(posterboard);
            $('#navigationSlider_viewProgram_file_path').val(file_path);
            $('#navigationSlider_viewProgram_summary_short').val(summary_short);
            $('#navigationSlider_viewProgram_provider_id').val(provider_id);
            $('#navigationSlider_viewProgram_asset_id').val(asset_id);
            $('#navigationSlider_viewProgram_status').val(status);
            $('#navigationSlider_viewProgram_estatus').val(estatus);
            $('#navigationSlider_viewProgram_emessage').val(emessage);
          }
        });
        taskDialog.open();
        that.taskDialog = taskDialog;
  ...
  var EditProgramListPage = {
    ...
    assetDialog: {
      ...
      bindEvents: function(){
        let that = this;
        $('#navigationSlider_navigation_add_asset')
          .on('click', '._dialog_foot ._cancel', () => {
            this.dialog.close();
          })
          .on('click', '._dialog_foot ._confirm', ({currentTarget, target}) => {
            if (currentTarget ==  target) {
              var selectedEl = $('#navigationSlider_navigation_asset_list .i-check input:checked');
          ...
          .on('click', '#navigationSlider_navigation_asset_list .i-check input', ({currentTarget, target}) => {
            if (currentTarget == target) {
      ...
      initTable: function(){
        ...
        let table = new StyledList({
          containerId: "navigationSlider_navigation_asset_list",
          rows: 12,
          columns: 7,
          titles: title,
          styles: style,
        ...
        table.getPageData = function (page, callback) {
          var start = (page - 1) * 12;
          var end = page * 12 - 1;
          var search = $('#navigationSlider_search_input').val();
          var option = {
            start: start,
            end: end,
            type: 'asset',
