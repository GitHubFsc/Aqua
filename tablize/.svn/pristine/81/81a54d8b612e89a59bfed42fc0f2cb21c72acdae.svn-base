var jobManage = (function ($) {
  ...
  var option_type_list = {
    ...
    addSelect: function (parameter, OptionContainer) {
      ...
      var Select = new TicketnewSelect(OptionDiv, [], {
        width: 240,
        height: 32,
        background: "white",
        fontSize: 14,
    ...
    addDate: function (parameter, OptionContainer) {
      var that = this;
      var id = "jobManage_taskDialog_option_" + parameter.pname;
  ...
  function showMsgDialog(msg, callback) {
    var dialog = new OverlayDialog({
      ...
      context: {},
      zIndex: 3000,
      id: 'jobManage_user-admin-msg',
      callback: function () {
        $('#userTag-admin-msg-content').text(msg);
        $('#userTag-admin-msg-close').on('click', function () {
          dialog.close();
        });
        $('#userTag-admin-msg-cancel').on('click', function () {
          dialog.close();
        });
        $('#userTag-admin-msg-submit').on('click', function () {
          dialog.close();
  ...
  var Main = {
    ...
    initModule: function () {
      ...
      var taskNameSelect = new TicketnewSelect("#jobManage_select_taskName", [], {
        width: 111,
        height: 30,
        background: "#4FB8F1",
        fontSize: 12,
      ...
      var isTimingSelect = new TicketnewSelect("#jobManage_select_isTiming", [], {
        width: 125,
        height: 30,
        background: "#4FB8F1",
        fontSize: 12,
      ...
      var enableSelect = new TicketnewSelect("#jobManage_select_startStatus", [], {
        width: 125,
        height: 30,
        background: "#4FB8F1",
        fontSize: 12,
      ...
      var typeSelect = new TicketnewSelect("#jobManage_select_type", [], {
        width: 121,
        height: 30,
        background: "#4FB8F1",
        fontSize: 12,
    ...
    bindEvent: function () {
      var that = this;
      $('#jobManage_job_list_search').keyup(function (event) {
        if (event.keyCode == 13) {
          that.options.taskNameSelect.setKey("")
          that.options.name = $("#jobManage_job_list_search").val();
          that.list_table.update();
        }
      });
      $("#jobManage_job_list_search_handle").click(function () {
        that.options.taskNameSelect.setKey("")
        that.options.name = $("#jobManage_job_list_search").val();
        that.list_table.update()
      });
    ...
    recover: function () {
      ...
      $("#jobManage_panel_page_head_joblist").show();
      $("#jobManage_panel_page_head_jobHistory").hide();
      that.list_table.refreshList();
  ...
  var jobDetailPage = {
    ...
    initModule: function () {
      ...
      var isTimingSelect = new TicketnewSelect("#jobManage_job_isTiming", [], {
        width: 295,
        height: 34,
        background: "white",
        fontSize: 14,
      ...
      var taskTypeSelect = new TicketnewSelect("#jobManage_job_taskType", [], {
        width: 148,
        height: 34,
        background: "white",
        fontSize: 14,
      ...
      var scheduleSelect = new TicketnewSelect("#jobManage_job_schedule", [], {
        width: 295,
        height: 34,
        background: "white",
        fontSize: 14,
      ...
      $('#jobManage_option_job_schedule_crontab .day .item').each(function (index, element) {
      ...
      var schedule_interval_period_select = new TicketnewSelect("#jobManage_option_job_schedule_interval_period", [], {
        width: 102,
        height: 34,
        background: "white",
        fontSize: 14,
      ...
      $("#jobManage_task_list_container").mCustomScrollbar({
        theme: "light"
      });
      jQuery("#jobManage_task_list_container" + " .mCSB_inside > .mCSB_container").css({
        "margin-right": "0px"
      })
    },
    bindEvent: function () {
      var that = this;
      $('#jobManage_createNewTaskButton').unbind("click").click(function () {
      ...
      that.options.isTimingSelect.listenLiClick = function (value) {
        if (value == "1") {
          $("#jobManage_option_job_schedule").show();
          $("#jobManage_option_job_schedule_crontab").show();
        } else {
          $("#jobManage_option_job_schedule").hide();
      ...
      that.options.scheduleSelect.listenLiClick = function (value) {
        if (value == "crontab") {
          ...
          $("#jobManage_option_job_schedule_interval").show();
          clearCrontab()
        }
      };
      $('#jobManage_option_job_schedule_crontab').on('click', '.month .item', function () {
        var item = $(this);
        var value = item.attr('data-value');
        item.toggleClass("selected")
      });
      $('#jobManage_option_job_schedule_crontab').on('click', '.oddeven .item', function () {
        var item = $(this);
        var type = item.attr('data-type');
        item.toggleClass("selected");


        var odd_selected = $("#jobManage_option_job_schedule_crontab .oddeven .item[data-type=odd]").hasClass("selected");
        var even_selected = $("#jobManage_option_job_schedule_crontab .oddeven .item[data-type=even]").hasClass("selected");
        $('#jobManage_option_job_schedule_crontab .day .item').removeClass("selected");
        odd_selected ? $('#jobManage_option_job_schedule_crontab .day .item[data-daytype=odd]').addClass('selected') : $('#jobManage_option_job_schedule_crontab .day .item[data-daytype=odd]').removeClass('selected')
        even_selected ? $('#jobManage_option_job_schedule_crontab .day .item[data-daytype=even]').addClass('selected') : $('#jobManage_option_job_schedule_crontab .day .item[data-daytype=even]').removeClass('selected')
      });
      $('#jobManage_option_job_schedule_crontab').on('click', '.week .item', function () {
        var item = $(this);
        var value = item.attr('data-value');
        item.toggleClass("selected")
      });
      $('#jobManage_option_job_schedule_crontab').on('click', '.day .item', function () {
        var item = $(this);
        var value = item.attr('data-value');
        if (value == "")return;
        item.toggleClass("selected");

        var odd_length = $('#jobManage_option_job_schedule_crontab .day .item[data-daytype=odd].selected').size();
        var even_length = $('#jobManage_option_job_schedule_crontab .day .item[data-daytype=even].selected').size();
        if (odd_length == 16) {
          $("#jobManage_option_job_schedule_crontab .oddeven .item[data-type=odd]").addClass('selected');
        } else {
          $("#jobManage_option_job_schedule_crontab .oddeven .item[data-type=odd]").removeClass('selected');
        }
        if (even_length == 15) {
          $("#jobManage_option_job_schedule_crontab .oddeven .item[data-type=even]").addClass('selected');
        } else {
          $("#jobManage_option_job_schedule_crontab .oddeven .item[data-type=even]").removeClass('selected');
        }
      });
      ...
      function clearCrontab() {
        $('#jobManage_option_job_schedule_crontab .item').removeClass('selected');
        $("#jobManage_option_job_schedule_crontab").hide();
      }

      function clearInterval() {
        $('#jobManage_option_job_schedule_interval_every').val('');
        that.options.schedule_interval_period_select.setKey("minutes");
        $("#jobManage_option_job_schedule_interval").hide();
      }
    },
    close: function () {
      var that = this;
    ...
    check: function () {
      ...
      var that = this;
      var result = true;
      var job_name = $("#jobManage_job_name").val();
      if (job_name == "") {
        result = false;
        $("#jobManage_option_job_name .regist-star").show();
      }
      ...
      if (job_isTiming == "1") {
        ...
        if (job_schedule == "crontab") {
          var length = $('#jobManage_option_job_schedule_crontab .item.selected').size();
          if (length == 0) {
            result = false;
            $("#jobManage_option_job_schedule_crontab .regist-star").show();
          }
          var hour = $('#jobManage_option_job_schedule_crontab_hour').val();
          var minute = $('#jobManage_option_job_schedule_crontab_minute').val();
          if ((hour == "") || (minute == "")) {
            result = false;
            $("#jobManage_option_job_schedule_crontab .regist-star").show();
          }
        } else {
          var job_schedule_interval_every = $("#jobManage_option_job_schedule_interval_every").val();
          if (job_schedule_interval_every == "") {
            result = false;
            $("#jobManage_option_job_schedule_interval .regist-star").show();
          }
        }
      }
      ...
      if (taskList.length == 0) {
        result = false;
        $("#jobManage_option_job_tasks .regist-star").show();
      }
      if (job_taskType == "chain") {
      } else {
        if (taskList.length > 1) {
          result = false;
          $("#jobManage_option_job_taskType .regist-star").show();
    ...
    set: function (item) {
      ...
      if (that.readonly) {
        ...
        $("#jobManage_createNewTaskButton").hide();
        ...
        $('#jobManage_option_job_schedule_crontab').unbind("click");
        $('#jobManage_option_job_schedule_crontab .item').addClass("readonly")
      }
      if (item.type == "user") {
        $("#jobManage_job_type").val(i18n("JOBMANAGE_JOBTYPE_USER"));
      } else if (item.type == "sys") {
        $("#jobManage_job_type").val(i18n("JOBMANAGE_JOBTYPE_SYS"));
      } else {
        $("#jobManage_job_type").val(item.type);
      }
      $("#jobManage_job_name").val(item.name);
      if (item.schedule) {
        ...
        if (item.schedule.crontab) {
          ...
          $("#jobManage_option_job_schedule_crontab_hour").val(hour);
          $("#jobManage_option_job_schedule_crontab_minute").val(minute);
          ...
          day_of_month.forEach(function (item) {
            $('#jobManage_option_job_schedule_crontab .day .item[data-value=' + item + ']').addClass("selected");
          });
          day_of_week.forEach(function (item) {
            $('#jobManage_option_job_schedule_crontab .week .item[data-value=' + item + ']').addClass("selected");
          });
          month_of_year.forEach(function (item) {
            $('#jobManage_option_job_schedule_crontab .month .item[data-value=' + item + ']').addClass("selected");
          });
          var odd_length = $('#jobManage_option_job_schedule_crontab .day .item[data-daytype=odd].selected').size();
          var even_length = $('#jobManage_option_job_schedule_crontab .day .item[data-daytype=even].selected').size();
          if (odd_length == 16) {
          ...
          $("#jobManage_option_job_schedule_interval_every").val(item.schedule.interval.every || "");
      ...
      if (typeof item.timeout !== 'undefined') {
        $("#jobManage_job_timeout").val(item.timeout);
      }
    ...
    get: function () {
      ...
      if (job_isTiming == "1") {
        ...
        if (job_schedule == "crontab") {
          ...
          var crontab_minute = $('#jobManage_option_job_schedule_crontab_minute').val();
          var crontab_hour = $('#jobManage_option_job_schedule_crontab_hour').val();
          if(crontab_minute){
          ...
          var day_of_week = [];
          $('#jobManage_option_job_schedule_crontab .week .item.selected').each(function (index, element) {
            day_of_week.push($(element).attr('data-value'));
          });
          var day_of_month = [];
          $('#jobManage_option_job_schedule_crontab .day .item.selected').each(function (index, element) {
            day_of_month.push($(element).attr('data-value'));
          });
          var month_of_year = [];
          $('#jobManage_option_job_schedule_crontab .month .item.selected').each(function (index, element) {
      ...
      var job_timeout = $("#jobManage_job_timeout").val();
      if(job_timeout){
    ...
    taskDialog: {
      open: function (data, callback) {
        ...
        var taskDialog = new PopupDialog({
          ...
          callback: function () {
            ...
            var taskNameSelect = new TicketnewSelect("#jobManage_dialog_taskName", [], {
              width: 306,
              height: 32,
              background: "white",
              fontSize: 14,
            ...
            if(data){
              $('#jobManage_taskDialog_title').html(i18n('JOBMANAGE_EDITTASK'));
              $('#jobManage_dialog_task_confirm').html(i18n('LECTURETICKET_BIANBSPNBSPJI'));
              that.set(data);
            }

            $("#jobManage_dialog_task_confirm").click(function () {
              if (that.checkRequired()) {
                var result = that.get();
                taskDialog.close();
      ...
      initOptions: function (taskDefinition, data) {
        var that = this;
        var container = $("#jobManage_taskDialog_options");
        container.empty();
        that.container = container;
        ...
        var height = $("#jobManage_taskDialog_options").height() + 205;
      ...
      checkRequired: function () {
        $("#jobManage_taskDialog_options .jobManage_option_halfRow .regist-star").hide();
        var that = this;
        var result = true;
    ...
    taskList: {
          ...
          switch (type) {
            case 'del':
              showMsgDialog(i18n("USERGROUP_DELETE_CONTENT1") + "Task_" + (index + 1) + i18n("USERGROUP_DELETE_CONTENT2"), function () {
                that.deleteTask(id);
                $("#jobManage_task_list_container").mCustomScrollbar("update");
              });
              break;
            case 'edit':
      ...
      addTask: function (task) {
        ...
        var $container = $("#jobManage_task_list_container .background");
  ...
  var jobListPage = {
    init: function (option) {
      ...
      $("#jobManage_list").hide();
      $("#jobManage .panel_page_head_right").hide();
      $("#jobManage_panel_page_head_joblist").hide();
      $("#jobManage_panel_page_head_jobHistory").show();
      $("#jobManage_panel_page_head_jobHistory .panel_page_head_title").text(option.name);
    ...
    initJobHistory: function () {
      ...
      var jobHistoryList = new scrollBarList({
        ...
        containerId: 'jobManage_jobHistory_Table',
        titles: titles,
        listType: 0,
        data: [],
        styles: {
    ...
    initTaskHistory: function () {
      ...
      var taskHistoryList = new scrollBarList({
        rows: 12,
        columns: titles.length,
        containerId: 'jobManage_taskHistory_Table',
        titles: titles,
        listType: 0,
        data: [],
        styles: {
    ...
    bindEvent: function () {
      var that = this;
      $('#jobManage_panel_page_head_jobHistory .panel_page_head_button.back').unbind("click").click(function () {
        Main.recover();
      });
