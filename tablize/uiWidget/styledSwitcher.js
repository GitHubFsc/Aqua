...
StyledSwitcher.prototype = {
	...
	getSwitcherEls: function(contRow){
		...
		for(var i = 0; i < this.elLen; i++){
			...
			jqSwEl.append(elLabel);
			
			if(element.dropdown){
				jqSwEl.append('&nbsp;' +
					 '<div class="switcher_dropdown_icon" style="position:absolute;right:3px;top:calc(50% - 8px);">' +
					 this.getDropIcon() +
					 '</div>');
				var dropDownItem = {
					open: false
				...
				var getChangeListener = elDropDown.getChangeListener;
				if(typeof getChangeListener == 'function'){
					getChangeListener(function(el){
			...
			jqSwEl.bind('click', function(ev){
				if(ev.target === this || (this.children && (this.children.length > 0) && (ev.target === this.children[0]))){
			...
			for(var i = 0, max = this.swEls.length; i < max; i++){
				...
				if(item === dropItem){
					...
					if(dropdown.open == true){
						dropdown.open = false;
						dropdown.view.hide();
						jQuery(item).children('.switcher_dropdown_icon').empty().append(this.getDropIcon());
						if(this.elLen === 1){
		...
		jQuery.each(this.swEls, function(i, item){
			...
			if(dropdown && (i == self.focusIndex)){
				if(dropdown.open == true){
					dropdown.open = false;
					dropdown.view.hide();
					jqItem.children('.switcher_dropdown_icon').empty().append(self.getDropIcon());
					if(self.elLen === 1){
						itemBg = outColor;
					}
				}else{
					dropdown.open = true;
					dropdown.view.show();
					jqItem.children('.switcher_dropdown_icon').empty().append(self.getUpIcon());
				}
				dropdownToggle = true;
			}else if(dropdown){
				dropdown.open = false;
