function newSelect(cantainID, obj, style, listenLiClick, options){
    /*
     style{
     width,
     height,
...
newSelect.prototype.init = function(options){
    this.initParm(options);
    this.createSelect();
}

newSelect.prototype.initParm = function(options){
...
newSelect.prototype.freshselectItem = function(values, status){
    var that = this;
    var i = 0;
    var item;
    this.content.empty();
    if(values instanceof Array){
        for(i = 0;i<values.length; i++){
            ...
            if((status == "init") && (i == 0)){
                ...
                this.header.text(values[i][that.options.key]);
            }
        }
    }else{
        for(item in values){
...
newSelect.prototype.setValue = function(value, showValue){
    var that = this;
    ...
    if(data instanceof Array){
        for(i = 0 ;i<data.length; i++ ){
            if(value == data[i][that.options.value]){
                this.header.text(data[i][that.options.key]);
    ...
    if(!boolean_found){
        ...
        this.header.text((showValue ? showValue : value))
    }
}
