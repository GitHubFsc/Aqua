var trendAnalysis_Lib = (function ($) {
  ...
  var filter = {
    ...
    initContent: function () {
      ...
      var searchSelect = new newSelect("#option_search_select", searchSelectOptions, {
        width: 120,
        height: 30,
        background: "#ffffff",
        ...
        $("#option_search_inputModel input").val("");
        that.searchText = "";
    ...
    bindEvent: function () {
      var that = this;
      $("#option_search_inputModel .option_search_inputModel_button").unbind("click").click(function () {
        var result;
        var search_name = $("#option_search_inputModel input").val();
        that.searchText = search_name;
        that.searchResultList.update()
      });
      $('#ResultList').unbind("click").on('click', "input[name='searchResult']", function (ev) {
        //选择逻辑，与过滤框内容同步
      ...
      $("#FilterInput").keyup(function () {
        delay_till_last('FilterInput_writting', function () {
          var result = $("#FilterInput").val();
    ...
    updateSearchResultListCheckbox: function () {
      ...
      $("#ResultList input[name='searchResult']").removeAttr("checked").prop("checked", false);
      $("#ResultList input[name='searchResult']").each(function(index,el){
        var key = $(el).data("key") + "";
        if(result.some(function(item){
          return item === key
          })){
          $("#ResultList input[name='searchResult'][data-key='" + key + "']").removeAttr("checked").prop("checked", true);
        }
      });
    },
  };
