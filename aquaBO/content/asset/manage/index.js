...
var AssetManagement = (function ($, Table, Dialog, Combo, Switcher, Tree) {
  ...
  let Pane = {
    ...
    initTable(columns, curPage) {
      ...
      let title = [
        ...
        {label: '<div class="i-check"><input name="check" type="checkbox" ><label></label></div>', fixed: true},
      ...
      if (this.widgets) {
        if (data) {
          if(this.widgets.list){
            for (var i =0; this.widgets.list.length>i;i++) {
              let item = this.widgets.list[i];
              $("#"+item.name+"_filter").remove();
              Pane.search[item.name]="";
            }
            this.widgets.list.length = 0;
          }
          Pane.initTable();
          this.widgets.list = JSON.parse(JSON.stringify(data));
          let newarr = JSON.parse(JSON.stringify(this.widgets.list));
          for (var i =0; newarr.length>i; i++) {
            let item = newarr[i];
            var metadatatype = API.getType(item.name);
            $("#setcolumn").before("<div class='combo_filter' id="+item.name+"_filter></div>");
            let type = new Combo('#'+item.name+'_filter', Model.Type(item,metadatatype), this.widgets.ComboStyle, (value) => {
              Pane.setSearch(item.name, value)
              this.initTable();
            })
            if (newarr.length>5) {
              $("#"+item.name+"_filter").css("width",440/(newarr.length-0.5 )+"px");
              $("#"+item.name+"_filter .select-header").css({"font-size":"12px","text-indent":"0px"});
            }
          }
        }
      } else {
        ...
        this.widgets.filter = new Combo("#type_filter", filterData, this.widgets.ComboStyle, (value) => {
          switch (value) {
            case '0':
        ...
        //上下架过滤器
        this.widgets.visible = new Combo('#visible_filter', Model.visible(), this.widgets.ComboStyle, (value) => {
          Pane.setSearch('visible', value)
          this.initTable();
        })
        this.widgets.list = null;
      }
    },
    ...
    bindEvents(widgets) {
      ...
      //节目导出
      $('.asset_manage._frame .header #output').unbind().bind('click', ({currentTarget, target}) => {//
      ...
      //设置列功能入口
      $('#setcolumn img[src$="Setcolumn.png"]').unbind().bind('click', ({currentTarget, target}) => {
        if (currentTarget == target) {
          var widgets  = Pane.widgets.list;
          var titles = Pane.table.titles
          var tableDefiner = new TableDefiner({titles,widgets}, (columns) => {
            columns.titles?columns.titles=columns.titles:columns.titles=[];
            columns.widgets?columns.widgets=columns.widgets:columns.widgets=[];
            this.defaultFlexibleColumn = columns.titles;
            Pane.initTable(columns.titles, Pane.table.currPage)
            this.initWidgets(columns.widgets)
          })
        }
      });
      //列表操作
        ...
        .on('click', 'thead .i-check input', ({ currentTarget, target }) => {
          if (currentTarget == target) {
              if ($(target).is(":checked")) {
                  $("tbody .i-check input").prop('checked', true);
              } else {
                  $("tbody .i-check input").prop('checked', false);
              }
          }
        })
        .on("click", "tbody input[name=check]", (e) => {//check_box
          let el = $(e.target);
  ...
  let API = {
    ...
    getList({start, end, search, model, callback}) {
      ...
      let arr = [];API.getAssetDef((resp) => {arr =[...resp]});
      for(key in search) {
        ...
        switch (key) {
          ...
          case 'Category':
            ...
            break;
          default:
            if (arr.filter(item => item.name == key)){
              if (value) {
                opts.push(key + '=' + value + '&' + key + '_op=eq')
              }
            }else{
    ...
    output() {
      ...
      for(key in Pane.search) {
        let value = Pane.search[key];
        key=="provider_id"||key=="Provider_ID"?"":urlParam.push(key + '=' + value);
      }
      ret +='?' + urlParam.join('&');
      for (var i =1; Pane.table.titles.length>i; i++) {
        let item = Pane.table.titles[i]
        switch (item.label) {
        case "类型":
  ...
  var TableDefiner = (function() {
    var Pane = {
      init(columns) {
        var dialog = this.initDialog((resp) => {
          this.loadTitlesData(resp, () => {
            this.setChecked(columns.titles.filter(item => item.fixed == false),"titles")
          });
          this.loadWidgetsData(resp, () => {
            if (columns.widgets) {
              this.setChecked(columns.widgets.filter(item => item.fixed == false),"widgets")
            }
          });
        })
        dialog.create();
      ...
      initDialog(cb) {
        ...
        var dom = `<div id='${id}' class='_dialog'>
          <style>
            ...
            ._dialog_widgets_body,._dialog_titles_body {height: 385px;}
            ...
            #tree_detail .tabs{width: 246px;height: 36px;margin: 20px auto;display: flex;}
            #tree_detail .tabs .tab{width: 122px;height: 30px;text-align: center;line-height: 30px;border: 1px solid #d1d1d1;font-size: 16px;color: #727272; border-radius: 2px;background-color: #ededed;}
            #tree_detail .tabs .active{color: #f2f2f2;border: 1px solid #4488a7;background-color: #2cbdea; }
          </style>
          <div class='_dialog_title'>
            <div class='_dialog_title_label'>自定义</div>
            <div class='_dialog_span'></div>
            <div class='_dialog_close'></div>
          </div>
          <div id='tree_detail' class="tree_detail">
            <div class='tabs' id="tabs">
              <div class='tab active' name='widgets'>自定义筛选</div>
              <div class='tab' name='titles'>自定义列表</div>
            </div>
            <div class='tab-ctn' id="tab-ctn">
              <div id="widgets" name="widgets" class="_dialog_widgets_body" >
              </div>
              <div id="titles" name="titles" class="_dialog_titles_body" style="display: none">
              </div>
            </div>
          </div>
          <div class='_dialog_foot'>
      ...
      bindEvents(dialog) {
        ...
        .on('click', '._dialog_foot .btn.confirm', ({currentTarget, target}) => {
          if (currentTarget == target) {
            var titieschecked = $('#' + dialog.dialogId + ' #titles .i-check input:checked')
            var widgetschecked = $('#' + dialog.dialogId + ' #widgets .i-check input:checked')
            var columns={};
            columns.titles = this.getFormatedColumns(titieschecked);
            columns.widgets = this.getFormatedColumns(widgetschecked);
            if (columns.titles.length > 0 || columns.widgets.length>0 ) {
        ...
        .on('click', '._dialog_foot .reset', ({currentTarget, target}) => {
          if (currentTarget == target) {
            if ($("div[name='widgets']").is(".active")) {
              console.log($('#' + dialog.dialogId + ' #widgets .i-check input:checked'))
              $('#' + dialog.dialogId + ' #widgets .i-check input:checked').removeAttr('checked')
            }else{
              console.log($('#' + dialog.dialogId + ' #titles .i-check input:checked'))
              $('#' + dialog.dialogId + ' #titles .i-check input:checked').removeAttr('checked')
            }
          }
        })
      ...
      loadTitlesData(dialog, callback) {
        this.api.getContent((resp) => {
          this.model.format(resp, (fragment) => {
            $('#' + dialog.dialogId).find('._dialog_titles_body').append(fragment)
            $('#' + dialog.dialogId + ' ._dialog_titles_body').mCustomScrollbar({ 
              theme: 'groundgreen'
            });
            callback && callback();
          })
      ...
      loadWidgetsData(dialog, callback) {
        this.api.getContent((resp) => {
          this.model.widgets(resp,(fragment) => {
            $('#' + dialog.dialogId).find('._dialog_widgets_body').append(fragment)
            $('#' + dialog.dialogId + ' ._dialog_widgets_body').mCustomScrollbar({
              theme: 'groundgreen'
            });
            callback && callback();
          })
      ...
      setChecked(checked,type) {
        for (var i = 0; i < checked.length; i++) {
          ...
          var {name} = item;
          if (type=="widgets") {
            $('#widgets .i-check[data-name=' + name + '] input').click();
          }else{
            $('#titles .i-check[data-name=' + name + '] input').click();
          }
        }
      }
    };
    ...
    var Model = {
      ...
      widgets(resp, callback){
        var ds = []
        for (var i = 0; i < resp.length; i++) {
          var item = resp[i]
          item.name=="Category"||item.name=="category"?"":ds.push(item);
        }
        var fragment = $('<div>').addClass('');
          var dom = `<div>
            <div class='content'></div>
        ...
        for (var i = 0; i < ds.length; i++) {
          ctn.find('.content').append(this.item(ds[i],callback))
          fragment.append(ctn);
        }
        callback(fragment)
      },
