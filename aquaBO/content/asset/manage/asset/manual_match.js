    ...
    let Model = {
        table(data) {
          ...
          for(var i = 0; i < data.length; i++) {
            ...
            let op_content = "<a name='content' data-index='" + i + "'>" + i18n("ASSET_TABLE_OPR_CONTENT") + "</a>";
            let op_del = "<a name='del' data-index='" + i + "'>" + i18n("ASSET_TABLE_OPR_EDL") + "</a>";
            row.push({
              label: "<span class='op'>" + [op_content, op_del].join("") + "</span>"
            });
            list.push(row);
          }
          return list;
