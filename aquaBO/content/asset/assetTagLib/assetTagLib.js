var assetTagLib = (($, Table, Dialog) => {
    ...
    let tagSpaceType="assettag";
		...
		this.resultData=[];
		this.currentTagIsAlwaysValid=false;//新增或编辑标签时，默认不永久有效
		...
		function addEditTag_(para,choosedTagShowNameInTable,choosedTagStoredName,obj_){
			 ...
			 if( (para!="newAdd")&&(para!="edit") ){//删除标签
			  ...
			  var inputValue=document.getElementById("addTagPopup_input").value
				...
				var currentTagIsAlwaysValid=that.currentTagIsAlwaysValid;
					...
					error:function(error){
						...
						if(para=="edit"){
							...
							callback: function(){
								...
								jq("#productDel_ok").unbind().bind('click',{choosedTagStoredName_encode: choosedTagStoredName_encode},function(events) {
										that.dialog_confirmUpdateTagsInProuducts.close();
								...
								jq("#productDel_cancel").unbind().bind("click", () => {
									that.dialog_confirmUpdateTagsInProuducts.close();
								});
						}});		
		...
		function bindEvents_dialog_addEditTag(para,choosedTagShowNameInTable,choosedTagStoredName,obj_,validTimeStart,validTimeEnd){
          ...
          containerId: 'addTagPopup_validTimeEnd'
				...
				jq("#addTagPopup_alwaysValid").unbind().bind('click', function () {
				  that.currentTagIsAlwaysValid==false?that.currentTagIsAlwaysValid=true:that.currentTagIsAlwaysValid=false;
					var dislay_alwaysValid=that.currentTagIsAlwaysValid?"block":"none";
					...
					if(that.currentTagIsAlwaysValid==true){
		...
		function setTimeAlwaysValid(){
		    var jq=jQuery;
				this.currentTagIsAlwaysValid=true;
		...
		function initTable() {
		  var that=this;
			...
			var list_table_table = new Table({
				...
				specialStyle:{"tdTextAlignLeftIndex":6/that.itemsCountOneRow,"tdColorBlueCursorPointerIndex":1}
			});
	...
	function getServeDatas(pageNum, callback){
		...
		$.ajax({
				...
				for(i=lastRowBeginIndex;i<result_length;i++){//最后一行的第一个。
					...
					jq(this).unbind().bind('click', function () {
						  let dialog_addTag = new Dialog({
						      ...
						      var choosedTagStoredName=jq(editObj).parent().find(".tag_name_stored").text();
									var effective_time=that.resultData&&that.resultData[objIndex]&&that.resultData[objIndex].effective_time||"";
									var expiration_time=that.resultData&&that.resultData[objIndex]&&that.resultData[objIndex].expiration_time||"";
