...
batchExport.initParam = function () {
	...
	this.curPage = 1; //当前页
	this.spliter = new SpliterTwo("batchExport.spliter", "page_spliter", this.pageSize,
...
batchExport.resizePage = function () {
	...
	var topHeight = jQuery(".pageTitleContainerB").outerHeight() + parseInt(jQuery(
		"#pageAllBody").css("margin-top").replace("px", "")) + jQuery(
	...
	jQuery("#resultTable").outerHeight(clientHeight - topHeight - bottomHeight -
		tableHead);
};
/*
 * 打开查询窗口
...
batchExport.month_ = function (num_) {
	...
	this.endDate = _cur_date.getFullYear() + "-" + zerolize(_cur_date.getMonth() + 1, 2) + "-" + zerolize(_cur_date.getDate(), 2);
	this.beginDate = _begin_d.getFullYear() + "-" + zerolize(_begin_d.getMonth() + 1, 2) + "-" + zerolize(_begin_d.getDate(), 2);
}
