 ...
 var Pane = {
      ...
      <div class='basic_info group'>${
        type == 'tree' ?
          ...
          <div class="row" style="height: 40px;align-items: center;">
            ...
            <div class="col">
              <input id='need_release' disabled/>
            </div>
          </div>
          <div class="row" style="height: 40px;align-items: center;">
          ...
          <div class="row" style="height: 40px;align-items: center;">
            <div class='col' style='width: 100%;'>
              <input id="tree_name" style='width: calc(50% + 232px)'>
            </div>
          </div>
          <div class="row" style="height: 40px;align-items: center;">
          ...
          <div class="row" style="height: 40px;align-items: center;">
            <div class="col">
              <div class="combo" id="is_public"></div>
            </div>
            <div class="col">
              <input id="tree_app_key">
            </div>
          </div>
          <div class='row'>
        ...
        :
          ...
          <div class="row">
            <div class="col">
              <input id="folder_name" disabled>
              <label id="change_name">${i18n('NAVIGATE_EDITOR_MODIFY')}</label>
            </div>
            <div class="row">
              <input id="node_id">
            </div>
          ...
          <div class="row">
            <div class="col">
              <div class="combo" id="is_manul"></div>
            </div>
            <div class="row">
              <input id="node_no">
            </div>
   ...
   initFrame(type){
     switch (type) {
       ...
       this.widget.setNavPath = new Combo('#set_nav_path', [{
            key: i18n('NAVIGATE_YES'),
            value: true
          }, {
            key: i18n('NAVIGATE_NO'),
       ...
       default:
         ...
         key: i18n('NAVIGATE_CREATE_FOLDER_ISMANUAL_MANUAL'),
            value: true
          }, {
         key: i18n('NAVIGATE_CREATE_FOLDER_ISMANUAL_AUTO'),
            value: false
          }], {
           backgroundIMGStyle: 2,
           width: '100%',
     ...
     this.widget.appType = new Combo('#node_app_type', appType.map((item, index) => {
       return {
         key: item.appKey,
         value: index
       }
   ...
   loadFrame({type, data = {}}) {
     if (type == 'tree') {
       ...
       $('.attr .basic_info #tree_id').val(data.id||"");
       $('.attr .basic_info #tree_name').val(data.name||"");
       $('.attr .basic_info #tree_app_key').val(data.creator_app_key || Model.tree.creator_app_key || "");
       $('.attr .basic_info #need_release').val(typeof need_release == 'boolean' ? i18n('NAVIGATION_' + String(data.metadata_public.need_release).toUpperCase()) : "");
       $('.attr .basic_info #tags').val((data.metadata_public.tags||[]).join(',')||"");
       ...
       $('.attr .basic_info #node_type').val(i18n('NAVIGATE_CREATE_FOLDER_FOLDER'));
       $('.attr .basic_info #folder_name').val(data.name||"");
       $('.attr .basic_info #node_id').val(data.ext_id||"");
       $('.attr .basic_info #title').val(data.title||"");
       $('.attr .basic_info #node_no').val(data.index||"");
       Pane.widget.manul.setValue(data.is_manual);
       switch (data.is_manual) {
         case true:
          $('.attr .basic_info .manual').addClass('active');
          $('.attr .basic_info #desc').val(data.description||"");
           break;
         default:
          $('.attr .basic_info .manual').removeClass('active');
          $('.attr .basic_info #objects_url').val(data.objects_url||"");
          $('.attr .basic_info #objects_url_script').val(data.objects_url_script.join(',')||"");
       }
     }
   ...
   bindEvents(){
     // metadata add
     $('#tree_editor .tab-ctn [name=attr] .group')
     ...
     .on('click', '#change_name', ({currentTarget, target}) => {
       if (target ==currentTarget) {
         var isActive = $(currentTarget).hasClass('active');
         if (isActive) {
              API.updateName({
                name: $(currentTarget).siblings('#folder_name').val()
              }, () => {
             $(currentTarget).removeClass('active').html(i18n('NAVIGATE_EDITOR_MODIFY')).siblings('#folder_name').attr('disabled', 'disabled');
             API.getNode((resp) => {
               Pane.loadFrame({
                 type: Pane.type,
                 data: resp
           ...
           $(currentTarget).addClass('active').html(i18n('NAVIGATE_EDITOR_FINISH')).siblings('#folder_name').attr('disabled', null);
         }
       }
     });
    },
    getValue() {
      ...
      if (this.type !== 'tree') {
        ...
        ret.ext_id = $('#tree_editor .tab-ctn [name=attr] #node_id').val()
        ret.title = $('#tree_editor .tab-ctn [name=attr] #title').val()
        var is_manual = this.widget.manul.getValue() == 'true' ? true : false;
        ret.is_manual = is_manual;
        ret.index = parseInt($('#tree_editor .tab-ctn [name=attr] #node_no').val())
        if (is_manual) {
          ret.description = $('#tree_editor .tab-ctn [name=attr] #desc').val()
          ret.objects_url = '';
          ret.objects_url_script=[]
        } else {
          ret.description=''
          ret.objects_url = $('#tree_editor .tab-ctn [name=attr] #objects_url').val()
          ret.objects_url_script = $('#tree_editor .tab-ctn [name=attr] #objects_url_script').val().split(',').filter(item => item)
        }
      } else {
        ret.is_public = this.widget.public.getValue() == 'true' ? true : false;
        ret.creator_app_key = $('#tree_editor .tab-ctn [name=attr] #tree_app_key').val();
        ret.metadata_public.needSetNavPath = this.widget.setNavPath.getValue() == 'true' ? true : false;
        ret.metadata_public.tags = $('#tree_editor .tab-ctn [name=attr] #tags').val().split(',').filter(item => item)
      }
