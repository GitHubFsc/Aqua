var authStats = (function($){
	...
	var stats = {
		...
		updateTrendAuthori: function(setting){
			var self = this;
			if(window.authStatsAuthoriInterval){
				clearInterval(window.authStatsAuthoriInterval);
			}
			window.authStatsAuthoriInterval = setInterval(function(){
				if($('#authstats-trend-authori').length == 0){
					clearInterval(window.authStatsAuthoriInterval);
					window.authStatsAuthoriInterval = null;
				} else {
		...
		updateTrendAuthenti: function(setting){
			var self = this;
			if(window.authStatsAuthentiInterval){
				clearInterval(window.authStatsAuthentiInterval);
			}
			window.authStatsAuthentiInterval = setInterval(function(){
				if($('#authstats-trend-authenti').length == 0){
					clearInterval(window.authStatsAuthentiInterval);
					window.authStatsAuthentiInterval = null;
				} else {
