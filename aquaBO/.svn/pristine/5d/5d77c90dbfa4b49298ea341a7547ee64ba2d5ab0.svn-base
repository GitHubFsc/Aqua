...
(function(JS){
    ...
    analysis.programHistory = {
        ...
        setPickerWidgets: function() {
            ...
            var calendar = new Calendar('au_time_picker_calendar');
            var calendarStyles = {
                width: 250,
                navTitleHeight: 25,
            ...
            function checkDatesState() {
                ...
                jQuery('#au_time_picker_calendar').hide().show();
            ...
            calendar.onCalendarRefresh = function() {
                ...
                self.checkedDates.weekdays = jQuery('#au_time_picker_calendar').find('.calendar-weekday').css('cursor', 'pointer').unbind().click(function(ev) {
                ...
                self.checkedDates.monthLabel = jQuery('#au_time_picker_calendar').find('.calendar-month-label').css('cursor', 'pointer').unbind().click(function(ev) {
                    var dateElements = calendar.getCurrMonthDateElements();
            ...
            var beginHour = new StyledSelector({
                containerId: 'au_time_picker_start_hour',
                options: hourOptions,
                styles: selStyles
            });

            var beginMin = new StyledSelector({
                containerId: 'au_time_picker_start_min',
                options: minOptions,
                styles: selStyles
            });

            var endHour = new StyledSelector({
                containerId: 'au_time_picker_end_hour',
                options: hourOptions,
                styles: selStyles
            });

            var endMin = new StyledSelector({
                containerId: 'au_time_picker_end_min',
                options: minOptions,
                styles: selStyles
            });

            var channelsDropdown = new ChannelDropdown({
                container: '#au_time_picker_channels_dropdown',
                styles: selStyles
            });
