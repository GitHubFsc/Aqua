var UploadModule = (function ($) {
  ...
  function UploadModule(aquaHost, storage_username, storage_password, storage_domain) {
    var that = this;
    that.my_aqua = getMy_aqua();
    that.my_aqua.aquaHost = aquaHost;
    that.my_aqua.restRoot = that.my_aqua.aquaHost + "/aqua/rest/cdmi";
    that.STATUS = "enable";
    that.file_status_array = [];
    that.files_status_upload = false;
    that.allUploadDataArray = [];
    that.uploadFilesNumber = 0;
    that.my_aqua.init(storage_username, storage_password, storage_domain);
  }
  ...
  UploadModule.prototype = {
    ...
    init: function (setting, simpleFun, allFun) {
      ...
      if (typeof(allFun) == "object") {
        this.getFileRenameResult = allFun._getFileRenameResult ? allFun._getFileRenameResult : function (result) {
            return result
          };//所有文件-重命名回调函数
    ...
    startuploadFile: function () {
      ...
      that.allUploadDataArray = new Array(that.files.files.length);
      if (typeof that.allStartFun !== 'undefined') {
        that.allStartFun && that.allStartFun(function () {
          that.uploadFile(that.files)
        });
      } else {
        that.uploadFile(that.files)
      }
      ...
      if (_file && _file.files && _file.files.length > 0) {
        ...
        if (that.checkExist_Boolean) {
          ...
          for (i = 0; i < _file.files.length; i++) {
            ...
            aquaFile = new that.my_aqua.file({
              name: name,
          ...
          if (existArray.length > 0) {
            that.checkAllExistFun && that.checkAllExistFun(existArray, function () {
              for (i = 0; i < _file.files.length; i++) {
      ...
      var config = {
        ...
        onerror: function (e) {
          if ($.isFunction(that.errorFun)) {
            ...
            that.errorFun(e, _file.files[i].name);
          }
        ...
        onend: function (updateSize, fileSize, self) {
          ...
          if ($.isFunction(that.endFun)) {
            ...
            file_url = that.my_aqua.getDownloadFileURL(file_url);
            if (file_url.indexOf('?aquatoken=') >= 0) {
              file_url = file_url.split("?aquatoken=")[0];
            }
            that.allUploadDataArray[i] = {
              name: aquaFile.name,
              file_url: file_url
    ...
    uploadJson: function (setting, simpleFun) {
      var that = this;
      that.initParm();
      var path = setting.path, name = setting.name, data = setting.data;
      that.uploadPath = path;
      if (typeof(simpleFun) == "object") {
        this.startFun = simpleFun._startfun;  //已开始上传情况下回调函数
        this.progressFun = simpleFun._progressfun; //上传过程中回调函数
        this.errorFun = simpleFun._errorfun; //上传失败回调函数
      ...
      var config = {
        ...
        onstart: function (updateSize, fileSize) {
          var self = this;
          if ($.isFunction(that.startFun))
            that.startFun(file.name, updateSize, fileSize, self);
        },
        onprogress: function (updateSize, fileSize) {
          var self = this;
          if ($.isFunction(that.progressFun))
            that.progressFun(file.name, updateSize, fileSize, self);
        },
        onerror: function (e) {
          if ($.isFunction(that.errorFun)) {
            that.errorFun(e, file.name);
          }
          this.cancel();
          return false;
        },
        onend: function (updateSize, fileSize, self) {
          ...
          if ($.isFunction(that.endFun)) {
            ...
            that.endFun(aquaFile.name, updateSize, fileSize, file_url, self, null, null);
          }
          this.cancel();
        }
      };
  ...
  var getMy_aqua = function () {
    var my_aqua = {};

    my_aqua.restRoot = '' + "/aqua/rest/cdmi";
...
//my_aqua.password = null;
    ...
    my_aqua.file.prototype.setMetadata = function (metadata) {
      ...
      if (metadata && this.isExist == true) {
        ...
        if (names.length > 0) {
          ...
          xhr.send('{"mimetype":"' + my_aqua.getContentType(this.name) + '","metadata":' + dojo.toJson(putMetadata) + '}');
    ...
    my_aqua.getDownloadFileURL = function (URI) {
      ...
      if (URI.indexOf('http://') < 0) {
        URI = my_aqua.aquaHost + URI.slice(2);
      }
...
// };
    ...
    my_aqua.folder.prototype.copy = function (dest_folder_url, synccopy) {
      var ret = false;
      if (this.isExist === true && dest_folder_url && dest_folder_url !== '') {
        var xhr = new my_aqua.xhr();
        var url = my_aqua.restRoot + my_aqua.encodePath(dest_folder_url) + encodeURIComponent(this.name) + '/';
        if (synccopy) {
          url = url + "?synccopy=" + synccopy;
        }
        xhr.open("PUT", url, false);
        my_aqua.addXHRHeaderRequest(xhr, "PUT", url, "application/cdmi-container");
    ...
    my_aqua.init = function (userName, password, domain) {
      ...
      my_aqua.Authorization = "Basic " + _base64Encode(my_aqua.utf16to8(my_aqua.userName + ":" + password));
      var xhr = new my_aqua.xhr();
      ...
      xhr.setRequestHeader("Authorization", "Basic " + _base64Encode(my_aqua.utf16to8(my_aqua.userName + ":" + password)));
      //    console.info(my_aqua.base64Decode(_base64Encode(my_aqua.userName + ":" + password)))
    ...
    if (typeof FileReader != 'undefined') {
      ...
      my_aqua.createFile = function (args) {
        ...
        var obj = {
          mimetype: my_aqua.getContentType(this.fileName),
          metadata: {
      ...
      my_aqua.createFile.prototype._updateFileBloToAqua = function (bloNumber, buff) {
        ...
        if (this.isPause == false) {
          ...
          //this.xhr.open("PUT", my_aqua.getHasTokenUrl("PUT", this.url, 600), true);
          //this.xhr.setRequestHeader("Accept", my_aqua.getContentType(this.fileName));
          //this.xhr.setRequestHeader("X-CDMI-Specification-Version", "1.0.2");
          //this.xhr.setRequestHeader("Authorization", my_aqua.Authorization);
          //this.xhr.setRequestHeader("Content-Type", my_aqua.getContentType(this.fileName));
          this.xhr.open("PUT", this.url, true);
          my_aqua.addXHRHeaderRequest(this.xhr, "PUT", this.url, my_aqua.getContentType(this.fileName));
    ...
    my_aqua.getContentType = function (str) {
      var ret = "application/octet-stream";
      var a = str.split(".");
      if (a.length > 0) {
        if (my_aqua.contentTypeMap[a[a.length - 1].toLowerCase()]) {
          ret = my_aqua.contentTypeMap[a[a.length - 1].toLowerCase()];
        }
      }
      return ret;
    }
