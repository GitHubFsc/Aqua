 ...
 */
function includePage(container,folder,html,jses,callback){
	if(folder && container){
		container = $$(container);
		...
		//加载新页面前移出前一页
		removePage();
		pageObj.load(callback);
		priviousPage = pageObj;
		if(!isNull(navigate.intervalId) && navigate.intervalId != ""){
			clearInterval(navigate.intervalId);
		}
	}
}
...
function innerFile(container,htmlFile){
    ...
    xmlHttp.onreadystatechange = function(){
		...
		if(strHtml && container){
			...
			var tmpChild = null;
			for(var i = 0; i < len; i++){
				tmpChild = container.children[i];
				if(tmpChild){
					if(tmpChild.style.display != "none"){
						height += tmpChild.offsetHeight;
					}
					tmpChild = null;
				}
			}
			//当注入的内容比父容（DIV）高时，改变它的高度
...
function paasLogin(user_name, pwd, callback, errCallback){
  var queryUrl =aquapaas_host+ '/aquapaas/rest/users/access_token/basic';
...
function getPaaSUser(id, token, callback){
  ...
  var queryUrl =aquapaas_host+ '/aquapaas/rest/users/user/' + id + query;
  jQuery.ajax({
    type: 'GET',
    url: queryUrl,
    dataType: 'json',
...
function aquaLogin(name, pwd, domainUri, callback, errCallback){
  jQuery.ajax({
    type: 'GET',
    url:aquapaas_host+ '/aqua/rest/cdmi/cdmi_users/' + name,
    headers: {
