var treeEditor = (function($, Tree, Alert){
  ...
  let Pane = {
    ...
    init() {
      ...
      var dom = `<div class='_frame _flex' id='tree_editor'>
      <div class='header _flex'>
      <div class='header_item logo'>${i18n('NAVIGATE_MNG')}</div>
      <div class='header_item span'></div>
      ${need_release ? `<div class="header_item btn icon" id="audit_tree">${i18n('NAVIGATE_AUDIT')}</div>`: ''}
      <div class='header_item btn icon' id='create_folder'>${i18n('NAVIGATE_EDITOR_CREATEFOLDER')}</div>
      </div>
      <div class='body'>
        <ul id='nav_tree' class='zTree'></ul>
        <div id='tree_detail'>
          <div class='tabs'>
            <div class='tab active' name='attr' style='display: none;' title='${i18n("NAVIGATE_EDITOR_TREEATTR")}'>${i18n('NAVIGATE_EDITOR_TREEATTR')}</div>
            ${haveAqua ? `<div class='tab' name='file_attr' style='display: none;' title='${i18n("NAVIGATE_FOLDER_ATTR")}'>${i18n('NAVIGATE_FOLDER_ATTR')}</div>`: ``}
            <div class='tab' name='bundle' style='display: none;' title='${i18n("NAVIGATE_EDITOR_BUNDLEMANUAL")}'>${i18n('NAVIGATE_EDITOR_BUNDLEMANUAL')}</div>
            <div class='tab' name='asset_search' style='display: none;' title='${i18n("NAVIGATE_EDITOR_BUNDLESEARCH")}'>${i18n('NAVIGATE_EDITOR_BUNDLESEARCH')}</div>
            <div class='tab' name='folder' style='display: none;' title='${i18n("NAVIGATE_EDITOR_BUNDLELIST")}'>${i18n('NAVIGATE_EDITOR_BUNDLELIST')}</div>
            ${need_release? `<div class='tab' name='vers' style='display: none;' title='${i18n("NAVIGATE_EDITOR_VERS")}'>${i18n('NAVIGATE_EDITOR_VERS')}</div>
            <div class='tab' name='audit_his' style='display: none;' title='${i18n("NAVIGATE_EDITOR_AUDITHIS")}'>${i18n('NAVIGATE_EDITOR_AUDITHIS')}</div>
            <div class='tab' name='his_ver' style='display: none;' title='${i18n("NAVIGATE_EDITOR_HISVER")}'>${i18n('NAVIGATE_EDITOR_HISVER')}</div>` : ``}
          </div>
          <div class='tab-ctn'>
            <div name='attr'></div>
    ...
    bindEvents() {
      ...
      .on('click', '#audit_tree', ({currentTarget, target}) => {
      ...
      .on('click', '.tabs .tab', ({target, currentTarget}) => {
        if (target == currentTarget) {
          ...
          $('#nav_mng .tab-ctn > div[name=' + type + ']').show().siblings().hide();//change ctn status
          //保存缓存信息
    ...
    initWidgets() {
      Tree.init($('#nav_tree'), Model.treeSetting(), Model.tree(Model.tree_data));
      Pane.tree = Tree.getZTreeObj('nav_tree');
    },
    treeClick(e, treeId, treeNode, clickFlag) {
      ...
      Pane.tmpAttr = null;
      var activePane = $('#tree_editor .tabs .tab.active').attr('name');
      let {node_class, is_manual} = Pane.curFocus.data;
      var tabs = ['attr','file_attr', 'folder']
      switch (node_class) {
        case 'folder':
          $('#tree_editor .tabs .tab[name=attr]').html(i18n('NAVIGATE_EDITOR_FOLDERATTR'))
        ...
        default:
          ...
          $('#tree_editor .tabs .tab[name=attr]').html(i18n('NAVIGATE_EDITOR_TREEATTR'))
      ...
      switch (is_manual) {
        case true:
          ...
          $('#tree_editor .tabs .tab[name=bundle]').html(i18n('NAVIGATE_EDITOR_BUNDLEMANUAL'))
          tabs.push('bundle')
          break;
        case undefined:
        ...
        default:
          ...
          $('#tree_editor .tabs .tab[name=bundle]').html(i18n('NAVIGATE_EDITOR_BUNDLEAUTO'))
      }
      tabs.push('asset_search');
      $('#tree_editor .tabs .tab').hide()
      for (var i = 0; i < tabs.length; i++) {
        $('#tree_editor .tabs .tab[name=' + tabs[i] + ']').show()
      }
      if (!tabs.includes(activePane)) {
        activePane = tabs[0]
        $('#tree_editor .tabs .tab[name=' + activePane + ']').click();
      } else {
        switch (activePane) {
          case 'attr':
            Pane.loadAttrFrame();
    ...
    getTree() {
      ...
      $.ajax({
        ...
        if (state == 'success') {
          ...
          $("#nav_tree").find('#' + Pane.tree.getNodes()[0].tId + '_span').click();
        }
      })
    },
        ...
        callback: {
          ...
          onAsyncSuccess: (event, treeId, treeNode, msg)=>{
            ...
            if (treeNode.needClick) {
              ...
              var treeObj =Tree.getZTreeObj("nav_tree");
