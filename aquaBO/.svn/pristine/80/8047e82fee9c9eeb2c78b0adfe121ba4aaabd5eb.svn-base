  ...
  var Pane = {
    ...
    bindEvents(){
      $('#tree_creator')
        ...
        Pane.widgets.Public = new Combo('#tree_public', [{key: i18n('NAVIGATE_NO'),value: false}, {key: i18n('NAVIGATE_YES'),value: true}], {
        ...
        Pane.widgets.NeedRelease = new Combo('#need_release', [{key: i18n('NAVIGATE_NO'),value: false}, {key: i18n('NAVIGATE_YES'),value: true}], {
          backgroundIMGStyle: 2,
          width: 165,
          height: 30,
          background: "#ffffff",
        ...
        Pane.widgets.SetNavPath = new Combo('#set_nav_path', [{key: i18n('NAVIGATE_YES'),value: true},{key: i18n('NAVIGATE_NO'),value: false}], {
          backgroundIMGStyle: 2,
          width: 165,
          height: 30,
          background: "#ffffff",
    ...
    load({el, data}){
      ...
      switch (el) {
        case 'public':
          var source = Model.metadata(index, 'source', data.source).appendTo($('#tree_creator .group.public_info'));
          index++;
          var readonly = Model.metadata(index, 'readonly', data.readonly).appendTo($('#tree_creator .group.public_info'))
          index++;
          delete data.source
          delete data.readonly
          for (var item in data) {
            if (data.hasOwnProperty(item)) {
              Model.metadata(index, item, data[item]).appendTo($('#tree_creator .group.public_info'));
              index++;
            }
          }
          Model.metadata(index, '', '').appendTo($('#tree_creator .group.public_info'));
          break;
        default:
        for (var item in data) {
          if (data.hasOwnProperty(item)) {
            Model.metadata(index, item, data[item]).appendTo($('#tree_creator .group.metadata_info'));
            index++;
          }
        }
        Model.metadata(index, '', '').appendTo($('#tree_creator .group.metadata_info'));
      }
    }
  };
  var API = {
    ...
    getData(){
      ...
      data.name = $('#tree_creator #tree_name').val();
      ...
      data.creator_app_key = $('#tree_creator #tree_app').val();
      ...
      if (data.metadata_public.needSetNavPath) {
        data.metadata_public.tags = $('#tree_creator #tags').val().split(',').filter(item => !(item == ''  || typeof item == 'undefined' || item == null || isNaN(item)));
      }
      ...
      // })
      data.metadata = {};
      $('#tree_creator .group.metadata_info .row').map((idx, item)=>{
        var key = $(item).find('.metadata_key').val();
        var val = $(item).find('.metadata_val').val();
        if (val && key) {
    ...
    sendable(data) {
      ...
      if (!!!data.name) {
        $('#tree_creator #tree_name').css('border-color', 'red')
        ret = false;
      } else {
        $('#tree_creator #tree_name').css('border-color', '')
      }
      if (!!!data.creator_app_key) {
        $('#tree_creator #tree_app').css('border-color', 'red')
        ret = false;
      } else {
        $('#tree_creator #tree_app').css('border-color', '')
      }
      return ret;
    }
  };
