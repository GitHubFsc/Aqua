  ...
  let Pane = {
    ...
    initTable(){
      ...
      let style = {
        ...
        iconColor: "#0099CB",
        columnsWidth: [0.06, 0.09, 0.13, 0.21, 0.2, 0.22, 0.08]
      }
  ...
  let Model = {
    ...
    table(resp){
      ...
      for (var i = 0; i < resp.length; i++) {
        var item = resp[i];
        var row = [],
        {ispackage,provider_id, provider_asset_id, title,  visible, category = []} = item;//licensing_window_start,
        let check_box = '<div class="i-check"><input name="check" type="checkbox" data-index="' + i + '"><label></label></div>';
        row.push({
          label: check_box
        })
        row.push({
            label:i18n('ASSET_FILTER_SELECT_' + (ispackage == 'Y' ? 'bundle': 'asset').toUpperCase())
        })
        row.push({
          label: provider_id
        })
