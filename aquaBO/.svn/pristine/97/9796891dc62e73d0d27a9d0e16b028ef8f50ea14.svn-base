var trendAnalysis = (function ($) {
  ...
  trendAnalysis.init = function () {
    ...
    that.initParm();
    that.initTime();
    that.updateOptionsForGrouping("display_container_fenzu", "grouping", that.options.groupingInput_Type, []);
    that.bindEvent();
    that.initLineChart();
    that.initTable();
  };
  ...
  trendAnalysis.initTime = function () {
    ...
    var topNSelect = new newSelect("#topN", select_options, {
      width: 115,
      height: 32,
      background: "#ffffff",
      fontSize: 12,
  ...
  trendAnalysis.bindEvent = function () {
    ...
    $("#trendAnalysis_quota_button").unbind("click").click(function () {
      trendAnalysis_Lib.openQuotaDialog(that.options.SelectedQuotaArray, function(result){
        ...
        that.updateOptionsForGrouping("display_container_fenzu", "grouping", that.options.groupingInput_Type, that.options.GroupData);
    ...
    //分组在单选情况下的逻辑
    $('#display_container_fenzu').unbind("click").on('click', '.trendAnalysis_checkbox .checkbox input[type=radio]', function (ev) {
      var $radio = $(this);
      if ($radio.data('radiochecked')) {
        $('#display_container_fenzu .trendAnalysis_checkbox .checkbox input[type=radio]').data('radiochecked', false);
    ...
    $("#trendAnalysis_filtration_button").unbind("click").click(function () {
      ...
      trendAnalysis_Lib.openFilterDialog(AllGroupingArray, function (result) {
        that.updateOptionsForFilter("filter_display_container", "filter", "checkbox", result);
        that.options.FilterData = that.getAllGroupingArray();
        that.topNchange();
      });
    });
    //过滤内的checkbox勾选事件触发是否显示TopN对话框
    $('#filter_display_container').unbind("click").on('click', '.trendAnalysis_checkbox.filter', function (ev) {
      that.topNchange();
    });
    //重置条件
  ...
  trendAnalysis.topNchange = function(){
    ...
    $("#filter_display_container .trendAnalysis_checkbox>.checkbox>input:checked").each(function (index, el) {
      filterCheckedNumber = filterCheckedNumber + 1;
    });
    $("#display_container_fenzu .trendAnalysis_checkbox>.checkbox>input:checked").each(function (index, el) {
  ...
  trendAnalysis.updateOptionsForGrouping = function (containerID, tag, checkboxType, data) {
    ...
    var selectContainer = $("<div class='option_content_left' id='GroupingSelect'></div>");
    subContainer.append(selectContainer);
    var groupingSelect = new newSelect("#GroupingSelect", [], {
      width: 90,
      height: 30,
      background: "#ffffff",
      fontSize: 12,
      ...
      $("#display_container_fenzu .trendAnalysis_checkbox>.checkbox>input:first").prop('checked', true);
      that.emitFilterChange();
      that.topNchange();
    });
    ...
    var GroupingCheckbox = $("<div class='option_content_right' id='GroupingCheckbox'></div>");
  ...
  trendAnalysis.getAllGroupingArray = function () {
    var result = [];
    $("#display_container_fenzu .trendAnalysis_checkbox>.checkbox>input").each(function (index, el) {
      var id = $(el).data("id") + '';
      ...
      result.push({
        ...
        selected: (function () {
          var result = [];
          $("#filter_display_container .trendAnalysis_checkbox>.checkbox>input[data-subtag='" + id + "']:checked").each(function (index, el) {
